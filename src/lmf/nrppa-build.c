        ...
        uint8_t requested_measurements)
{
    ...
    if (!requested_measurements) {
        log_warn("Requested measurements bitmap is zero, forcing CELL_ID|SS_RSRP");
        requested_measurements =
            NRPPA_ECID_MEASUREMENT_TYPE_RSRP;
    }

    log_info("Building NRPPa ECID request (v17.2.0 ProtocolIE format): "
    ...
    if (!ie) {
        log_error("CALLOC failed for ProtocolIE-Field");
        goto err;
    }
    ...
    if (rv != 0) {
        log_error("ASN_SEQUENCE_ADD failed for UE-Measurement-ID, rv=%d", rv);
        ogs_free(ie);
        goto err;
    }
    ie = NULL;
    ...
    rv = ASN_SEQUENCE_ADD(&request.protocolIEs.list, ie);
    if (rv != 0) {
        log_error("ASN_SEQUENCE_ADD failed for ReportCharacteristics, rv=%d", rv);
        ogs_free(ie);
        goto err;
    }
    ie = NULL;
    ...
    if (!quantity_ie) {
        log_error("CALLOC failed for MeasurementQuantities_ItemIEs");
        ogs_free(ie);
        goto err;
    }
    ...
    if (rv != 0) {
        log_error("ASN_SEQUENCE_ADD failed for cell_ID, rv=%d", rv);
    ...
    if (requested_measurements & NRPPA_ECID_MEASUREMENT_TYPE_RSRP) {
        quantity_ie = CALLOC(1, sizeof(*quantity_ie));
        if (!quantity_ie) {
            log_error("CALLOC failed for MeasurementQuantities_ItemIEs (SS-RSRP)");
            goto err;
        }
        quantity_ie->id = ProtocolIE_ID_id_MeasurementQuantities_Item;
        quantity_ie->criticality = Criticality_reject;
        ...
        rv = ASN_SEQUENCE_ADD(&ie->value.choice.MeasurementQuantities.list, quantity_ie);
        if (rv != 0) {
            log_error("ASN_SEQUENCE_ADD failed for sS_RSRP, rv=%d", rv);
            ogs_free(quantity_ie);
            goto err;
        }
        quantity_ie = NULL;
    ...
    if (requested_measurements & NRPPA_ECID_MEASUREMENT_TYPE_TA) {
        quantity_ie = CALLOC(1, sizeof(*quantity_ie));
        if (!quantity_ie) {
            log_error("CALLOC failed for MeasurementQuantities_ItemIEs (TA)");
            goto err;
        }
        quantity_ie->id = ProtocolIE_ID_id_MeasurementQuantities_Item;
        quantity_ie->criticality = Criticality_reject;
        ...
        rv = ASN_SEQUENCE_ADD(&ie->value.choice.MeasurementQuantities.list, quantity_ie);
        if (rv != 0) {
            log_error("ASN_SEQUENCE_ADD failed for timingAdvanceNR, rv=%d", rv);
            ogs_free(quantity_ie);
            goto err;
        }
        quantity_ie = NULL;
    ...
    if (requested_measurements & NRPPA_ECID_MEASUREMENT_TYPE_AOA) {
        quantity_ie = CALLOC(1, sizeof(*quantity_ie));
        if (!quantity_ie) {
            log_error("CALLOC failed for MeasurementQuantities_ItemIEs (AoA-NR)");
            goto err;
        }
        quantity_ie->id = ProtocolIE_ID_id_MeasurementQuantities_Item;
        quantity_ie->criticality = Criticality_reject;
        ...
        rv = ASN_SEQUENCE_ADD(&ie->value.choice.MeasurementQuantities.list, quantity_ie);
        if (rv != 0) {
            log_error("ASN_SEQUENCE_ADD failed for angleOfArrivalNR, rv=%d", rv);
            ogs_free(quantity_ie);
            goto err;
        }
        quantity_ie = NULL;
        log_info("  Added angleOfArrivalNR to measurement quantities");
    }

    /* Add MeasurementQuantities IE to protocolIEs */
    rv = ASN_SEQUENCE_ADD(&request.protocolIEs.list, ie);
    if (rv != 0) {
        log_error("ASN_SEQUENCE_ADD failed for MeasurementQuantities, rv=%d", rv);
        ogs_free(ie);
        goto err;
    }
    ie = NULL;
    ...
    /* Verify structure */
    log_info("  protocolIEs.list.count = %d", request.protocolIEs.list.count);
    if (request.protocolIEs.list.count < 3) {
        log_error("Protocol IEs count is %d, expected at least 3", request.protocolIEs.list.count);
        goto err;
    }
    ...
    if (!request_pkbuf) {
        log_error("ogs_asn_encode failed for request");
        goto err;
    }
    ...
    *p++ = transaction_id & 0xFF;  /* Low byte */
    ...
    log_info("NRPPa InitiatingMessage (E-CID Request) built, "
