...
static location_data_t *location_data_create_internal(
    ...
    location_data_t *location_data_local_var = malloc(sizeof(location_data_t));
    if (!location_data_local_var) {
        return NULL;
    }
    memset(location_data_local_var, 0, sizeof(location_data_t));
    location_data_local_var->_library_owned = 1;
...
__attribute__((deprecated)) location_data_t *location_data_create(
    ...
    location_data_t *result = location_data_create_internal (
        location_estimate,
...
void location_data_free(location_data_t *location_data) {
    ...
    if(location_data->_library_owned != 1){
    ...
    if (location_data->accuracy_fulfilment_indicator) {
        accuracy_fulfilment_indicator_free(location_data->accuracy_fulfilment_indicator);
        location_data->accuracy_fulfilment_indicator = NULL;
    }
    if (location_data->age_of_location_estimate) {
        free(location_data->age_of_location_estimate);
        location_data->age_of_location_estimate = NULL;
    }
    ...
    if (location_data->positioning_data_list) {
        list_ForEach(listEntry, location_data->positioning_data_list) {
            positioning_method_and_usage_free(listEntry->data);
        }
        list_freeList(location_data->positioning_data_list);
        location_data->positioning_data_list = NULL;
    }
    if (location_data->gnss_positioning_data_list) {
        list_ForEach(listEntry, location_data->gnss_positioning_data_list) {
            gnss_positioning_method_and_usage_free(listEntry->data);
        }
        list_freeList(location_data->gnss_positioning_data_list);
    ...
    if (location_data->remote_ue_ind) {
        free(location_data->remote_ue_ind);
        location_data->remote_ue_ind = NULL;
    }
    if (location_data->altitude) {
        free(location_data->altitude);
        location_data->altitude = NULL;
    }
    if (location_data->barometric_pressure) {
        free(location_data->barometric_pressure);
        location_data->barometric_pressure = NULL;
    }
    ...
    if (location_data->direct_report_ind) {
        free(location_data->direct_report_ind);
        location_data->direct_report_ind = NULL;
    }
    if (location_data->indoor_outdoor_ind) {
        indoor_outdoor_ind_free(location_data->indoor_outdoor_ind);
        location_data->indoor_outdoor_ind = NULL;
    }
    if (location_data->accepted_periodic_event_info) {
        periodic_event_info_free(location_data->accepted_periodic_event_info);
    ...
    if (location_data->los_nlos_measure_ind) {
        los_nlos_measure_ind_free(location_data->los_nlos_measure_ind);
        location_data->los_nlos_measure_ind = NULL;
    }
    ...
    if (location_data->_2d_relative_location) {
        model_2_d_relative_location_free(location_data->_2d_relative_location);
        location_data->_2d_relative_location = NULL;
    }
    if (location_data->_3d_relative_location) {
        model_3_d_relative_location_free(location_data->_3d_relative_location);
    ...
    if (location_data->integrity_result) {
        integrity_result_free(location_data->integrity_result);
        location_data->integrity_result = NULL;
    }
    if (location_data->nrppa_periodic_ind) {
        free(location_data->nrppa_periodic_ind);
        location_data->nrppa_periodic_ind = NULL;
    }
    free(location_data);
}
...
cJSON *location_data_convertToJSON(location_data_t *location_data) {
    ...
    cJSON *location_estimate_local_JSON = geographic_area_convertToJSON(location_data->location_estimate);
    ...
    // location_data->accuracy_fulfilment_indicator
    if(location_data->accuracy_fulfilment_indicator) {
    cJSON *accuracy_fulfilment_indicator_local_JSON = accuracy_fulfilment_indicator_convertToJSON(location_data->accuracy_fulfilment_indicator);
    ...
    // location_data->age_of_location_estimate
    if(location_data->age_of_location_estimate) {
    ...
    // location_data->timestamp_of_location_estimate
    if(location_data->timestamp_of_location_estimate) {
    ...
    if(location_data->velocity_estimate) {
    cJSON *velocity_estimate_local_JSON = velocity_estimate_convertToJSON(location_data->velocity_estimate);
    ...
    if(location_data->civic_address) {
    cJSON *civic_address_local_JSON = civic_address_convertToJSON(location_data->civic_address);
    ...
    if(location_data->local_location_estimate) {
    cJSON *local_location_estimate_local_JSON = local_area_convertToJSON(location_data->local_location_estimate);
    ...
    // location_data->positioning_data_list
    if(location_data->positioning_data_list) {
    ...
    if (location_data->positioning_data_list) {
    list_ForEach(positioning_data_listListEntry, location_data->positioning_data_list) {
    ...
    // location_data->gnss_positioning_data_list
    if(location_data->gnss_positioning_data_list) {
    ...
    if (location_data->gnss_positioning_data_list) {
    list_ForEach(gnss_positioning_data_listListEntry, location_data->gnss_positioning_data_list) {
    ...
    // location_data->ecgi
    if(location_data->ecgi) {
    cJSON *ecgi_local_JSON = ecgi_convertToJSON(location_data->ecgi);
    ...
    // location_data->ncgi
    if(location_data->ncgi) {
    cJSON *ncgi_local_JSON = ncgi_convertToJSON(location_data->ncgi);
    ...
    // location_data->serving_lmf_identification
    if(location_data->serving_lmf_identification) {
    ...
    if(location_data->ue_area_ind) {
    cJSON *ue_area_ind_local_JSON = ue_area_indication_convertToJSON(location_data->ue_area_ind);
    ...
    if(location_data->achieved_qos) {
    cJSON *achieved_qos_local_JSON = minor_location_qo_s_convertToJSON(location_data->achieved_qos);
    ...
    // location_data->indoor_outdoor_ind
    if(location_data->indoor_outdoor_ind) {
    cJSON *indoor_outdoor_ind_local_JSON = indoor_outdoor_ind_convertToJSON(location_data->indoor_outdoor_ind);
    ...
    if(location_data->accepted_periodic_event_info) {
    cJSON *accepted_periodic_event_info_local_JSON = periodic_event_info_convertToJSON(location_data->accepted_periodic_event_info);
    ...
    if(location_data->ha_gnss_metrics) {
    cJSON *ha_gnss_metrics_local_JSON = high_accuracy_gnss_metrics_convertToJSON(location_data->ha_gnss_metrics);
    ...
    // location_data->los_nlos_measure_ind
    if(location_data->los_nlos_measure_ind) {
    cJSON *los_nlos_measure_ind_local_JSON = los_nlos_measure_ind_convertToJSON(location_data->los_nlos_measure_ind);
    ...
    // location_data->related_applicationlayer_id
    if(location_data->related_applicationlayer_id) {
    ...
    if(location_data->distance_direction) {
    cJSON *distance_direction_local_JSON = range_direction_convertToJSON(location_data->distance_direction);
    ...
    if(location_data->_2d_relative_location) {
    cJSON *_2d_relative_location_local_JSON = model_2_d_relative_location_convertToJSON(location_data->_2d_relative_location);
    ...
    if(location_data->_3d_relative_location) {
    cJSON *_3d_relative_location_local_JSON = model_3_d_relative_location_convertToJSON(location_data->_3d_relative_location);
    ...
    if(location_data->relative_velocity) {
    cJSON *relative_velocity_local_JSON = velocity_estimate_convertToJSON(location_data->relative_velocity);
    ...
    if(location_data->ranging_sl_capability) {
    if(cJSON_AddStringToObject(item, "rangingSlCapability", location_data->ranging_sl_capability) == NULL) {
    goto fail; //ByteArray
    }
    }
    ...
    if(location_data->integrity_result) {
    cJSON *integrity_result_local_JSON = integrity_result_convertToJSON(location_data->integrity_result);
    ...
    // location_data->nrppa_periodic_ind
    if(location_data->nrppa_periodic_ind) {
...
location_data_t *location_data_parseFromJSON(cJSON *location_dataJSON){
    ...
    // define the local variable for location_data->location_estimate
    geographic_area_t *location_estimate_local_nonprim = NULL;

    // define the local variable for location_data->accuracy_fulfilment_indicator
    accuracy_fulfilment_indicator_t *accuracy_fulfilment_indicator_local_nonprim = NULL;

    // define the local variable for location_data->age_of_location_estimate
    int *age_of_location_estimate_local_var = NULL;

    char *timestamp_of_location_estimate_local_str = NULL;

    // define the local variable for location_data->velocity_estimate
    velocity_estimate_t *velocity_estimate_local_nonprim = NULL;

    // define the local variable for location_data->civic_address
    civic_address_t *civic_address_local_nonprim = NULL;

    // define the local variable for location_data->local_location_estimate
    local_area_t *local_location_estimate_local_nonprim = NULL;

    // define the local list for location_data->positioning_data_list
    list_t *positioning_data_listList = NULL;

    // define the local list for location_data->gnss_positioning_data_list
    list_t *gnss_positioning_data_listList = NULL;

    // define the local variable for location_data->ecgi
    ecgi_t *ecgi_local_nonprim = NULL;

    // define the local variable for location_data->ncgi
    ncgi_t *ncgi_local_nonprim = NULL;

    // define the local variable for location_data->remote_ue_ind
    int *remote_ue_ind_local_var = NULL;

    // define the local variable for location_data->altitude
    double *altitude_local_var = NULL;

    // define the local variable for location_data->barometric_pressure
    ...
    *nrppa_periodic_ind_local_var = nrppa_periodic_ind->valueint;
    ...
    location_data_local_var = location_data_create_internal (
