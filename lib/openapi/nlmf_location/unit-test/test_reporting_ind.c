...
void test_reporting_ind(int include_optional) {
    reporting_ind_t* reporting_ind_1 = instantiate_reporting_ind(include_optional);

	cJSON* jsonreporting_ind_1 = reporting_ind_convertToJSON(reporting_ind_1);
	printf("reporting_ind :\n%s\n", cJSON_Print(jsonreporting_ind_1));
	reporting_ind_t* reporting_ind_2 = reporting_ind_parseFromJSON(jsonreporting_ind_1);
	cJSON* jsonreporting_ind_2 = reporting_ind_convertToJSON(reporting_ind_2);
	printf("repeating reporting_ind:\n%s\n", cJSON_Print(jsonreporting_ind_2));
}
