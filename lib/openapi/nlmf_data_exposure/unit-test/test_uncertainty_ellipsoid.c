...
void test_uncertainty_ellipsoid(int include_optional) {
    uncertainty_ellipsoid_t* uncertainty_ellipsoid_1 = instantiate_uncertainty_ellipsoid(include_optional);

	cJSON* jsonuncertainty_ellipsoid_1 = uncertainty_ellipsoid_convertToJSON(uncertainty_ellipsoid_1);
	printf("uncertainty_ellipsoid :\n%s\n", cJSON_Print(jsonuncertainty_ellipsoid_1));
	uncertainty_ellipsoid_t* uncertainty_ellipsoid_2 = uncertainty_ellipsoid_parseFromJSON(jsonuncertainty_ellipsoid_1);
	cJSON* jsonuncertainty_ellipsoid_2 = uncertainty_ellipsoid_convertToJSON(uncertainty_ellipsoid_2);
