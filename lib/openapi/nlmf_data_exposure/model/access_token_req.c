...
char* access_token_req_grant_type_ToString(nlmf_dataexposure_access_token_req_GRANTTYPE_e grant_type) {
    char* grant_typeArray[] =  { "NULL", "client_credentials" };
    return grant_typeArray[grant_type];
}

nlmf_dataexposure_access_token_req_GRANTTYPE_e access_token_req_grant_type_FromString(char* grant_type){
...
static access_token_req_t *access_token_req_create_internal(
    nlmf_dataexposure_access_token_req_GRANTTYPE_e grant_type,
    char *nf_instance_id,
    nf_type_t *nf_type,
    ...
    memset(access_token_req_local_var, 0, sizeof(access_token_req_t));
    access_token_req_local_var->_library_owned = 1;
...
__attribute__((deprecated)) access_token_req_t *access_token_req_create(
    nlmf_dataexposure_access_token_req_GRANTTYPE_e grant_type,
    char *nf_instance_id,
    nf_type_t *nf_type,
    nf_type_t *target_nf_type,
    ...
    access_token_req_t *result = access_token_req_create_internal (
        grant_type,
        nf_instance_id,
        nf_type,
        target_nf_type,
...
void access_token_req_free(access_token_req_t *access_token_req) {
    ...
    if(access_token_req->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "access_token_req_free");
        return ;
    }
    ...
    if (access_token_req->source_vendor_id) {
        free(access_token_req->source_vendor_id);
...
cJSON *access_token_req_convertToJSON(access_token_req_t *access_token_req) {
    ...
    if (nlmf_dataexposure_access_token_req_GRANTTYPE_NULL == access_token_req->grant_type) {
        goto fail;
    }
    if(cJSON_AddStringToObject(item, "grant_type", access_token_req_grant_type_ToString(access_token_req->grant_type)) == NULL)
    {
    goto fail; //Enum
    }
    ...
    if(cJSON_AddStringToObject(target_nsi_list, "", target_nsi_listListEntry->data) == NULL)
    {
        goto fail;
    }
    }
    ...
    cJSON *itemLocal = nwdaf_event_convertToJSON(analytics_idsListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
...
access_token_req_t *access_token_req_parseFromJSON(cJSON *access_token_reqJSON){
    ...
    char *nf_instance_id_local_str = NULL;
    ...
    char *target_nf_instance_id_local_str = NULL;
    ...
    char *requester_fqdn_local_str = NULL;
    ...
    char *target_nf_set_id_local_str = NULL;

    char *target_nf_service_set_id_local_str = NULL;

    char *hnrf_access_token_uri_local_str = NULL;

    char *source_nf_instance_id_local_str = NULL;

    char *vendor_id_local_str = NULL;
    ...
    char *source_vendor_id_local_str = NULL;

    char *af_id_local_str = NULL;
    ...
    nlmf_dataexposure_access_token_req_GRANTTYPE_e grant_typeVariable;
    ...
    if (cJSON_IsNull(nf_instance_id)) {
        nf_instance_id = NULL;
    }
    if (!nf_instance_id) {
        goto end;
    ...
    if (cJSON_IsNull(target_nf_type)) {
        target_nf_type = NULL;
    }
    if (target_nf_type) { 
    ...
    if (cJSON_IsNull(target_nf_instance_id)) {
    ...
    if (cJSON_IsNull(requester_plmn)) {
        requester_plmn = NULL;
    }
    if (requester_plmn) { 
    ...
    if (cJSON_IsNull(requester_plmn_list)) {
        requester_plmn_list = NULL;
    }
    ...
    if (cJSON_IsNull(requester_snssai_list)) {
        requester_snssai_list = NULL;
    }
    ...
    if (cJSON_IsNull(requester_fqdn)) {
        requester_fqdn = NULL;
    }
    if (requester_fqdn) { 
    ...
    if (cJSON_IsNull(requester_snpn_list)) {
        requester_snpn_list = NULL;
    }
    ...
    if (cJSON_IsNull(target_snssai_list)) {
        target_snssai_list = NULL;
    }
    ...
    if (cJSON_IsNull(target_nsi_list)) {
        target_nsi_list = NULL;
    }
    if (target_nsi_list) { 
    ...
    if (cJSON_IsNull(target_nf_set_id)) {
        target_nf_set_id = NULL;
    }
    if (target_nf_set_id) { 
    ...
    if (cJSON_IsNull(target_nf_service_set_id)) {
    ...
    if (cJSON_IsNull(hnrf_access_token_uri)) {
    ...
    if (cJSON_IsNull(source_nf_instance_id)) {
    ...
    cJSON_ArrayForEach(analytics_ids_local_nonprimitive,analytics_ids )
    {
        ...
        nwdaf_event_t *analytics_idsItem = nwdaf_event_parseFromJSON(analytics_ids_local_nonprimitive);
    ...
    if (cJSON_IsNull(requester_inter_ind_list)) {
        requester_inter_ind_list = NULL;
    }
    ...
    if (cJSON_IsNull(source_vendor_id)) {
    ...
    if (nf_instance_id && !cJSON_IsNull(nf_instance_id)) nf_instance_id_local_str = strdup(nf_instance_id->valuestring);
    if (scope && !cJSON_IsNull(scope)) scope_local_str = strdup(scope->valuestring);
    if (target_nf_instance_id && !cJSON_IsNull(target_nf_instance_id)) target_nf_instance_id_local_str = strdup(target_nf_instance_id->valuestring);
    if (requester_fqdn && !cJSON_IsNull(requester_fqdn)) requester_fqdn_local_str = strdup(requester_fqdn->valuestring);
    if (target_nf_set_id && !cJSON_IsNull(target_nf_set_id)) target_nf_set_id_local_str = strdup(target_nf_set_id->valuestring);
    if (target_nf_service_set_id && !cJSON_IsNull(target_nf_service_set_id)) target_nf_service_set_id_local_str = strdup(target_nf_service_set_id->valuestring);
    if (hnrf_access_token_uri && !cJSON_IsNull(hnrf_access_token_uri)) hnrf_access_token_uri_local_str = strdup(hnrf_access_token_uri->valuestring);
    if (source_nf_instance_id && !cJSON_IsNull(source_nf_instance_id)) source_nf_instance_id_local_str = strdup(source_nf_instance_id->valuestring);
    if (vendor_id && !cJSON_IsNull(vendor_id)) vendor_id_local_str = strdup(vendor_id->valuestring);
    if (source_vendor_id && !cJSON_IsNull(source_vendor_id)) source_vendor_id_local_str = strdup(source_vendor_id->valuestring);
    if (af_id && !cJSON_IsNull(af_id)) af_id_local_str = strdup(af_id->valuestring);

    access_token_req_local_var = access_token_req_create_internal (
        grant_typeVariable,
        nf_instance_id_local_str,
        nf_type ? nf_type_local_nonprim : NULL,
        target_nf_type ? target_nf_type_local_nonprim : NULL,
        scope_local_str,
        target_nf_instance_id_local_str,
        ...
        target_nf_set_id_local_str,
        target_nf_service_set_id_local_str,
        hnrf_access_token_uri_local_str,
        source_nf_instance_id_local_str,
        vendor_id_local_str,
        analytics_ids ? analytics_idsList : NULL,
        requester_inter_ind_list ? requester_inter_ind_listList : NULL,
        source_vendor_id_local_str,
        af_id_local_str
        );
    ...
    if (nf_instance_id_local_str) {
        free(nf_instance_id_local_str);
        nf_instance_id_local_str = NULL;
    }
    ...
    if (target_nf_instance_id_local_str) {
        free(target_nf_instance_id_local_str);
        target_nf_instance_id_local_str = NULL;
    }
    ...
    if (requester_fqdn_local_str) {
        free(requester_fqdn_local_str);
        requester_fqdn_local_str = NULL;
    }
    ...
    if (target_nf_set_id_local_str) {
        free(target_nf_set_id_local_str);
        target_nf_set_id_local_str = NULL;
    }
    if (target_nf_service_set_id_local_str) {
        free(target_nf_service_set_id_local_str);
        target_nf_service_set_id_local_str = NULL;
    }
    if (hnrf_access_token_uri_local_str) {
        free(hnrf_access_token_uri_local_str);
        hnrf_access_token_uri_local_str = NULL;
    }
    if (source_nf_instance_id_local_str) {
        free(source_nf_instance_id_local_str);
        source_nf_instance_id_local_str = NULL;
    }
    if (vendor_id_local_str) {
        free(vendor_id_local_str);
        vendor_id_local_str = NULL;
    }
    ...
    if (source_vendor_id_local_str) {
        free(source_vendor_id_local_str);
        source_vendor_id_local_str = NULL;
    }
    if (af_id_local_str) {
        free(af_id_local_str);
        af_id_local_str = NULL;
    }
    return NULL;
