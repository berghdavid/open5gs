...
{
    OpenAPI_location_data_ext_t *location_data_ext_local_var = ogs_malloc(sizeof(OpenAPI_location_data_ext_t));
    ...
    location_data_ext_local_var->is_age_of_location_estimate = is_age_of_location_estimate;
    ...
    location_data_ext_local_var->is_remote_ue_ind = is_remote_ue_ind;
    ...
    location_data_ext_local_var->is_barometric_pressure = is_barometric_pressure;
    ...
    location_data_ext_local_var->is_direct_report_ind = is_direct_report_ind;
    ...
    location_data_ext_local_var->is_nrppa_periodic_ind = is_nrppa_periodic_ind;
...
void OpenAPI_location_data_ext_free(OpenAPI_location_data_ext_t *location_data_ext)
{
    ...
    OpenAPI_geographic_area_free(location_data_ext->location_estimate);
    OpenAPI_accuracy_fulfilment_indicator_free(location_data_ext->accuracy_fulfilment_indicator);
    ogs_free(location_data_ext->timestamp_of_location_estimate);
    OpenAPI_velocity_estimate_free(location_data_ext->velocity_estimate);
    OpenAPI_civic_address_free(location_data_ext->civic_address);
    OpenAPI_local_area_free(location_data_ext->local_location_estimate);
    OpenAPI_list_for_each(location_data_ext->positioning_data_list, node) {
        OpenAPI_positioning_method_and_usage_free(node->data);
    }
    OpenAPI_list_free(location_data_ext->positioning_data_list);
    OpenAPI_list_for_each(location_data_ext->gnss_positioning_data_list, node) {
        OpenAPI_gnss_positioning_method_and_usage_free(node->data);
    }
    OpenAPI_list_free(location_data_ext->gnss_positioning_data_list);
    ...
    OpenAPI_minor_location_qo_s_free(location_data_ext->achieved_qos);
    OpenAPI_indoor_outdoor_ind_free(location_data_ext->indoor_outdoor_ind);
    OpenAPI_periodic_event_info_free(location_data_ext->accepted_periodic_event_info);
    OpenAPI_high_accuracy_gnss_metrics_free(location_data_ext->ha_gnss_metrics);
    OpenAPI_los_nlos_measure_ind_free(location_data_ext->los_nlos_measure_ind);
    ogs_free(location_data_ext->related_applicationlayer_id);
    OpenAPI_range_direction_free(location_data_ext->distance_direction);
    OpenAPI_model_2_d_relative_location_free(location_data_ext->_2d_relative_location);
    OpenAPI_model_3_d_relative_location_free(location_data_ext->_3d_relative_location);
    OpenAPI_velocity_estimate_free(location_data_ext->relative_velocity);
    OpenAPI_integrity_result_free(location_data_ext->integrity_result);
    OpenAPI_list_for_each(location_data_ext->add_location_datas, node) {
...
cJSON *OpenAPI_location_data_ext_convertToJSON(OpenAPI_location_data_ext_t *location_data_ext)
{
    ...
    if (location_data_ext == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [LocationDataExt]");
        return NULL;
    }

    item = cJSON_CreateObject();
    cJSON *location_estimate_local_JSON = OpenAPI_geographic_area_convertToJSON(location_data_ext->location_estimate);
    if (location_estimate_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [location_estimate]");
        goto end;
    }
    ...
    cJSON *accuracy_fulfilment_indicator_local_JSON = OpenAPI_accuracy_fulfilment_indicator_convertToJSON(location_data_ext->accuracy_fulfilment_indicator);
    if (accuracy_fulfilment_indicator_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [accuracy_fulfilment_indicator]");
        goto end;
    }
    ...
    if (cJSON_AddNumberToObject(item, "ageOfLocationEstimate", location_data_ext->age_of_location_estimate) == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [age_of_location_estimate]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "timestampOfLocationEstimate", location_data_ext->timestamp_of_location_estimate) == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [timestamp_of_location_estimate]");
        goto end;
    }
    }

    if (location_data_ext->velocity_estimate) {
    cJSON *velocity_estimate_local_JSON = OpenAPI_velocity_estimate_convertToJSON(location_data_ext->velocity_estimate);
    if (velocity_estimate_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [velocity_estimate]");
        goto end;
    }
    ...
    cJSON *civic_address_local_JSON = OpenAPI_civic_address_convertToJSON(location_data_ext->civic_address);
    if (civic_address_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [civic_address]");
        goto end;
    }
    ...
    cJSON *local_location_estimate_local_JSON = OpenAPI_local_area_convertToJSON(location_data_ext->local_location_estimate);
    if (local_location_estimate_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [local_location_estimate]");
        goto end;
    }
    ...
    if (positioning_data_listList == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [positioning_data_list]");
        goto end;
    }

    OpenAPI_lnode_t *positioning_data_list_node;
    if (location_data_ext->positioning_data_list) {
        OpenAPI_list_for_each(location_data_ext->positioning_data_list, positioning_data_list_node) {
    ...
    if (gnss_positioning_data_listList == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [gnss_positioning_data_list]");
        goto end;
    }

    OpenAPI_lnode_t *gnss_positioning_data_list_node;
    if (location_data_ext->gnss_positioning_data_list) {
        OpenAPI_list_for_each(location_data_ext->gnss_positioning_data_list, gnss_positioning_data_list_node) {
    ...
    cJSON *ecgi_local_JSON = OpenAPI_ecgi_convertToJSON(location_data_ext->ecgi);
    if (ecgi_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [ecgi]");
        goto end;
    }
    ...
    cJSON *ncgi_local_JSON = OpenAPI_ncgi_convertToJSON(location_data_ext->ncgi);
    if (ncgi_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [ncgi]");
        goto end;
    }
    ...
    if (location_data_ext->is_remote_ue_ind) {
    if (cJSON_AddBoolToObject(item, "remoteUeInd", location_data_ext->remote_ue_ind) == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [remote_ue_ind]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "altitude", location_data_ext->altitude) == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [altitude]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "barometricPressure", location_data_ext->barometric_pressure) == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [barometric_pressure]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "servingLMFIdentification", location_data_ext->serving_lmf_identification) == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [serving_lmf_identification]");
        goto end;
    }
    }

    if (location_data_ext->ue_positioning_cap) {
    if (cJSON_AddNumberToObject(item, "uePositioningCap", location_data_ext->ue_positioning_cap) == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [ue_positioning_cap]");
        goto end;
    }
    }

    if (location_data_ext->ue_area_ind) {
    cJSON *ue_area_ind_local_JSON = OpenAPI_ue_area_indication_convertToJSON(location_data_ext->ue_area_ind);
    if (ue_area_ind_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [ue_area_ind]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", location_data_ext->supported_features) == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [supported_features]");
        goto end;
    }
    }

    if (location_data_ext->achieved_qos) {
    cJSON *achieved_qos_local_JSON = OpenAPI_minor_location_qo_s_convertToJSON(location_data_ext->achieved_qos);
    if (achieved_qos_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [achieved_qos]");
        goto end;
    }
    ...
    if (cJSON_AddBoolToObject(item, "directReportInd", location_data_ext->direct_report_ind) == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [direct_report_ind]");
        goto end;
    }
    }

    if (location_data_ext->indoor_outdoor_ind) {
    cJSON *indoor_outdoor_ind_local_JSON = OpenAPI_indoor_outdoor_ind_convertToJSON(location_data_ext->indoor_outdoor_ind);
    if (indoor_outdoor_ind_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [indoor_outdoor_ind]");
        goto end;
    }
    ...
    cJSON *accepted_periodic_event_info_local_JSON = OpenAPI_periodic_event_info_convertToJSON(location_data_ext->accepted_periodic_event_info);
    if (accepted_periodic_event_info_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [accepted_periodic_event_info]");
        goto end;
    }
    ...
    cJSON *ha_gnss_metrics_local_JSON = OpenAPI_high_accuracy_gnss_metrics_convertToJSON(location_data_ext->ha_gnss_metrics);
    if (ha_gnss_metrics_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [ha_gnss_metrics]");
        goto end;
    }
    ...
    cJSON *los_nlos_measure_ind_local_JSON = OpenAPI_los_nlos_measure_ind_convertToJSON(location_data_ext->los_nlos_measure_ind);
    if (los_nlos_measure_ind_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [los_nlos_measure_ind]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "relatedApplicationlayerId", location_data_ext->related_applicationlayer_id) == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [related_applicationlayer_id]");
        goto end;
    }
    }

    if (location_data_ext->distance_direction) {
    cJSON *distance_direction_local_JSON = OpenAPI_range_direction_convertToJSON(location_data_ext->distance_direction);
    if (distance_direction_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [distance_direction]");
        goto end;
    }
    ...
    cJSON *_2d_relative_location_local_JSON = OpenAPI_model_2_d_relative_location_convertToJSON(location_data_ext->_2d_relative_location);
    if (_2d_relative_location_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [_2d_relative_location]");
        goto end;
    }
    ...
    cJSON *_3d_relative_location_local_JSON = OpenAPI_model_3_d_relative_location_convertToJSON(location_data_ext->_3d_relative_location);
    if (_3d_relative_location_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [_3d_relative_location]");
        goto end;
    }
    ...
    cJSON *relative_velocity_local_JSON = OpenAPI_velocity_estimate_convertToJSON(location_data_ext->relative_velocity);
    if (relative_velocity_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [relative_velocity]");
        goto end;
    }
    ...
    if (cJSON_AddNumberToObject(item, "rangingSlCapability", location_data_ext->ranging_sl_capability) == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [ranging_sl_capability]");
        goto end;
    }
    }

    if (location_data_ext->integrity_result) {
    cJSON *integrity_result_local_JSON = OpenAPI_integrity_result_convertToJSON(location_data_ext->integrity_result);
    if (integrity_result_local_JSON == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [integrity_result]");
        goto end;
    }
    ...
    if (location_data_ext->is_nrppa_periodic_ind) {
    if (cJSON_AddBoolToObject(item, "nrppaPeriodicInd", location_data_ext->nrppa_periodic_ind) == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [nrppa_periodic_ind]");
        goto end;
    }
    }
    ...
    if (add_location_datasList == NULL) {
        ogs_error("OpenAPI_location_data_ext_convertToJSON() failed [add_location_datas]");
        goto end;
    }

    OpenAPI_lnode_t *add_location_datas_node;
    if (location_data_ext->add_location_datas) {
        OpenAPI_list_for_each(location_data_ext->add_location_datas, add_location_datas_node) {
...
OpenAPI_location_data_ext_t *OpenAPI_location_data_ext_parseFromJSON(cJSON *location_data_extJSON)
{
    ...
    if (!location_estimate) {
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [location_estimate]");
        goto end;
    }
    ...
    if (!cJSON_IsNumber(age_of_location_estimate)) {
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [age_of_location_estimate]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(timestamp_of_location_estimate)) {
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [timestamp_of_location_estimate]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsArray(positioning_data_list)){
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [positioning_data_list]");
        goto end;
    }
    ...
    if (!cJSON_IsArray(gnss_positioning_data_list)){
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [gnss_positioning_data_list]");
        goto end;
    }
    ...
    if (!cJSON_IsBool(remote_ue_ind)) {
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [remote_ue_ind]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(altitude)) {
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [altitude]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(barometric_pressure)) {
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [barometric_pressure]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(serving_lmf_identification)) {
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [serving_lmf_identification]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(ue_positioning_cap)) {
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [ue_positioning_cap]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(supported_features)) {
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(direct_report_ind)) {
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [direct_report_ind]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(related_applicationlayer_id)) {
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [related_applicationlayer_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(ranging_sl_capability)) {
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [ranging_sl_capability]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(nrppa_periodic_ind)) {
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [nrppa_periodic_ind]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsArray(add_location_datas)){
        ogs_error("OpenAPI_location_data_ext_parseFromJSON() failed [add_location_datas]");
        goto end;
    }
