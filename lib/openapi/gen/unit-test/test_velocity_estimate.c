...
velocity_estimate_t* instantiate_velocity_estimate(int include_optional) {
  ...
  if (include_optional) {
    velocity_estimate = velocity_estimate_create(
      ...
      0,
      0,
      nlmf_dataexposure_velocity_estimate__UPWARD,
      0,
      0,
...
void test_velocity_estimate(int include_optional) {
    velocity_estimate_t* velocity_estimate_1 = instantiate_velocity_estimate(include_optional);

	cJSON* jsonvelocity_estimate_1 = velocity_estimate_convertToJSON(velocity_estimate_1);
	printf("velocity_estimate :\n%s\n", cJSON_Print(jsonvelocity_estimate_1));
	velocity_estimate_t* velocity_estimate_2 = velocity_estimate_parseFromJSON(jsonvelocity_estimate_1);
	cJSON* jsonvelocity_estimate_2 = velocity_estimate_convertToJSON(velocity_estimate_2);
