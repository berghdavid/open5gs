...
cJSON *patch_item_convertToJSON(patch_item_t *patch_item) {
    ...
    // patch_item->value
    if(patch_item->value) {
    cJSON *value_local_JSON = OpenAPI_any_type_convertToJSON(patch_item->value);
...
patch_item_t *patch_item_parseFromJSON(cJSON *patch_itemJSON){
    ...
    any_type_t *value_local_nonprim = NULL;
    ...
    if (value) { 
        value_local_nonprim = OpenAPI_any_type_parseFromJSON(value); //custom
    }
    ...
    if (value_local_nonprim) {
        OpenAPI_any_type_free(value_local_nonprim);
