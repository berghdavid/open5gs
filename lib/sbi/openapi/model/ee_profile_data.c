...
)
{
    ...
    log_assert(ee_profile_data_local_var);
...
cJSON *OpenAPI_ee_profile_data_convertToJSON(OpenAPI_ee_profile_data_t *ee_profile_data)
{
    ...
    if (ee_profile_data == NULL) {
        log_error("OpenAPI_ee_profile_data_convertToJSON() failed [EeProfileData]");
        return NULL;
    }
    ...
    if (restricted_event_typesList == NULL) {
        log_error("OpenAPI_ee_profile_data_convertToJSON() failed [restricted_event_types]");
        goto end;
    }
    OpenAPI_list_for_each(ee_profile_data->restricted_event_types, node) {
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", ee_profile_data->supported_features) == NULL) {
        log_error("OpenAPI_ee_profile_data_convertToJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (allowed_mtc_provider == NULL) {
        log_error("OpenAPI_ee_profile_data_convertToJSON() failed [allowed_mtc_provider]");
        goto end;
    }
    ...
    if (ee_profile_data->allowed_mtc_provider) {
        OpenAPI_list_for_each(ee_profile_data->allowed_mtc_provider, node) {
            ...
            cJSON *itemLocal = localKeyValue->value ?
                ...
                cJSON_CreateNull();
            if (itemLocal == NULL) {
                log_error("OpenAPI_ee_profile_data_convertToJSON() failed [inner]");
                goto end;
            }
    ...
    if (cJSON_AddBoolToObject(item, "iwkEpcRestricted", ee_profile_data->iwk_epc_restricted) == NULL) {
        log_error("OpenAPI_ee_profile_data_convertToJSON() failed [iwk_epc_restricted]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "imsi", ee_profile_data->imsi) == NULL) {
        log_error("OpenAPI_ee_profile_data_convertToJSON() failed [imsi]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "hssGroupId", ee_profile_data->hss_group_id) == NULL) {
        log_error("OpenAPI_ee_profile_data_convertToJSON() failed [hss_group_id]");
        goto end;
    }
    }
...
OpenAPI_ee_profile_data_t *OpenAPI_ee_profile_data_parseFromJSON(cJSON *ee_profile_dataJSON)
{
    ...
    if (restricted_event_types) {
        ...
        if (!cJSON_IsArray(restricted_event_types)) {
            log_error("OpenAPI_ee_profile_data_parseFromJSON() failed [restricted_event_types]");
            goto end;
        }
    ...
    if (!cJSON_IsString(supported_features) && !cJSON_IsNull(supported_features)) {
        log_error("OpenAPI_ee_profile_data_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (allowed_mtc_provider) {
        ...
        if (!cJSON_IsObject(allowed_mtc_provider) && !cJSON_IsNull(allowed_mtc_provider)) {
            log_error("OpenAPI_ee_profile_data_parseFromJSON() failed [allowed_mtc_provider]");
            goto end;
        }
        if (cJSON_IsObject(allowed_mtc_provider)) {
            ...
            cJSON_ArrayForEach(allowed_mtc_provider_local_map, allowed_mtc_provider) {
                ...
                if (cJSON_IsObject(localMapObject)) {
                    ...
                    log_error("OpenAPI_ee_profile_data_parseFromJSON() failed [inner]");
                    goto end;
                }
    ...
    if (!cJSON_IsBool(iwk_epc_restricted)) {
        log_error("OpenAPI_ee_profile_data_parseFromJSON() failed [iwk_epc_restricted]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(imsi) && !cJSON_IsNull(imsi)) {
        log_error("OpenAPI_ee_profile_data_parseFromJSON() failed [imsi]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(hss_group_id) && !cJSON_IsNull(hss_group_id)) {
        log_error("OpenAPI_ee_profile_data_parseFromJSON() failed [hss_group_id]");
        goto end;
    }
    }
...
OpenAPI_ee_profile_data_t *OpenAPI_ee_profile_data_copy(OpenAPI_ee_profile_data_t *dst, OpenAPI_ee_profile_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_ee_profile_data_convertToJSON() failed");
        return NULL;
    }
