...
)
{
    ...
    log_assert(ssc_modes_local_var);
...
cJSON *OpenAPI_ssc_modes_convertToJSON(OpenAPI_ssc_modes_t *ssc_modes)
{
    ...
    if (ssc_modes == NULL) {
        log_error("OpenAPI_ssc_modes_convertToJSON() failed [SscModes]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (ssc_modes->default_ssc_mode == OpenAPI_ssc_mode_NULL) {
        log_error("OpenAPI_ssc_modes_convertToJSON() failed [default_ssc_mode]");
        return NULL;
    }
    ...
    if (allowed_ssc_modesList == NULL) {
        log_error("OpenAPI_ssc_modes_convertToJSON() failed [allowed_ssc_modes]");
        goto end;
    }
...
OpenAPI_ssc_modes_t *OpenAPI_ssc_modes_parseFromJSON(cJSON *ssc_modesJSON)
{
    ...
    if (!default_ssc_mode) {
        log_error("OpenAPI_ssc_modes_parseFromJSON() failed [default_ssc_mode]");
        goto end;
    }
    ...
    if (allowed_ssc_modes) {
        ...
        if (!cJSON_IsArray(allowed_ssc_modes)) {
            log_error("OpenAPI_ssc_modes_parseFromJSON() failed [allowed_ssc_modes]");
            goto end;
        }
        ...
        if (allowed_ssc_modesList->count == 0) {
            log_error("OpenAPI_ssc_modes_parseFromJSON() failed: Expected allowed_ssc_modesList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
...
OpenAPI_ssc_modes_t *OpenAPI_ssc_modes_copy(OpenAPI_ssc_modes_t *dst, OpenAPI_ssc_modes_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_ssc_modes_convertToJSON() failed");
        return NULL;
    }
