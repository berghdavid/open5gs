...
)
{
    ...
    log_assert(authorized_network_slice_info_local_var);
...
cJSON *OpenAPI_authorized_network_slice_info_convertToJSON(OpenAPI_authorized_network_slice_info_t *authorized_network_slice_info)
{
    ...
    if (authorized_network_slice_info == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [AuthorizedNetworkSliceInfo]");
        return NULL;
    }
    ...
    if (allowed_nssai_listList == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [allowed_nssai_list]");
        goto end;
    }
    OpenAPI_list_for_each(authorized_network_slice_info->allowed_nssai_list, node) {
    ...
    if (configured_nssaiList == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [configured_nssai]");
        goto end;
    }
    OpenAPI_list_for_each(authorized_network_slice_info->configured_nssai, node) {
    ...
    if (cJSON_AddStringToObject(item, "targetAmfSet", authorized_network_slice_info->target_amf_set) == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [target_amf_set]");
        goto end;
    }
    }
    ...
    if (candidate_amf_listList == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [candidate_amf_list]");
        goto end;
    }
    OpenAPI_list_for_each(authorized_network_slice_info->candidate_amf_list, node) {
    ...
    if (rejected_nssai_in_plmnList == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [rejected_nssai_in_plmn]");
        goto end;
    }
    OpenAPI_list_for_each(authorized_network_slice_info->rejected_nssai_in_plmn, node) {
    ...
    if (rejected_nssai_in_taList == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [rejected_nssai_in_ta]");
        goto end;
    }
    OpenAPI_list_for_each(authorized_network_slice_info->rejected_nssai_in_ta, node) {
        cJSON *itemLocal = OpenAPI_snssai_convertToJSON(node->data);
    ...
    if (nsi_information_local_JSON == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [nsi_information]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", authorized_network_slice_info->supported_features) == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "nrfAmfSet", authorized_network_slice_info->nrf_amf_set) == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [nrf_amf_set]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "nrfAmfSetNfMgtUri", authorized_network_slice_info->nrf_amf_set_nf_mgt_uri) == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [nrf_amf_set_nf_mgt_uri]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "nrfAmfSetAccessTokenUri", authorized_network_slice_info->nrf_amf_set_access_token_uri) == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [nrf_amf_set_access_token_uri]");
        goto end;
    }
    }
    ...
    if (nrf_oauth2_required == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [nrf_oauth2_required]");
        goto end;
    }
    ...
    if (authorized_network_slice_info->nrf_oauth2_required) {
        OpenAPI_list_for_each(authorized_network_slice_info->nrf_oauth2_required, node) {
            ...
            if (cJSON_AddBoolToObject(localMapObject, localKeyValue->key, (uintptr_t)localKeyValue->value) == NULL) {
                log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [inner]");
                goto end;
            }
        }
    }
    ...
    if (cJSON_AddStringToObject(item, "targetAmfServiceSet", authorized_network_slice_info->target_amf_service_set) == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [target_amf_service_set]");
        goto end;
    }
    }
    ...
    if (target_nssaiList == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [target_nssai]");
        goto end;
    }
    OpenAPI_list_for_each(authorized_network_slice_info->target_nssai, node) {
        cJSON *itemLocal = OpenAPI_snssai_convertToJSON(node->data);
    ...
    if (nsag_infosList == NULL) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed [nsag_infos]");
        goto end;
    }
    OpenAPI_list_for_each(authorized_network_slice_info->nsag_infos, node) {
...
OpenAPI_authorized_network_slice_info_t *OpenAPI_authorized_network_slice_info_parseFromJSON(cJSON *authorized_network_slice_infoJSON)
{
    ...
    if (allowed_nssai_list) {
        ...
        if (!cJSON_IsArray(allowed_nssai_list)) {
            log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [allowed_nssai_list]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(allowed_nssai_list_local, allowed_nssai_list) {
            ...
            if (!allowed_nssai_listItem) {
                log_error("No allowed_nssai_listItem");
                goto end;
            }
    ...
    if (configured_nssai) {
        ...
        if (!cJSON_IsArray(configured_nssai)) {
            log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [configured_nssai]");
            goto end;
        }
    ...
    if (!cJSON_IsString(target_amf_set) && !cJSON_IsNull(target_amf_set)) {
        log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [target_amf_set]");
        goto end;
    }
    }
    ...
    if (candidate_amf_list) {
        ...
        if (!cJSON_IsArray(candidate_amf_list)) {
            log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [candidate_amf_list]");
            goto end;
        }
    ...
    if (rejected_nssai_in_plmn) {
        ...
        if (!cJSON_IsArray(rejected_nssai_in_plmn)) {
            log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [rejected_nssai_in_plmn]");
            goto end;
        }
    ...
    if (rejected_nssai_in_ta) {
        ...
        if (!cJSON_IsArray(rejected_nssai_in_ta)) {
            log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [rejected_nssai_in_ta]");
            goto end;
        }
    ...
    if (!nsi_information_local_nonprim) {
        log_error("OpenAPI_nsi_information_parseFromJSON failed [nsi_information]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(supported_features) && !cJSON_IsNull(supported_features)) {
        log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(nrf_amf_set) && !cJSON_IsNull(nrf_amf_set)) {
        log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [nrf_amf_set]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(nrf_amf_set_nf_mgt_uri) && !cJSON_IsNull(nrf_amf_set_nf_mgt_uri)) {
        log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [nrf_amf_set_nf_mgt_uri]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(nrf_amf_set_access_token_uri) && !cJSON_IsNull(nrf_amf_set_access_token_uri)) {
        log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [nrf_amf_set_access_token_uri]");
        goto end;
    }
    }
    ...
    if (nrf_oauth2_required) {
        ...
        if (!cJSON_IsObject(nrf_oauth2_required) && !cJSON_IsNull(nrf_oauth2_required)) {
            log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [nrf_oauth2_required]");
            goto end;
        }
        if (cJSON_IsObject(nrf_oauth2_required)) {
            ...
            cJSON_ArrayForEach(nrf_oauth2_required_local_map, nrf_oauth2_required) {
                ...
                if (!cJSON_IsBool(localMapObject)) {
                    log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [inner]");
                    goto end;
                }
    ...
    if (!cJSON_IsString(target_amf_service_set) && !cJSON_IsNull(target_amf_service_set)) {
        log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [target_amf_service_set]");
        goto end;
    }
    }
    ...
    if (target_nssai) {
        ...
        if (!cJSON_IsArray(target_nssai)) {
            log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [target_nssai]");
            goto end;
        }
    ...
    if (nsag_infos) {
        ...
        if (!cJSON_IsArray(nsag_infos)) {
            log_error("OpenAPI_authorized_network_slice_info_parseFromJSON() failed [nsag_infos]");
            goto end;
        }
...
OpenAPI_authorized_network_slice_info_t *OpenAPI_authorized_network_slice_info_copy(OpenAPI_authorized_network_slice_info_t *dst, OpenAPI_authorized_network_slice_info_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_authorized_network_slice_info_convertToJSON() failed");
        return NULL;
    }
