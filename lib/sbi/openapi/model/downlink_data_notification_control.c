...
)
{
    ...
    log_assert(downlink_data_notification_control_local_var);
...
cJSON *OpenAPI_downlink_data_notification_control_convertToJSON(OpenAPI_downlink_data_notification_control_t *downlink_data_notification_control)
{
    ...
    if (downlink_data_notification_control == NULL) {
        log_error("OpenAPI_downlink_data_notification_control_convertToJSON() failed [DownlinkDataNotificationControl]");
        return NULL;
    }
    ...
    if (notif_ctrl_indsList == NULL) {
        log_error("OpenAPI_downlink_data_notification_control_convertToJSON() failed [notif_ctrl_inds]");
        goto end;
    }
    ...
    if (types_of_notifList == NULL) {
        log_error("OpenAPI_downlink_data_notification_control_convertToJSON() failed [types_of_notif]");
        goto end;
    }
...
OpenAPI_downlink_data_notification_control_t *OpenAPI_downlink_data_notification_control_parseFromJSON(cJSON *downlink_data_notification_controlJSON)
{
    ...
    if (notif_ctrl_inds) {
        ...
        if (!cJSON_IsArray(notif_ctrl_inds)) {
            log_error("OpenAPI_downlink_data_notification_control_parseFromJSON() failed [notif_ctrl_inds]");
            goto end;
        }
        ...
        if (notif_ctrl_indsList->count == 0) {
            log_error("OpenAPI_downlink_data_notification_control_parseFromJSON() failed: Expected notif_ctrl_indsList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
    ...
    if (types_of_notif) {
        ...
        if (!cJSON_IsArray(types_of_notif)) {
            log_error("OpenAPI_downlink_data_notification_control_parseFromJSON() failed [types_of_notif]");
            goto end;
        }
        ...
        if (types_of_notifList->count == 0) {
            log_error("OpenAPI_downlink_data_notification_control_parseFromJSON() failed: Expected types_of_notifList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
...
OpenAPI_downlink_data_notification_control_t *OpenAPI_downlink_data_notification_control_copy(OpenAPI_downlink_data_notification_control_t *dst, OpenAPI_downlink_data_notification_control_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_downlink_data_notification_control_convertToJSON() failed");
        return NULL;
    }
