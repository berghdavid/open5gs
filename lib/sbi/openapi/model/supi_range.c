...
)
{
    ...
    log_assert(supi_range_local_var);
...
cJSON *OpenAPI_supi_range_convertToJSON(OpenAPI_supi_range_t *supi_range)
{
    ...
    if (supi_range == NULL) {
        log_error("OpenAPI_supi_range_convertToJSON() failed [SupiRange]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "start", supi_range->start) == NULL) {
        log_error("OpenAPI_supi_range_convertToJSON() failed [start]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "end", supi_range->end) == NULL) {
        log_error("OpenAPI_supi_range_convertToJSON() failed [end]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "pattern", supi_range->pattern) == NULL) {
        log_error("OpenAPI_supi_range_convertToJSON() failed [pattern]");
        goto end;
    }
    }
...
OpenAPI_supi_range_t *OpenAPI_supi_range_parseFromJSON(cJSON *supi_rangeJSON)
{
    ...
    if (!cJSON_IsString(start) && !cJSON_IsNull(start)) {
        log_error("OpenAPI_supi_range_parseFromJSON() failed [start]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(end) && !cJSON_IsNull(end)) {
        log_error("OpenAPI_supi_range_parseFromJSON() failed [end]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(pattern) && !cJSON_IsNull(pattern)) {
        log_error("OpenAPI_supi_range_parseFromJSON() failed [pattern]");
        goto end;
    }
    }
...
OpenAPI_supi_range_t *OpenAPI_supi_range_copy(OpenAPI_supi_range_t *dst, OpenAPI_supi_range_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_supi_range_convertToJSON() failed");
        return NULL;
    }
