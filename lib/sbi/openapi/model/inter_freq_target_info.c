...
)
{
    ...
    log_assert(inter_freq_target_info_local_var);
...
cJSON *OpenAPI_inter_freq_target_info_convertToJSON(OpenAPI_inter_freq_target_info_t *inter_freq_target_info)
{
    ...
    if (inter_freq_target_info == NULL) {
        log_error("OpenAPI_inter_freq_target_info_convertToJSON() failed [InterFreqTargetInfo]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (cJSON_AddNumberToObject(item, "dlCarrierFreq", inter_freq_target_info->dl_carrier_freq) == NULL) {
        log_error("OpenAPI_inter_freq_target_info_convertToJSON() failed [dl_carrier_freq]");
        goto end;
    }
    ...
    if (cell_id_listList == NULL) {
        log_error("OpenAPI_inter_freq_target_info_convertToJSON() failed [cell_id_list]");
        goto end;
    }
    OpenAPI_list_for_each(inter_freq_target_info->cell_id_list, node) {
        if (node->data == NULL) {
...
OpenAPI_inter_freq_target_info_t *OpenAPI_inter_freq_target_info_parseFromJSON(cJSON *inter_freq_target_infoJSON)
{
    ...
    if (!dl_carrier_freq) {
        log_error("OpenAPI_inter_freq_target_info_parseFromJSON() failed [dl_carrier_freq]");
        goto end;
    }
    ...
    if (cell_id_list) {
        ...
        if (!cJSON_IsArray(cell_id_list)) {
            log_error("OpenAPI_inter_freq_target_info_parseFromJSON() failed [cell_id_list]");
            goto end;
        }
...
OpenAPI_inter_freq_target_info_t *OpenAPI_inter_freq_target_info_copy(OpenAPI_inter_freq_target_info_t *dst, OpenAPI_inter_freq_target_info_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_inter_freq_target_info_convertToJSON() failed");
        return NULL;
    }
