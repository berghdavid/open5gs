...
)
{
    ...
    log_assert(ue_communication_local_var);
...
cJSON *OpenAPI_ue_communication_convertToJSON(OpenAPI_ue_communication_t *ue_communication)
{
    ...
    if (ue_communication == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [UeCommunication]");
        return NULL;
    }
    ...
    if (cJSON_AddNumberToObject(item, "commDur", ue_communication->comm_dur) == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [comm_dur]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "commDurVariance", ue_communication->comm_dur_variance) == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [comm_dur_variance]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "perioTime", ue_communication->perio_time) == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [perio_time]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "perioTimeVariance", ue_communication->perio_time_variance) == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [perio_time_variance]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "ts", ue_communication->ts) == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [ts]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "tsVariance", ue_communication->ts_variance) == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [ts_variance]");
        goto end;
    }
    }
    ...
    if (recurring_time_local_JSON == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [recurring_time]");
        goto end;
    }
    ...
    if (traf_char_local_JSON == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [traf_char]");
        goto end;
    }
    ...
    if (cJSON_AddNumberToObject(item, "ratio", ue_communication->ratio) == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [ratio]");
        goto end;
    }
    }
    ...
    if (cJSON_AddBoolToObject(item, "perioCommInd", ue_communication->perio_comm_ind) == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [perio_comm_ind]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "confidence", ue_communication->confidence) == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [confidence]");
        goto end;
    }
    }
    ...
    if (ana_of_app_list_local_JSON == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [ana_of_app_list]");
        goto end;
    }
    ...
    if (sess_inact_timer_local_JSON == NULL) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed [sess_inact_timer]");
        goto end;
    }
...
OpenAPI_ue_communication_t *OpenAPI_ue_communication_parseFromJSON(cJSON *ue_communicationJSON)
{
    ...
    if (!cJSON_IsNumber(comm_dur)) {
        log_error("OpenAPI_ue_communication_parseFromJSON() failed [comm_dur]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(comm_dur_variance)) {
        log_error("OpenAPI_ue_communication_parseFromJSON() failed [comm_dur_variance]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(perio_time)) {
        log_error("OpenAPI_ue_communication_parseFromJSON() failed [perio_time]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(perio_time_variance)) {
        log_error("OpenAPI_ue_communication_parseFromJSON() failed [perio_time_variance]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(ts) && !cJSON_IsNull(ts)) {
        log_error("OpenAPI_ue_communication_parseFromJSON() failed [ts]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(ts_variance)) {
        log_error("OpenAPI_ue_communication_parseFromJSON() failed [ts_variance]");
        goto end;
    }
    }
    ...
    if (!traf_char_local_nonprim) {
        log_error("OpenAPI_traffic_characterization_parseFromJSON failed [traf_char]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(ratio)) {
        log_error("OpenAPI_ue_communication_parseFromJSON() failed [ratio]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(perio_comm_ind)) {
        log_error("OpenAPI_ue_communication_parseFromJSON() failed [perio_comm_ind]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(confidence)) {
        log_error("OpenAPI_ue_communication_parseFromJSON() failed [confidence]");
        goto end;
    }
    }
    ...
    if (!ana_of_app_list_local_nonprim) {
        log_error("OpenAPI_app_list_for_ue_comm_parseFromJSON failed [ana_of_app_list]");
        goto end;
    }
    }
    ...
    if (!sess_inact_timer_local_nonprim) {
        log_error("OpenAPI_sess_inact_timer_for_ue_comm_parseFromJSON failed [sess_inact_timer]");
        goto end;
    }
    }
...
OpenAPI_ue_communication_t *OpenAPI_ue_communication_copy(OpenAPI_ue_communication_t *dst, OpenAPI_ue_communication_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_ue_communication_convertToJSON() failed");
        return NULL;
    }
