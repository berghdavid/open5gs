...
)
{
    ...
    log_assert(mb_smf_info_local_var);
...
cJSON *OpenAPI_mb_smf_info_convertToJSON(OpenAPI_mb_smf_info_t *mb_smf_info)
{
    ...
    if (mb_smf_info == NULL) {
        log_error("OpenAPI_mb_smf_info_convertToJSON() failed [MbSmfInfo]");
        return NULL;
    }
    ...
    if (s_nssai_info_list == NULL) {
        log_error("OpenAPI_mb_smf_info_convertToJSON() failed [s_nssai_info_list]");
        goto end;
    }
    ...
    if (mb_smf_info->s_nssai_info_list) {
        OpenAPI_list_for_each(mb_smf_info->s_nssai_info_list, node) {
            ...
            cJSON *itemLocal = localKeyValue->value ?
                ...
                cJSON_CreateNull();
            if (itemLocal == NULL) {
                log_error("OpenAPI_mb_smf_info_convertToJSON() failed [inner]");
                goto end;
            }
    ...
    if (tmgi_range_list == NULL) {
        log_error("OpenAPI_mb_smf_info_convertToJSON() failed [tmgi_range_list]");
        goto end;
    }
    ...
    if (tai_listList == NULL) {
        log_error("OpenAPI_mb_smf_info_convertToJSON() failed [tai_list]");
        goto end;
    }
    OpenAPI_list_for_each(mb_smf_info->tai_list, node) {
    ...
    if (tai_range_listList == NULL) {
        log_error("OpenAPI_mb_smf_info_convertToJSON() failed [tai_range_list]");
        goto end;
    }
    OpenAPI_list_for_each(mb_smf_info->tai_range_list, node) {
    ...
    if (mbs_session_list == NULL) {
        log_error("OpenAPI_mb_smf_info_convertToJSON() failed [mbs_session_list]");
        goto end;
    }
...
OpenAPI_mb_smf_info_t *OpenAPI_mb_smf_info_parseFromJSON(cJSON *mb_smf_infoJSON)
{
    ...
    if (s_nssai_info_list) {
        ...
        if (!cJSON_IsObject(s_nssai_info_list) && !cJSON_IsNull(s_nssai_info_list)) {
            log_error("OpenAPI_mb_smf_info_parseFromJSON() failed [s_nssai_info_list]");
            goto end;
        }
        if (cJSON_IsObject(s_nssai_info_list)) {
            ...
            cJSON_ArrayForEach(s_nssai_info_list_local_map, s_nssai_info_list) {
                ...
                if (cJSON_IsObject(localMapObject)) {
                    ...
                    log_error("OpenAPI_mb_smf_info_parseFromJSON() failed [inner]");
                    goto end;
                }
    ...
    if (tmgi_range_list) {
        ...
        if (!cJSON_IsObject(tmgi_range_list) && !cJSON_IsNull(tmgi_range_list)) {
            log_error("OpenAPI_mb_smf_info_parseFromJSON() failed [tmgi_range_list]");
            goto end;
        }
    ...
    if (tai_list) {
        ...
        if (!cJSON_IsArray(tai_list)) {
            log_error("OpenAPI_mb_smf_info_parseFromJSON() failed [tai_list]");
            goto end;
        }
    ...
    if (tai_range_list) {
        ...
        if (!cJSON_IsArray(tai_range_list)) {
            log_error("OpenAPI_mb_smf_info_parseFromJSON() failed [tai_range_list]");
            goto end;
        }
    ...
    if (mbs_session_list) {
        ...
        if (!cJSON_IsObject(mbs_session_list) && !cJSON_IsNull(mbs_session_list)) {
            log_error("OpenAPI_mb_smf_info_parseFromJSON() failed [mbs_session_list]");
            goto end;
        }
...
OpenAPI_mb_smf_info_t *OpenAPI_mb_smf_info_copy(OpenAPI_mb_smf_info_t *dst, OpenAPI_mb_smf_info_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_mb_smf_info_convertToJSON() failed");
        return NULL;
    }
