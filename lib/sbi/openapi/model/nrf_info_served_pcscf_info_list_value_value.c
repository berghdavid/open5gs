...
)
{
    ...
    log_assert(nrf_info_served_pcscf_info_list_value_value_local_var);
...
cJSON *OpenAPI_nrf_info_served_pcscf_info_list_value_value_convertToJSON(OpenAPI_nrf_info_served_pcscf_info_list_value_value_t *nrf_info_served_pcscf_info_list_value_value)
{
    ...
    if (nrf_info_served_pcscf_info_list_value_value == NULL) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_convertToJSON() failed [NrfInfo_servedPcscfInfoList_value_value]");
        return NULL;
    }
    ...
    if (access_typeList == NULL) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_convertToJSON() failed [access_type]");
        goto end;
    }
    ...
    if (dnn_listList == NULL) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_convertToJSON() failed [dnn_list]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_pcscf_info_list_value_value->dnn_list, node) {
    ...
    if (cJSON_AddStringToObject(item, "gmFqdn", nrf_info_served_pcscf_info_list_value_value->gm_fqdn) == NULL) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_convertToJSON() failed [gm_fqdn]");
        goto end;
    }
    }
    ...
    if (gm_ipv4_addressesList == NULL) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_convertToJSON() failed [gm_ipv4_addresses]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_pcscf_info_list_value_value->gm_ipv4_addresses, node) {
    ...
    if (gm_ipv6_addressesList == NULL) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_convertToJSON() failed [gm_ipv6_addresses]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_pcscf_info_list_value_value->gm_ipv6_addresses, node) {
    ...
    if (cJSON_AddStringToObject(item, "mwFqdn", nrf_info_served_pcscf_info_list_value_value->mw_fqdn) == NULL) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_convertToJSON() failed [mw_fqdn]");
        goto end;
    }
    }
    ...
    if (mw_ipv4_addressesList == NULL) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_convertToJSON() failed [mw_ipv4_addresses]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_pcscf_info_list_value_value->mw_ipv4_addresses, node) {
    ...
    if (mw_ipv6_addressesList == NULL) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_convertToJSON() failed [mw_ipv6_addresses]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_pcscf_info_list_value_value->mw_ipv6_addresses, node) {
    ...
    if (served_ipv4_address_rangesList == NULL) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_convertToJSON() failed [served_ipv4_address_ranges]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_pcscf_info_list_value_value->served_ipv4_address_ranges, node) {
    ...
    if (served_ipv6_prefix_rangesList == NULL) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_convertToJSON() failed [served_ipv6_prefix_ranges]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_pcscf_info_list_value_value->served_ipv6_prefix_ranges, node) {
...
OpenAPI_nrf_info_served_pcscf_info_list_value_value_t *OpenAPI_nrf_info_served_pcscf_info_list_value_value_parseFromJSON(cJSON *nrf_info_served_pcscf_info_list_value_valueJSON)
{
    ...
    if (access_type) {
        ...
        if (!cJSON_IsArray(access_type)) {
            log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_parseFromJSON() failed [access_type]");
            goto end;
        }
        ...
        if (access_typeList->count == 0) {
            log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_parseFromJSON() failed: Expected access_typeList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
    ...
    if (dnn_list) {
        ...
        if (!cJSON_IsArray(dnn_list)) {
            log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_parseFromJSON() failed [dnn_list]");
            goto end;
        }
    ...
    if (!cJSON_IsString(gm_fqdn) && !cJSON_IsNull(gm_fqdn)) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_parseFromJSON() failed [gm_fqdn]");
        goto end;
    }
    }
    ...
    if (gm_ipv4_addresses) {
        ...
        if (!cJSON_IsArray(gm_ipv4_addresses)) {
            log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_parseFromJSON() failed [gm_ipv4_addresses]");
            goto end;
        }
    ...
    if (gm_ipv6_addresses) {
        ...
        if (!cJSON_IsArray(gm_ipv6_addresses)) {
            log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_parseFromJSON() failed [gm_ipv6_addresses]");
            goto end;
        }
    ...
    if (!cJSON_IsString(mw_fqdn) && !cJSON_IsNull(mw_fqdn)) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_parseFromJSON() failed [mw_fqdn]");
        goto end;
    }
    }
    ...
    if (mw_ipv4_addresses) {
        ...
        if (!cJSON_IsArray(mw_ipv4_addresses)) {
            log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_parseFromJSON() failed [mw_ipv4_addresses]");
            goto end;
        }
    ...
    if (mw_ipv6_addresses) {
        ...
        if (!cJSON_IsArray(mw_ipv6_addresses)) {
            log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_parseFromJSON() failed [mw_ipv6_addresses]");
            goto end;
        }
    ...
    if (served_ipv4_address_ranges) {
        ...
        if (!cJSON_IsArray(served_ipv4_address_ranges)) {
            log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_parseFromJSON() failed [served_ipv4_address_ranges]");
            goto end;
        }
    ...
    if (served_ipv6_prefix_ranges) {
        ...
        if (!cJSON_IsArray(served_ipv6_prefix_ranges)) {
            log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_parseFromJSON() failed [served_ipv6_prefix_ranges]");
            goto end;
        }
...
OpenAPI_nrf_info_served_pcscf_info_list_value_value_t *OpenAPI_nrf_info_served_pcscf_info_list_value_value_copy(OpenAPI_nrf_info_served_pcscf_info_list_value_value_t *dst, OpenAPI_nrf_info_served_pcscf_info_list_value_value_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_nrf_info_served_pcscf_info_list_value_value_convertToJSON() failed");
        return NULL;
    }
