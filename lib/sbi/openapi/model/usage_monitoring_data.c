...
)
{
    ...
    log_assert(usage_monitoring_data_local_var);
...
cJSON *OpenAPI_usage_monitoring_data_convertToJSON(OpenAPI_usage_monitoring_data_t *usage_monitoring_data)
{
    ...
    if (usage_monitoring_data == NULL) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed [UsageMonitoringData]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!usage_monitoring_data->um_id) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed [um_id]");
        return NULL;
    }
    ...
    if (cJSON_AddNumberToObject(item, "volumeThreshold", usage_monitoring_data->volume_threshold) == NULL) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed [volume_threshold]");
        goto end;
    }
    ...
    if (cJSON_AddNumberToObject(item, "volumeThresholdUplink", usage_monitoring_data->volume_threshold_uplink) == NULL) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed [volume_threshold_uplink]");
        goto end;
    }
    ...
    if (cJSON_AddNumberToObject(item, "volumeThresholdDownlink", usage_monitoring_data->volume_threshold_downlink) == NULL) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed [volume_threshold_downlink]");
        goto end;
    }
    ...
    if (cJSON_AddNumberToObject(item, "timeThreshold", usage_monitoring_data->time_threshold) == NULL) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed [time_threshold]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "monitoringTime", usage_monitoring_data->monitoring_time) == NULL) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed [monitoring_time]");
        goto end;
    }
    ...
    if (cJSON_AddNumberToObject(item, "nextVolThreshold", usage_monitoring_data->next_vol_threshold) == NULL) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed [next_vol_threshold]");
        goto end;
    }
    ...
    if (cJSON_AddNumberToObject(item, "nextVolThresholdUplink", usage_monitoring_data->next_vol_threshold_uplink) == NULL) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed [next_vol_threshold_uplink]");
        goto end;
    }
    ...
    if (cJSON_AddNumberToObject(item, "nextVolThresholdDownlink", usage_monitoring_data->next_vol_threshold_downlink) == NULL) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed [next_vol_threshold_downlink]");
        goto end;
    }
    ...
    if (cJSON_AddNumberToObject(item, "nextTimeThreshold", usage_monitoring_data->next_time_threshold) == NULL) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed [next_time_threshold]");
        goto end;
    }
    ...
    if (cJSON_AddNumberToObject(item, "inactivityTime", usage_monitoring_data->inactivity_time) == NULL) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed [inactivity_time]");
        goto end;
    }
    ...
    if (ex_usage_pcc_rule_idsList == NULL) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed [ex_usage_pcc_rule_ids]");
        goto end;
    }
    OpenAPI_list_for_each(usage_monitoring_data->ex_usage_pcc_rule_ids, node) {
...
OpenAPI_usage_monitoring_data_t *OpenAPI_usage_monitoring_data_parseFromJSON(cJSON *usage_monitoring_dataJSON)
{
    ...
    if (!um_id) {
        log_error("OpenAPI_usage_monitoring_data_parseFromJSON() failed [um_id]");
        goto end;
    }
    ...
    if (!cJSON_IsNumber(volume_threshold)) {
        log_error("OpenAPI_usage_monitoring_data_parseFromJSON() failed [volume_threshold]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsNumber(volume_threshold_uplink)) {
        log_error("OpenAPI_usage_monitoring_data_parseFromJSON() failed [volume_threshold_uplink]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsNumber(volume_threshold_downlink)) {
        log_error("OpenAPI_usage_monitoring_data_parseFromJSON() failed [volume_threshold_downlink]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsNumber(time_threshold)) {
        log_error("OpenAPI_usage_monitoring_data_parseFromJSON() failed [time_threshold]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsString(monitoring_time) && !cJSON_IsNull(monitoring_time)) {
        log_error("OpenAPI_usage_monitoring_data_parseFromJSON() failed [monitoring_time]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsNumber(next_vol_threshold)) {
        log_error("OpenAPI_usage_monitoring_data_parseFromJSON() failed [next_vol_threshold]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsNumber(next_vol_threshold_uplink)) {
        log_error("OpenAPI_usage_monitoring_data_parseFromJSON() failed [next_vol_threshold_uplink]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsNumber(next_vol_threshold_downlink)) {
        log_error("OpenAPI_usage_monitoring_data_parseFromJSON() failed [next_vol_threshold_downlink]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsNumber(next_time_threshold)) {
        log_error("OpenAPI_usage_monitoring_data_parseFromJSON() failed [next_time_threshold]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsNumber(inactivity_time)) {
        log_error("OpenAPI_usage_monitoring_data_parseFromJSON() failed [inactivity_time]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsNull(ex_usage_pcc_rule_ids)) {
        ...
        if (!cJSON_IsArray(ex_usage_pcc_rule_ids)) {
            log_error("OpenAPI_usage_monitoring_data_parseFromJSON() failed [ex_usage_pcc_rule_ids]");
            goto end;
        }
...
OpenAPI_usage_monitoring_data_t *OpenAPI_usage_monitoring_data_copy(OpenAPI_usage_monitoring_data_t *dst, OpenAPI_usage_monitoring_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_usage_monitoring_data_convertToJSON() failed");
        return NULL;
    }
