...
)
{
    ...
    log_assert(network_perf_requirement_local_var);
...
cJSON *OpenAPI_network_perf_requirement_convertToJSON(OpenAPI_network_perf_requirement_t *network_perf_requirement)
{
    ...
    if (network_perf_requirement == NULL) {
        log_error("OpenAPI_network_perf_requirement_convertToJSON() failed [NetworkPerfRequirement]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!network_perf_requirement->nw_perf_type) {
        log_error("OpenAPI_network_perf_requirement_convertToJSON() failed [nw_perf_type]");
        return NULL;
    }
    ...
    if (cJSON_AddNumberToObject(item, "relativeRatio", network_perf_requirement->relative_ratio) == NULL) {
        log_error("OpenAPI_network_perf_requirement_convertToJSON() failed [relative_ratio]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "absoluteNum", network_perf_requirement->absolute_num) == NULL) {
        log_error("OpenAPI_network_perf_requirement_convertToJSON() failed [absolute_num]");
        goto end;
    }
    }
...
OpenAPI_network_perf_requirement_t *OpenAPI_network_perf_requirement_parseFromJSON(cJSON *network_perf_requirementJSON)
{
    ...
    if (!nw_perf_type) {
        log_error("OpenAPI_network_perf_requirement_parseFromJSON() failed [nw_perf_type]");
        goto end;
    }
    ...
    if (!cJSON_IsNumber(relative_ratio)) {
        log_error("OpenAPI_network_perf_requirement_parseFromJSON() failed [relative_ratio]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(absolute_num)) {
        log_error("OpenAPI_network_perf_requirement_parseFromJSON() failed [absolute_num]");
        goto end;
    }
    }
...
OpenAPI_network_perf_requirement_t *OpenAPI_network_perf_requirement_copy(OpenAPI_network_perf_requirement_t *dst, OpenAPI_network_perf_requirement_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_network_perf_requirement_convertToJSON() failed");
        return NULL;
    }
