...
)
{
    ...
    log_assert(mbs_session_local_var);
...
cJSON *OpenAPI_mbs_session_convertToJSON(OpenAPI_mbs_session_t *mbs_session)
{
    ...
    if (mbs_session == NULL) {
        log_error("OpenAPI_mbs_session_convertToJSON() failed [MbsSession]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!mbs_session->mbs_session_id) {
        log_error("OpenAPI_mbs_session_convertToJSON() failed [mbs_session_id]");
        return NULL;
    }
    ...
    if (mbs_area_sessions == NULL) {
        log_error("OpenAPI_mbs_session_convertToJSON() failed [mbs_area_sessions]");
        goto end;
    }
    ...
    if (mbs_session->mbs_area_sessions) {
        OpenAPI_list_for_each(mbs_session->mbs_area_sessions, node) {
            ...
            cJSON *itemLocal = localKeyValue->value ?
                ...
                cJSON_CreateNull();
            if (itemLocal == NULL) {
                log_error("OpenAPI_mbs_session_convertToJSON() failed [inner]");
                goto end;
            }
...
OpenAPI_mbs_session_t *OpenAPI_mbs_session_parseFromJSON(cJSON *mbs_sessionJSON)
{
    ...
    if (!mbs_session_id) {
        log_error("OpenAPI_mbs_session_parseFromJSON() failed [mbs_session_id]");
        goto end;
    }
    ...
    if (mbs_area_sessions) {
        ...
        if (!cJSON_IsObject(mbs_area_sessions) && !cJSON_IsNull(mbs_area_sessions)) {
            log_error("OpenAPI_mbs_session_parseFromJSON() failed [mbs_area_sessions]");
            goto end;
        }
        if (cJSON_IsObject(mbs_area_sessions)) {
            ...
            cJSON_ArrayForEach(mbs_area_sessions_local_map, mbs_area_sessions) {
                ...
                if (cJSON_IsObject(localMapObject)) {
                    ...
                    log_error("OpenAPI_mbs_session_parseFromJSON() failed [inner]");
                    goto end;
                }
...
OpenAPI_mbs_session_t *OpenAPI_mbs_session_copy(OpenAPI_mbs_session_t *dst, OpenAPI_mbs_session_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_mbs_session_convertToJSON() failed");
        return NULL;
    }
