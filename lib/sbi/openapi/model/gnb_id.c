...
cJSON *OpenAPI_gnb_id_convertToJSON(OpenAPI_gnb_id_t *gnb_id)
{
    ...
    if (gnb_id == NULL) {
        log_error("OpenAPI_gnb_id_convertToJSON() failed [GNbId]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (cJSON_AddNumberToObject(item, "bitLength", gnb_id->bit_length) == NULL) {
        log_error("OpenAPI_gnb_id_convertToJSON() failed [bit_length]");
        goto end;
    }

    if (!gnb_id->g_nb_value) {
        log_error("OpenAPI_gnb_id_convertToJSON() failed [g_nb_value]");
        return NULL;
    }
...
OpenAPI_gnb_id_t *OpenAPI_gnb_id_parseFromJSON(cJSON *gnb_idJSON)
{
    ...
    if (!bit_length) {
        log_error("OpenAPI_gnb_id_parseFromJSON() failed [bit_length]");
        goto end;
    }
    ...
    if (!g_nb_value) {
        log_error("OpenAPI_gnb_id_parseFromJSON() failed [g_nb_value]");
        goto end;
    }
...
OpenAPI_gnb_id_t *OpenAPI_gnb_id_copy(OpenAPI_gnb_id_t *dst, OpenAPI_gnb_id_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_gnb_id_convertToJSON() failed");
        return NULL;
    }
