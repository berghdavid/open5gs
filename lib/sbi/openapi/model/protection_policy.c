...
)
{
    ...
    log_assert(protection_policy_local_var);
...
cJSON *OpenAPI_protection_policy_convertToJSON(OpenAPI_protection_policy_t *protection_policy)
{
    ...
    if (protection_policy == NULL) {
        log_error("OpenAPI_protection_policy_convertToJSON() failed [ProtectionPolicy]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!protection_policy->api_ie_mapping_list) {
        log_error("OpenAPI_protection_policy_convertToJSON() failed [api_ie_mapping_list]");
        return NULL;
    }
    ...
    if (data_type_enc_policyList == NULL) {
        log_error("OpenAPI_protection_policy_convertToJSON() failed [data_type_enc_policy]");
        goto end;
    }
...
OpenAPI_protection_policy_t *OpenAPI_protection_policy_parseFromJSON(cJSON *protection_policyJSON)
{
    ...
    if (!api_ie_mapping_list) {
        log_error("OpenAPI_protection_policy_parseFromJSON() failed [api_ie_mapping_list]");
        goto end;
    }
        ...
        cJSON_ArrayForEach(api_ie_mapping_list_local, api_ie_mapping_list) {
            ...
            if (!api_ie_mapping_listItem) {
                log_error("No api_ie_mapping_listItem");
                goto end;
            }
    ...
    if (data_type_enc_policy) {
        ...
        if (!cJSON_IsArray(data_type_enc_policy)) {
            log_error("OpenAPI_protection_policy_parseFromJSON() failed [data_type_enc_policy]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(data_type_enc_policy_local, data_type_enc_policy) {
            ...
            if (!localEnum) {
                log_info("Enum value \"%s\" for field \"data_type_enc_policy\" is not supported. Ignoring it ...",
        ...
        if (data_type_enc_policyList->count == 0) {
            log_error("OpenAPI_protection_policy_parseFromJSON() failed: Expected data_type_enc_policyList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
...
OpenAPI_protection_policy_t *OpenAPI_protection_policy_copy(OpenAPI_protection_policy_t *dst, OpenAPI_protection_policy_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_protection_policy_convertToJSON() failed");
        return NULL;
    }
