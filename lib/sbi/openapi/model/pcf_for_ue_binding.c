...
)
{
    ...
    log_assert(pcf_for_ue_binding_local_var);
...
cJSON *OpenAPI_pcf_for_ue_binding_convertToJSON(OpenAPI_pcf_for_ue_binding_t *pcf_for_ue_binding)
{
    ...
    if (pcf_for_ue_binding == NULL) {
        log_error("OpenAPI_pcf_for_ue_binding_convertToJSON() failed [PcfForUeBinding]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!pcf_for_ue_binding->supi) {
        log_error("OpenAPI_pcf_for_ue_binding_convertToJSON() failed [supi]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "gpsi", pcf_for_ue_binding->gpsi) == NULL) {
        log_error("OpenAPI_pcf_for_ue_binding_convertToJSON() failed [gpsi]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "pcfForUeFqdn", pcf_for_ue_binding->pcf_for_ue_fqdn) == NULL) {
        log_error("OpenAPI_pcf_for_ue_binding_convertToJSON() failed [pcf_for_ue_fqdn]");
        goto end;
    }
    }
    ...
    if (pcf_for_ue_ip_end_pointsList == NULL) {
        log_error("OpenAPI_pcf_for_ue_binding_convertToJSON() failed [pcf_for_ue_ip_end_points]");
        goto end;
    }
    OpenAPI_list_for_each(pcf_for_ue_binding->pcf_for_ue_ip_end_points, node) {
    ...
    if (cJSON_AddStringToObject(item, "pcfId", pcf_for_ue_binding->pcf_id) == NULL) {
        log_error("OpenAPI_pcf_for_ue_binding_convertToJSON() failed [pcf_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "pcfSetId", pcf_for_ue_binding->pcf_set_id) == NULL) {
        log_error("OpenAPI_pcf_for_ue_binding_convertToJSON() failed [pcf_set_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "bindLevel", OpenAPI_binding_level_ToString(pcf_for_ue_binding->bind_level)) == NULL) {
        log_error("OpenAPI_pcf_for_ue_binding_convertToJSON() failed [bind_level]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "suppFeat", pcf_for_ue_binding->supp_feat) == NULL) {
        log_error("OpenAPI_pcf_for_ue_binding_convertToJSON() failed [supp_feat]");
        goto end;
    }
    }
...
OpenAPI_pcf_for_ue_binding_t *OpenAPI_pcf_for_ue_binding_parseFromJSON(cJSON *pcf_for_ue_bindingJSON)
{
    ...
    if (!supi) {
        log_error("OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [supi]");
        goto end;
    }
    ...
    if (!cJSON_IsString(gpsi) && !cJSON_IsNull(gpsi)) {
        log_error("OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [gpsi]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(pcf_for_ue_fqdn) && !cJSON_IsNull(pcf_for_ue_fqdn)) {
        log_error("OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [pcf_for_ue_fqdn]");
        goto end;
    }
    }
    ...
    if (pcf_for_ue_ip_end_points) {
        ...
        if (!cJSON_IsArray(pcf_for_ue_ip_end_points)) {
            log_error("OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [pcf_for_ue_ip_end_points]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(pcf_for_ue_ip_end_points_local, pcf_for_ue_ip_end_points) {
            ...
            if (!pcf_for_ue_ip_end_pointsItem) {
                log_error("No pcf_for_ue_ip_end_pointsItem");
                goto end;
            }
    ...
    if (!cJSON_IsString(pcf_id) && !cJSON_IsNull(pcf_id)) {
        log_error("OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [pcf_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(pcf_set_id) && !cJSON_IsNull(pcf_set_id)) {
        log_error("OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [pcf_set_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(bind_level)) {
        log_error("OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [bind_level]");
        goto end;
    }
    ...
    if (!cJSON_IsString(supp_feat) && !cJSON_IsNull(supp_feat)) {
        log_error("OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [supp_feat]");
        goto end;
    }
    }
...
OpenAPI_pcf_for_ue_binding_t *OpenAPI_pcf_for_ue_binding_copy(OpenAPI_pcf_for_ue_binding_t *dst, OpenAPI_pcf_for_ue_binding_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_pcf_for_ue_binding_convertToJSON() failed");
        return NULL;
    }
