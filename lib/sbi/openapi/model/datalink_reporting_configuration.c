...
)
{
    ...
    log_assert(datalink_reporting_configuration_local_var);
...
cJSON *OpenAPI_datalink_reporting_configuration_convertToJSON(OpenAPI_datalink_reporting_configuration_t *datalink_reporting_configuration)
{
    ...
    if (datalink_reporting_configuration == NULL) {
        log_error("OpenAPI_datalink_reporting_configuration_convertToJSON() failed [DatalinkReportingConfiguration]");
        return NULL;
    }
    ...
    if (ddd_traffic_desList == NULL) {
        log_error("OpenAPI_datalink_reporting_configuration_convertToJSON() failed [ddd_traffic_des]");
        goto end;
    }
    OpenAPI_list_for_each(datalink_reporting_configuration->ddd_traffic_des, node) {
    ...
    if (cJSON_AddStringToObject(item, "dnn", datalink_reporting_configuration->dnn) == NULL) {
        log_error("OpenAPI_datalink_reporting_configuration_convertToJSON() failed [dnn]");
        goto end;
    }
    }
    ...
    if (slice_local_JSON == NULL) {
        log_error("OpenAPI_datalink_reporting_configuration_convertToJSON() failed [slice]");
        goto end;
    }
    ...
    if (ddd_status_listList == NULL) {
        log_error("OpenAPI_datalink_reporting_configuration_convertToJSON() failed [ddd_status_list]");
        goto end;
    }
...
OpenAPI_datalink_reporting_configuration_t *OpenAPI_datalink_reporting_configuration_parseFromJSON(cJSON *datalink_reporting_configurationJSON)
{
    ...
    if (ddd_traffic_des) {
        ...
        if (!cJSON_IsArray(ddd_traffic_des)) {
            log_error("OpenAPI_datalink_reporting_configuration_parseFromJSON() failed [ddd_traffic_des]");
            goto end;
        }
    ...
    if (!cJSON_IsString(dnn) && !cJSON_IsNull(dnn)) {
        log_error("OpenAPI_datalink_reporting_configuration_parseFromJSON() failed [dnn]");
        goto end;
    }
    }
    ...
    if (ddd_status_list) {
        ...
        if (!cJSON_IsArray(ddd_status_list)) {
            log_error("OpenAPI_datalink_reporting_configuration_parseFromJSON() failed [ddd_status_list]");
            goto end;
        }
        ...
        if (ddd_status_listList->count == 0) {
            log_error("OpenAPI_datalink_reporting_configuration_parseFromJSON() failed: Expected ddd_status_listList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
...
OpenAPI_datalink_reporting_configuration_t *OpenAPI_datalink_reporting_configuration_copy(OpenAPI_datalink_reporting_configuration_t *dst, OpenAPI_datalink_reporting_configuration_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_datalink_reporting_configuration_convertToJSON() failed");
        return NULL;
    }
