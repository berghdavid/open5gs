...
)
{
    ...
    log_assert(nwdaf_registration_modification_local_var);
...
cJSON *OpenAPI_nwdaf_registration_modification_convertToJSON(OpenAPI_nwdaf_registration_modification_t *nwdaf_registration_modification)
{
    ...
    if (nwdaf_registration_modification == NULL) {
        log_error("OpenAPI_nwdaf_registration_modification_convertToJSON() failed [NwdafRegistrationModification]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!nwdaf_registration_modification->nwdaf_instance_id) {
        log_error("OpenAPI_nwdaf_registration_modification_convertToJSON() failed [nwdaf_instance_id]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "nwdafSetId", nwdaf_registration_modification->nwdaf_set_id) == NULL) {
        log_error("OpenAPI_nwdaf_registration_modification_convertToJSON() failed [nwdaf_set_id]");
        goto end;
    }
    }
    ...
    if (analytics_idsList == NULL) {
        log_error("OpenAPI_nwdaf_registration_modification_convertToJSON() failed [analytics_ids]");
        goto end;
    }
    OpenAPI_list_for_each(nwdaf_registration_modification->analytics_ids, node) {
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", nwdaf_registration_modification->supported_features) == NULL) {
        log_error("OpenAPI_nwdaf_registration_modification_convertToJSON() failed [supported_features]");
        goto end;
    }
    }
...
OpenAPI_nwdaf_registration_modification_t *OpenAPI_nwdaf_registration_modification_parseFromJSON(cJSON *nwdaf_registration_modificationJSON)
{
    ...
    if (!nwdaf_instance_id) {
        log_error("OpenAPI_nwdaf_registration_modification_parseFromJSON() failed [nwdaf_instance_id]");
        goto end;
    }
    ...
    if (!cJSON_IsString(nwdaf_set_id) && !cJSON_IsNull(nwdaf_set_id)) {
        log_error("OpenAPI_nwdaf_registration_modification_parseFromJSON() failed [nwdaf_set_id]");
        goto end;
    }
    }
    ...
    if (analytics_ids) {
        ...
        if (!cJSON_IsArray(analytics_ids)) {
            log_error("OpenAPI_nwdaf_registration_modification_parseFromJSON() failed [analytics_ids]");
            goto end;
        }
    ...
    if (!cJSON_IsString(supported_features) && !cJSON_IsNull(supported_features)) {
        log_error("OpenAPI_nwdaf_registration_modification_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
...
OpenAPI_nwdaf_registration_modification_t *OpenAPI_nwdaf_registration_modification_copy(OpenAPI_nwdaf_registration_modification_t *dst, OpenAPI_nwdaf_registration_modification_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_nwdaf_registration_modification_convertToJSON() failed");
        return NULL;
    }
