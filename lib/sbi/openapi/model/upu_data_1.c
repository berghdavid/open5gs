...
)
{
    ...
    log_assert(upu_data_1_local_var);
...
cJSON *OpenAPI_upu_data_1_convertToJSON(OpenAPI_upu_data_1_t *upu_data_1)
{
    ...
    if (upu_data_1 == NULL) {
        log_error("OpenAPI_upu_data_1_convertToJSON() failed [UpuData_1]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "secPacket", upu_data_1->sec_packet) == NULL) {
        log_error("OpenAPI_upu_data_1_convertToJSON() failed [sec_packet]");
        goto end;
    }
    }
    ...
    if (default_conf_nssaiList == NULL) {
        log_error("OpenAPI_upu_data_1_convertToJSON() failed [default_conf_nssai]");
        goto end;
    }
    OpenAPI_list_for_each(upu_data_1->default_conf_nssai, node) {
    ...
    if (cJSON_AddStringToObject(item, "routingId", upu_data_1->routing_id) == NULL) {
        log_error("OpenAPI_upu_data_1_convertToJSON() failed [routing_id]");
        goto end;
    }
    }
...
OpenAPI_upu_data_1_t *OpenAPI_upu_data_1_parseFromJSON(cJSON *upu_data_1JSON)
{
    ...
    if (!cJSON_IsString(sec_packet) && !cJSON_IsNull(sec_packet)) {
        log_error("OpenAPI_upu_data_1_parseFromJSON() failed [sec_packet]");
        goto end;
    }
    }
    ...
    if (default_conf_nssai) {
        ...
        if (!cJSON_IsArray(default_conf_nssai)) {
            log_error("OpenAPI_upu_data_1_parseFromJSON() failed [default_conf_nssai]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(default_conf_nssai_local, default_conf_nssai) {
            ...
            if (!default_conf_nssaiItem) {
                log_error("No default_conf_nssaiItem");
                goto end;
            }
    ...
    if (!cJSON_IsString(routing_id) && !cJSON_IsNull(routing_id)) {
        log_error("OpenAPI_upu_data_1_parseFromJSON() failed [routing_id]");
        goto end;
    }
    }
...
OpenAPI_upu_data_1_t *OpenAPI_upu_data_1_copy(OpenAPI_upu_data_1_t *dst, OpenAPI_upu_data_1_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_upu_data_1_convertToJSON() failed");
        return NULL;
    }
