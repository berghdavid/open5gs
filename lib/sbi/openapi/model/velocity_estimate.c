...
)
{
    ...
    log_assert(velocity_estimate_local_var);
...
cJSON *OpenAPI_velocity_estimate_convertToJSON(OpenAPI_velocity_estimate_t *velocity_estimate)
{
    ...
    if (velocity_estimate == NULL) {
        log_error("OpenAPI_velocity_estimate_convertToJSON() failed [VelocityEstimate]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (cJSON_AddNumberToObject(item, "hSpeed", velocity_estimate->h_speed) == NULL) {
        log_error("OpenAPI_velocity_estimate_convertToJSON() failed [h_speed]");
        goto end;
    }

    if (cJSON_AddNumberToObject(item, "bearing", velocity_estimate->bearing) == NULL) {
        log_error("OpenAPI_velocity_estimate_convertToJSON() failed [bearing]");
        goto end;
    }

    if (cJSON_AddNumberToObject(item, "vSpeed", velocity_estimate->v_speed) == NULL) {
        log_error("OpenAPI_velocity_estimate_convertToJSON() failed [v_speed]");
        goto end;
    }

    if (velocity_estimate->v_direction == OpenAPI_vertical_direction_NULL) {
        log_error("OpenAPI_velocity_estimate_convertToJSON() failed [v_direction]");
        return NULL;
    }
    ...
    if (cJSON_AddNumberToObject(item, "hUncertainty", velocity_estimate->h_uncertainty) == NULL) {
        log_error("OpenAPI_velocity_estimate_convertToJSON() failed [h_uncertainty]");
        goto end;
    }

    if (cJSON_AddNumberToObject(item, "vUncertainty", velocity_estimate->v_uncertainty) == NULL) {
        log_error("OpenAPI_velocity_estimate_convertToJSON() failed [v_uncertainty]");
        goto end;
    }
...
OpenAPI_velocity_estimate_t *OpenAPI_velocity_estimate_parseFromJSON(cJSON *velocity_estimateJSON)
{
    ...
    if (!h_speed) {
        log_error("OpenAPI_velocity_estimate_parseFromJSON() failed [h_speed]");
        goto end;
    }
    ...
    if (!bearing) {
        log_error("OpenAPI_velocity_estimate_parseFromJSON() failed [bearing]");
        goto end;
    }
    ...
    if (!v_speed) {
        log_error("OpenAPI_velocity_estimate_parseFromJSON() failed [v_speed]");
        goto end;
    }
    ...
    if (!v_direction) {
        log_error("OpenAPI_velocity_estimate_parseFromJSON() failed [v_direction]");
        goto end;
    }
    ...
    if (!h_uncertainty) {
        log_error("OpenAPI_velocity_estimate_parseFromJSON() failed [h_uncertainty]");
        goto end;
    }
    ...
    if (!v_uncertainty) {
        log_error("OpenAPI_velocity_estimate_parseFromJSON() failed [v_uncertainty]");
        goto end;
    }
...
OpenAPI_velocity_estimate_t *OpenAPI_velocity_estimate_copy(OpenAPI_velocity_estimate_t *dst, OpenAPI_velocity_estimate_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_velocity_estimate_convertToJSON() failed");
        return NULL;
    }
