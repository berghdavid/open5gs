...
)
{
    ...
    log_assert(route_to_location_local_var);
...
cJSON *OpenAPI_route_to_location_convertToJSON(OpenAPI_route_to_location_t *route_to_location)
{
    ...
    if (route_to_location == NULL) {
        log_error("OpenAPI_route_to_location_convertToJSON() failed [RouteToLocation]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!route_to_location->dnai) {
        log_error("OpenAPI_route_to_location_convertToJSON() failed [dnai]");
        return NULL;
    }
    ...
    if (route_info_local_JSON == NULL) {
        log_error("OpenAPI_route_to_location_convertToJSON() failed [route_info]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "routeProfId", route_to_location->route_prof_id) == NULL) {
        log_error("OpenAPI_route_to_location_convertToJSON() failed [route_prof_id]");
        goto end;
    }
...
OpenAPI_route_to_location_t *OpenAPI_route_to_location_parseFromJSON(cJSON *route_to_locationJSON)
{
    ...
    if (!dnai) {
        log_error("OpenAPI_route_to_location_parseFromJSON() failed [dnai]");
        goto end;
    }
    ...
    if (!route_info_local_nonprim) {
        log_error("OpenAPI_route_information_parseFromJSON failed [route_info]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsString(route_prof_id) && !cJSON_IsNull(route_prof_id)) {
        log_error("OpenAPI_route_to_location_parseFromJSON() failed [route_prof_id]");
        goto end;
    }
    }
    }
...
OpenAPI_route_to_location_t *OpenAPI_route_to_location_copy(OpenAPI_route_to_location_t *dst, OpenAPI_route_to_location_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_route_to_location_convertToJSON() failed");
        return NULL;
    }
