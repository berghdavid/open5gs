...
)
{
    ...
    log_assert(subscr_cond_local_var);
...
cJSON *OpenAPI_subscr_cond_convertToJSON(OpenAPI_subscr_cond_t *subscr_cond)
{
    ...
    if (subscr_cond == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [SubscrCond]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "nfInstanceId", subscr_cond->nf_instance_id) == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [nf_instance_id]");
        goto end;
    }
    }
    ...
    if (nf_instance_listList == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [nf_instance_list]");
        goto end;
    }
    OpenAPI_list_for_each(subscr_cond->nf_instance_list, node) {
    ...
    if (cJSON_AddStringToObject(item, "nfType", OpenAPI_nf_type_ToString(subscr_cond->nf_type)) == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [nf_type]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "serviceName", subscr_cond->service_name) == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [service_name]");
        goto end;
    }
    }
    ...
    if (service_name_listList == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [service_name_list]");
        goto end;
    }
    OpenAPI_list_for_each(subscr_cond->service_name_list, node) {
    ...
    if (cJSON_AddStringToObject(item, "amfSetId", subscr_cond->amf_set_id) == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [amf_set_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "amfRegionId", subscr_cond->amf_region_id) == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [amf_region_id]");
        goto end;
    }
    }
    ...
    if (guami_listList == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [guami_list]");
        goto end;
    }
    OpenAPI_list_for_each(subscr_cond->guami_list, node) {
    ...
    if (snssai_listList == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [snssai_list]");
        goto end;
    }
    OpenAPI_list_for_each(subscr_cond->snssai_list, node) {
    ...
    if (nsi_listList == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [nsi_list]");
        goto end;
    }
    OpenAPI_list_for_each(subscr_cond->nsi_list, node) {
    ...
    if (cJSON_AddStringToObject(item, "nfGroupId", subscr_cond->nf_group_id) == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [nf_group_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "nfSetId", subscr_cond->nf_set_id) == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [nf_set_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "nfServiceSetId", subscr_cond->nf_service_set_id) == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [nf_service_set_id]");
        goto end;
    }
    }
    ...
    if (smf_serving_areaList == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [smf_serving_area]");
        goto end;
    }
    OpenAPI_list_for_each(subscr_cond->smf_serving_area, node) {
    ...
    if (tai_listList == NULL) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed [tai_list]");
        goto end;
    }
    OpenAPI_list_for_each(subscr_cond->tai_list, node) {
...
OpenAPI_subscr_cond_t *OpenAPI_subscr_cond_parseFromJSON(cJSON *subscr_condJSON)
{
    ...
    if (!cJSON_IsString(nf_instance_id) && !cJSON_IsNull(nf_instance_id)) {
        log_error("OpenAPI_subscr_cond_parseFromJSON() failed [nf_instance_id]");
        goto end;
    }
    }
    ...
    if (nf_instance_list) {
        ...
        if (!cJSON_IsArray(nf_instance_list)) {
            log_error("OpenAPI_subscr_cond_parseFromJSON() failed [nf_instance_list]");
            goto end;
        }
    ...
    if (!cJSON_IsString(nf_type)) {
        log_error("OpenAPI_subscr_cond_parseFromJSON() failed [nf_type]");
        goto end;
    }
    ...
    if (!cJSON_IsString(service_name) && !cJSON_IsNull(service_name)) {
        log_error("OpenAPI_subscr_cond_parseFromJSON() failed [service_name]");
        goto end;
    }
    }
    ...
    if (service_name_list) {
        ...
        if (!cJSON_IsArray(service_name_list)) {
            log_error("OpenAPI_subscr_cond_parseFromJSON() failed [service_name_list]");
            goto end;
        }
    ...
    if (!cJSON_IsString(amf_set_id) && !cJSON_IsNull(amf_set_id)) {
        log_error("OpenAPI_subscr_cond_parseFromJSON() failed [amf_set_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(amf_region_id) && !cJSON_IsNull(amf_region_id)) {
        log_error("OpenAPI_subscr_cond_parseFromJSON() failed [amf_region_id]");
        goto end;
    }
    }
    ...
    if (guami_list) {
        ...
        if (!cJSON_IsArray(guami_list)) {
            log_error("OpenAPI_subscr_cond_parseFromJSON() failed [guami_list]");
            goto end;
        }
    ...
    if (snssai_list) {
        ...
        if (!cJSON_IsArray(snssai_list)) {
            log_error("OpenAPI_subscr_cond_parseFromJSON() failed [snssai_list]");
            goto end;
        }
    ...
    if (nsi_list) {
        ...
        if (!cJSON_IsArray(nsi_list)) {
            log_error("OpenAPI_subscr_cond_parseFromJSON() failed [nsi_list]");
            goto end;
        }
    ...
    if (!cJSON_IsString(nf_group_id) && !cJSON_IsNull(nf_group_id)) {
        log_error("OpenAPI_subscr_cond_parseFromJSON() failed [nf_group_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(nf_set_id) && !cJSON_IsNull(nf_set_id)) {
        log_error("OpenAPI_subscr_cond_parseFromJSON() failed [nf_set_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(nf_service_set_id) && !cJSON_IsNull(nf_service_set_id)) {
        log_error("OpenAPI_subscr_cond_parseFromJSON() failed [nf_service_set_id]");
        goto end;
    }
    }
    ...
    if (smf_serving_area) {
        ...
        if (!cJSON_IsArray(smf_serving_area)) {
            log_error("OpenAPI_subscr_cond_parseFromJSON() failed [smf_serving_area]");
            goto end;
        }
    ...
    if (tai_list) {
        ...
        if (!cJSON_IsArray(tai_list)) {
            log_error("OpenAPI_subscr_cond_parseFromJSON() failed [tai_list]");
            goto end;
        }
...
OpenAPI_subscr_cond_t *OpenAPI_subscr_cond_copy(OpenAPI_subscr_cond_t *dst, OpenAPI_subscr_cond_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_subscr_cond_convertToJSON() failed");
        return NULL;
    }
