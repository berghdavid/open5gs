...
cJSON *OpenAPI_lmf_info_convertToJSON(OpenAPI_lmf_info_t *lmf_info)
{
    ...
    if (lmf_info == NULL) {
        log_error("OpenAPI_lmf_info_convertToJSON() failed [LmfInfo]");
        return NULL;
    }
    ...
    if (serving_client_typesList == NULL) {
        log_error("OpenAPI_lmf_info_convertToJSON() failed [serving_client_types]");
        goto end;
    }
    OpenAPI_list_for_each(lmf_info->serving_client_types, node) {
    ...
    if (cJSON_AddStringToObject(item, "lmfId", lmf_info->lmf_id) == NULL) {
        log_error("OpenAPI_lmf_info_convertToJSON() failed [lmf_id]");
        goto end;
    }
    }
    ...
    if (serving_access_typesList == NULL) {
        log_error("OpenAPI_lmf_info_convertToJSON() failed [serving_access_types]");
        goto end;
    }
    ...
    if (serving_an_node_typesList == NULL) {
        log_error("OpenAPI_lmf_info_convertToJSON() failed [serving_an_node_types]");
        goto end;
    }
    ...
    if (serving_rat_typesList == NULL) {
        log_error("OpenAPI_lmf_info_convertToJSON() failed [serving_rat_types]");
        goto end;
    }
    ...
    if (tai_listList == NULL) {
        log_error("OpenAPI_lmf_info_convertToJSON() failed [tai_list]");
        goto end;
    }
    OpenAPI_list_for_each(lmf_info->tai_list, node) {
    ...
    if (tai_range_listList == NULL) {
        log_error("OpenAPI_lmf_info_convertToJSON() failed [tai_range_list]");
        goto end;
    }
    OpenAPI_list_for_each(lmf_info->tai_range_list, node) {
    ...
    if (supported_gad_shapesList == NULL) {
        log_error("OpenAPI_lmf_info_convertToJSON() failed [supported_gad_shapes]");
        goto end;
    }
    OpenAPI_list_for_each(lmf_info->supported_gad_shapes, node) {
...
OpenAPI_lmf_info_t *OpenAPI_lmf_info_parseFromJSON(cJSON *lmf_infoJSON)
{
    ...
    if (serving_client_types) {
        ...
        if (!cJSON_IsArray(serving_client_types)) {
            log_error("OpenAPI_lmf_info_parseFromJSON() failed [serving_client_types]");
            goto end;
        }
    ...
    if (!cJSON_IsString(lmf_id) && !cJSON_IsNull(lmf_id)) {
        log_error("OpenAPI_lmf_info_parseFromJSON() failed [lmf_id]");
        goto end;
    }
    }
    ...
    if (serving_access_types) {
        ...
        if (!cJSON_IsArray(serving_access_types)) {
            log_error("OpenAPI_lmf_info_parseFromJSON() failed [serving_access_types]");
            goto end;
        }
        ...
        if (serving_access_typesList->count == 0) {
            log_error("OpenAPI_lmf_info_parseFromJSON() failed: Expected serving_access_typesList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
    ...
    if (serving_an_node_types) {
        ...
        if (!cJSON_IsArray(serving_an_node_types)) {
            log_error("OpenAPI_lmf_info_parseFromJSON() failed [serving_an_node_types]");
            goto end;
        }
        ...
        if (serving_an_node_typesList->count == 0) {
            log_error("OpenAPI_lmf_info_parseFromJSON() failed: Expected serving_an_node_typesList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
    ...
    if (serving_rat_types) {
        ...
        if (!cJSON_IsArray(serving_rat_types)) {
            log_error("OpenAPI_lmf_info_parseFromJSON() failed [serving_rat_types]");
            goto end;
        }
        ...
        if (serving_rat_typesList->count == 0) {
            log_error("OpenAPI_lmf_info_parseFromJSON() failed: Expected serving_rat_typesList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
    ...
    if (tai_list) {
        ...
        if (!cJSON_IsArray(tai_list)) {
            log_error("OpenAPI_lmf_info_parseFromJSON() failed [tai_list]");
            goto end;
        }
    ...
    if (tai_range_list) {
        ...
        if (!cJSON_IsArray(tai_range_list)) {
            log_error("OpenAPI_lmf_info_parseFromJSON() failed [tai_range_list]");
            goto end;
        }
    ...
    if (supported_gad_shapes) {
        ...
        if (!cJSON_IsArray(supported_gad_shapes)) {
            log_error("OpenAPI_lmf_info_parseFromJSON() failed [supported_gad_shapes]");
            goto end;
        }
...
OpenAPI_lmf_info_t *OpenAPI_lmf_info_copy(OpenAPI_lmf_info_t *dst, OpenAPI_lmf_info_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_lmf_info_convertToJSON() failed");
        return NULL;
    }
