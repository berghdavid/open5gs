...
cJSON *OpenAPI_ncgi_tai_convertToJSON(OpenAPI_ncgi_tai_t *ncgi_tai)
{
    ...
    if (ncgi_tai == NULL) {
        log_error("OpenAPI_ncgi_tai_convertToJSON() failed [NcgiTai]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!ncgi_tai->tai) {
        log_error("OpenAPI_ncgi_tai_convertToJSON() failed [tai]");
        return NULL;
    }
    ...
    if (!ncgi_tai->cell_list) {
        log_error("OpenAPI_ncgi_tai_convertToJSON() failed [cell_list]");
        return NULL;
    }
...
OpenAPI_ncgi_tai_t *OpenAPI_ncgi_tai_parseFromJSON(cJSON *ncgi_taiJSON)
{
    ...
    if (!tai) {
        log_error("OpenAPI_ncgi_tai_parseFromJSON() failed [tai]");
        goto end;
    }
    ...
    if (!cell_list) {
        log_error("OpenAPI_ncgi_tai_parseFromJSON() failed [cell_list]");
        goto end;
    }
...
OpenAPI_ncgi_tai_t *OpenAPI_ncgi_tai_copy(OpenAPI_ncgi_tai_t *dst, OpenAPI_ncgi_tai_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_ncgi_tai_convertToJSON() failed");
        return NULL;
    }
