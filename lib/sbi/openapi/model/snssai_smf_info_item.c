...
)
{
    ...
    log_assert(snssai_smf_info_item_local_var);
...
cJSON *OpenAPI_snssai_smf_info_item_convertToJSON(OpenAPI_snssai_smf_info_item_t *snssai_smf_info_item)
{
    ...
    if (snssai_smf_info_item == NULL) {
        log_error("OpenAPI_snssai_smf_info_item_convertToJSON() failed [SnssaiSmfInfoItem]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!snssai_smf_info_item->s_nssai) {
        log_error("OpenAPI_snssai_smf_info_item_convertToJSON() failed [s_nssai]");
        return NULL;
    }
    ...
    if (!snssai_smf_info_item->dnn_smf_info_list) {
        log_error("OpenAPI_snssai_smf_info_item_convertToJSON() failed [dnn_smf_info_list]");
        return NULL;
    }
...
OpenAPI_snssai_smf_info_item_t *OpenAPI_snssai_smf_info_item_parseFromJSON(cJSON *snssai_smf_info_itemJSON)
{
    ...
    if (!s_nssai) {
        log_error("OpenAPI_snssai_smf_info_item_parseFromJSON() failed [s_nssai]");
        goto end;
    }
    ...
    if (!dnn_smf_info_list) {
        log_error("OpenAPI_snssai_smf_info_item_parseFromJSON() failed [dnn_smf_info_list]");
        goto end;
    }
        ...
        cJSON_ArrayForEach(dnn_smf_info_list_local, dnn_smf_info_list) {
            ...
            if (!dnn_smf_info_listItem) {
                log_error("No dnn_smf_info_listItem");
                goto end;
            }
...
OpenAPI_snssai_smf_info_item_t *OpenAPI_snssai_smf_info_item_copy(OpenAPI_snssai_smf_info_item_t *dst, OpenAPI_snssai_smf_info_item_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_snssai_smf_info_item_convertToJSON() failed");
        return NULL;
    }
