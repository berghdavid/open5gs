...
cJSON *OpenAPI_dnf_convertToJSON(OpenAPI_dnf_t *dnf)
{
    ...
    if (dnf == NULL) {
        log_error("OpenAPI_dnf_convertToJSON() failed [Dnf]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!dnf->dnf_units) {
        log_error("OpenAPI_dnf_convertToJSON() failed [dnf_units]");
        return NULL;
    }
...
OpenAPI_dnf_t *OpenAPI_dnf_parseFromJSON(cJSON *dnfJSON)
{
    ...
    if (!dnf_units) {
        log_error("OpenAPI_dnf_parseFromJSON() failed [dnf_units]");
        goto end;
    }
...
OpenAPI_dnf_t *OpenAPI_dnf_copy(OpenAPI_dnf_t *dst, OpenAPI_dnf_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_dnf_convertToJSON() failed");
        return NULL;
    }
