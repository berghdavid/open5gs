...
)
{
    ...
    log_assert(qos_flow_setup_item_local_var);
...
cJSON *OpenAPI_qos_flow_setup_item_convertToJSON(OpenAPI_qos_flow_setup_item_t *qos_flow_setup_item)
{
    ...
    if (qos_flow_setup_item == NULL) {
        log_error("OpenAPI_qos_flow_setup_item_convertToJSON() failed [QosFlowSetupItem]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (cJSON_AddNumberToObject(item, "qfi", qos_flow_setup_item->qfi) == NULL) {
        log_error("OpenAPI_qos_flow_setup_item_convertToJSON() failed [qfi]");
        goto end;
    }

    if (!qos_flow_setup_item->qos_rules) {
        log_error("OpenAPI_qos_flow_setup_item_convertToJSON() failed [qos_rules]");
        return NULL;
    }
    ...
    if (cJSON_AddNumberToObject(item, "ebi", qos_flow_setup_item->ebi) == NULL) {
        log_error("OpenAPI_qos_flow_setup_item_convertToJSON() failed [ebi]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "qosFlowDescription", qos_flow_setup_item->qos_flow_description) == NULL) {
        log_error("OpenAPI_qos_flow_setup_item_convertToJSON() failed [qos_flow_description]");
        goto end;
    }
    }
    ...
    if (qos_flow_profile_local_JSON == NULL) {
        log_error("OpenAPI_qos_flow_setup_item_convertToJSON() failed [qos_flow_profile]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "associatedAnType", OpenAPI_qos_flow_access_type_ToString(qos_flow_setup_item->associated_an_type)) == NULL) {
        log_error("OpenAPI_qos_flow_setup_item_convertToJSON() failed [associated_an_type]");
        goto end;
    }
    }
    ...
    if (cJSON_AddBoolToObject(item, "defaultQosRuleInd", qos_flow_setup_item->default_qos_rule_ind) == NULL) {
        log_error("OpenAPI_qos_flow_setup_item_convertToJSON() failed [default_qos_rule_ind]");
        goto end;
    }
    }
...
OpenAPI_qos_flow_setup_item_t *OpenAPI_qos_flow_setup_item_parseFromJSON(cJSON *qos_flow_setup_itemJSON)
{
    ...
    if (!qfi) {
        log_error("OpenAPI_qos_flow_setup_item_parseFromJSON() failed [qfi]");
        goto end;
    }
    ...
    if (!qos_rules) {
        log_error("OpenAPI_qos_flow_setup_item_parseFromJSON() failed [qos_rules]");
        goto end;
    }
    ...
    if (!cJSON_IsNumber(ebi)) {
        log_error("OpenAPI_qos_flow_setup_item_parseFromJSON() failed [ebi]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(qos_flow_description) && !cJSON_IsNull(qos_flow_description)) {
        log_error("OpenAPI_qos_flow_setup_item_parseFromJSON() failed [qos_flow_description]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(associated_an_type)) {
        log_error("OpenAPI_qos_flow_setup_item_parseFromJSON() failed [associated_an_type]");
        goto end;
    }
    ...
    if (!cJSON_IsBool(default_qos_rule_ind)) {
        log_error("OpenAPI_qos_flow_setup_item_parseFromJSON() failed [default_qos_rule_ind]");
        goto end;
    }
    }
...
OpenAPI_qos_flow_setup_item_t *OpenAPI_qos_flow_setup_item_copy(OpenAPI_qos_flow_setup_item_t *dst, OpenAPI_qos_flow_setup_item_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_qos_flow_setup_item_convertToJSON() failed");
        return NULL;
    }
