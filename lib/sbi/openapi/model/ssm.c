...
cJSON *OpenAPI_ssm_convertToJSON(OpenAPI_ssm_t *ssm)
{
    ...
    if (ssm == NULL) {
        log_error("OpenAPI_ssm_convertToJSON() failed [Ssm]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!ssm->source_ip_addr) {
        log_error("OpenAPI_ssm_convertToJSON() failed [source_ip_addr]");
        return NULL;
    }
    ...
    if (!ssm->dest_ip_addr) {
        log_error("OpenAPI_ssm_convertToJSON() failed [dest_ip_addr]");
        return NULL;
    }
...
OpenAPI_ssm_t *OpenAPI_ssm_copy(OpenAPI_ssm_t *dst, OpenAPI_ssm_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_ssm_convertToJSON() failed");
        return NULL;
    }
