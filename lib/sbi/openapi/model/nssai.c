...
cJSON *OpenAPI_nssai_convertToJSON(OpenAPI_nssai_t *nssai)
{
    ...
    if (nssai == NULL) {
        log_error("OpenAPI_nssai_convertToJSON() failed [Nssai]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", nssai->supported_features) == NULL) {
        log_error("OpenAPI_nssai_convertToJSON() failed [supported_features]");
        goto end;
    }
    }

    if (!nssai->default_single_nssais) {
        log_error("OpenAPI_nssai_convertToJSON() failed [default_single_nssais]");
        return NULL;
    }
    ...
    if (single_nssaisList == NULL) {
        log_error("OpenAPI_nssai_convertToJSON() failed [single_nssais]");
        goto end;
    }
    OpenAPI_list_for_each(nssai->single_nssais, node) {
        cJSON *itemLocal = OpenAPI_snssai_convertToJSON(node->data);
    ...
    if (cJSON_AddStringToObject(item, "provisioningTime", nssai->provisioning_time) == NULL) {
        log_error("OpenAPI_nssai_convertToJSON() failed [provisioning_time]");
        goto end;
    }
    }
    ...
    if (additional_snssai_data == NULL) {
        log_error("OpenAPI_nssai_convertToJSON() failed [additional_snssai_data]");
        goto end;
    }
    ...
    if (nssai->additional_snssai_data) {
        OpenAPI_list_for_each(nssai->additional_snssai_data, node) {
            ...
            cJSON *itemLocal = localKeyValue->value ?
                ...
                cJSON_CreateNull();
            if (itemLocal == NULL) {
                log_error("OpenAPI_nssai_convertToJSON() failed [inner]");
                goto end;
            }
    ...
    if (cJSON_AddBoolToObject(item, "suppressNssrgInd", nssai->suppress_nssrg_ind) == NULL) {
        log_error("OpenAPI_nssai_convertToJSON() failed [suppress_nssrg_ind]");
        goto end;
    }
    }
...
OpenAPI_nssai_t *OpenAPI_nssai_parseFromJSON(cJSON *nssaiJSON)
{
    ...
    if (!cJSON_IsString(supported_features) && !cJSON_IsNull(supported_features)) {
        log_error("OpenAPI_nssai_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (!default_single_nssais) {
        log_error("OpenAPI_nssai_parseFromJSON() failed [default_single_nssais]");
        goto end;
    }
    ...
    if (single_nssais) {
        ...
        if (!cJSON_IsArray(single_nssais)) {
            log_error("OpenAPI_nssai_parseFromJSON() failed [single_nssais]");
            goto end;
        }
    ...
    if (!cJSON_IsString(provisioning_time) && !cJSON_IsNull(provisioning_time)) {
        log_error("OpenAPI_nssai_parseFromJSON() failed [provisioning_time]");
        goto end;
    }
    }
    ...
    if (additional_snssai_data) {
        ...
        if (!cJSON_IsObject(additional_snssai_data) && !cJSON_IsNull(additional_snssai_data)) {
            log_error("OpenAPI_nssai_parseFromJSON() failed [additional_snssai_data]");
            goto end;
        }
        if (cJSON_IsObject(additional_snssai_data)) {
            ...
            cJSON_ArrayForEach(additional_snssai_data_local_map, additional_snssai_data) {
                ...
                if (cJSON_IsObject(localMapObject)) {
                    ...
                    log_error("OpenAPI_nssai_parseFromJSON() failed [inner]");
                    goto end;
                }
    ...
    if (!cJSON_IsBool(suppress_nssrg_ind)) {
        log_error("OpenAPI_nssai_parseFromJSON() failed [suppress_nssrg_ind]");
        goto end;
    }
    }
...
OpenAPI_nssai_t *OpenAPI_nssai_copy(OpenAPI_nssai_t *dst, OpenAPI_nssai_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_nssai_convertToJSON() failed");
        return NULL;
    }
