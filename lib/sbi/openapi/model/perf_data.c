...
)
{
    ...
    log_assert(perf_data_local_var);
...
cJSON *OpenAPI_perf_data_convertToJSON(OpenAPI_perf_data_t *perf_data)
{
    ...
    if (perf_data == NULL) {
        log_error("OpenAPI_perf_data_convertToJSON() failed [PerfData]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "avgTrafficRate", perf_data->avg_traffic_rate) == NULL) {
        log_error("OpenAPI_perf_data_convertToJSON() failed [avg_traffic_rate]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "maxTrafficRate", perf_data->max_traffic_rate) == NULL) {
        log_error("OpenAPI_perf_data_convertToJSON() failed [max_traffic_rate]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "avePacketDelay", perf_data->ave_packet_delay) == NULL) {
        log_error("OpenAPI_perf_data_convertToJSON() failed [ave_packet_delay]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "maxPacketDelay", perf_data->max_packet_delay) == NULL) {
        log_error("OpenAPI_perf_data_convertToJSON() failed [max_packet_delay]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "avgPacketLossRate", perf_data->avg_packet_loss_rate) == NULL) {
        log_error("OpenAPI_perf_data_convertToJSON() failed [avg_packet_loss_rate]");
        goto end;
    }
    }
...
OpenAPI_perf_data_t *OpenAPI_perf_data_parseFromJSON(cJSON *perf_dataJSON)
{
    ...
    if (!cJSON_IsString(avg_traffic_rate) && !cJSON_IsNull(avg_traffic_rate)) {
        log_error("OpenAPI_perf_data_parseFromJSON() failed [avg_traffic_rate]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(max_traffic_rate) && !cJSON_IsNull(max_traffic_rate)) {
        log_error("OpenAPI_perf_data_parseFromJSON() failed [max_traffic_rate]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(ave_packet_delay)) {
        log_error("OpenAPI_perf_data_parseFromJSON() failed [ave_packet_delay]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(max_packet_delay)) {
        log_error("OpenAPI_perf_data_parseFromJSON() failed [max_packet_delay]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(avg_packet_loss_rate)) {
        log_error("OpenAPI_perf_data_parseFromJSON() failed [avg_packet_loss_rate]");
        goto end;
    }
    }
...
OpenAPI_perf_data_t *OpenAPI_perf_data_copy(OpenAPI_perf_data_t *dst, OpenAPI_perf_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_perf_data_convertToJSON() failed");
        return NULL;
    }
