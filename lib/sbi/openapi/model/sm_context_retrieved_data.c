...
)
{
    ...
    log_assert(sm_context_retrieved_data_local_var);
...
cJSON *OpenAPI_sm_context_retrieved_data_convertToJSON(OpenAPI_sm_context_retrieved_data_t *sm_context_retrieved_data)
{
    ...
    if (sm_context_retrieved_data == NULL) {
        log_error("OpenAPI_sm_context_retrieved_data_convertToJSON() failed [SmContextRetrievedData]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!sm_context_retrieved_data->ue_eps_pdn_connection) {
        log_error("OpenAPI_sm_context_retrieved_data_convertToJSON() failed [ue_eps_pdn_connection]");
        return NULL;
    }
    ...
    if (sm_context_local_JSON == NULL) {
        log_error("OpenAPI_sm_context_retrieved_data_convertToJSON() failed [sm_context]");
        goto end;
    }
    ...
    if (small_data_rate_status_local_JSON == NULL) {
        log_error("OpenAPI_sm_context_retrieved_data_convertToJSON() failed [small_data_rate_status]");
        goto end;
    }
    ...
    if (apn_rate_status_local_JSON == NULL) {
        log_error("OpenAPI_sm_context_retrieved_data_convertToJSON() failed [apn_rate_status]");
        goto end;
    }
    ...
    if (cJSON_AddBoolToObject(item, "dlDataWaitingInd", sm_context_retrieved_data->dl_data_waiting_ind) == NULL) {
        log_error("OpenAPI_sm_context_retrieved_data_convertToJSON() failed [dl_data_waiting_ind]");
        goto end;
    }
    }
    ...
    if (af_coordination_info_local_JSON == NULL) {
        log_error("OpenAPI_sm_context_retrieved_data_convertToJSON() failed [af_coordination_info]");
        goto end;
    }
...
OpenAPI_sm_context_retrieved_data_t *OpenAPI_sm_context_retrieved_data_parseFromJSON(cJSON *sm_context_retrieved_dataJSON)
{
    ...
    if (!ue_eps_pdn_connection) {
        log_error("OpenAPI_sm_context_retrieved_data_parseFromJSON() failed [ue_eps_pdn_connection]");
        goto end;
    }
    ...
    if (!sm_context_local_nonprim) {
        log_error("OpenAPI_sm_context_parseFromJSON failed [sm_context]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(dl_data_waiting_ind)) {
        log_error("OpenAPI_sm_context_retrieved_data_parseFromJSON() failed [dl_data_waiting_ind]");
        goto end;
    }
    }
...
OpenAPI_sm_context_retrieved_data_t *OpenAPI_sm_context_retrieved_data_copy(OpenAPI_sm_context_retrieved_data_t *dst, OpenAPI_sm_context_retrieved_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_sm_context_retrieved_data_convertToJSON() failed");
        return NULL;
    }
