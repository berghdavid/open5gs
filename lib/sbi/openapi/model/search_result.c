...
)
{
    ...
    log_assert(search_result_local_var);
...
cJSON *OpenAPI_search_result_convertToJSON(OpenAPI_search_result_t *search_result)
{
    ...
    if (search_result == NULL) {
        log_error("OpenAPI_search_result_convertToJSON() failed [SearchResult]");
        return NULL;
    }
    ...
    if (cJSON_AddNumberToObject(item, "validityPeriod", search_result->validity_period) == NULL) {
        log_error("OpenAPI_search_result_convertToJSON() failed [validity_period]");
        goto end;
    }
    }

    if (!search_result->nf_instances) {
        log_error("OpenAPI_search_result_convertToJSON() failed [nf_instances]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "searchId", search_result->search_id) == NULL) {
        log_error("OpenAPI_search_result_convertToJSON() failed [search_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "numNfInstComplete", search_result->num_nf_inst_complete) == NULL) {
        log_error("OpenAPI_search_result_convertToJSON() failed [num_nf_inst_complete]");
        goto end;
    }
    }
    ...
    if (preferred_search_local_JSON == NULL) {
        log_error("OpenAPI_search_result_convertToJSON() failed [preferred_search]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "nrfSupportedFeatures", search_result->nrf_supported_features) == NULL) {
        log_error("OpenAPI_search_result_convertToJSON() failed [nrf_supported_features]");
        goto end;
    }
    }
    ...
    if (nf_instance_list == NULL) {
        log_error("OpenAPI_search_result_convertToJSON() failed [nf_instance_list]");
        goto end;
    }
    ...
    if (search_result->nf_instance_list) {
        OpenAPI_list_for_each(search_result->nf_instance_list, node) {
            ...
            cJSON *itemLocal = localKeyValue->value ?
                ...
                cJSON_CreateNull();
            if (itemLocal == NULL) {
                log_error("OpenAPI_search_result_convertToJSON() failed [inner]");
                goto end;
            }
    ...
    if (cJSON_AddBoolToObject(item, "alteredPriorityInd", search_result->altered_priority_ind) == NULL) {
        log_error("OpenAPI_search_result_convertToJSON() failed [altered_priority_ind]");
        goto end;
    }
    }
    ...
    if (no_profile_match_info_local_JSON == NULL) {
        log_error("OpenAPI_search_result_convertToJSON() failed [no_profile_match_info]");
        goto end;
    }
...
OpenAPI_search_result_t *OpenAPI_search_result_parseFromJSON(cJSON *search_resultJSON)
{
    ...
    if (!cJSON_IsNumber(validity_period)) {
        log_error("OpenAPI_search_result_parseFromJSON() failed [validity_period]");
        goto end;
    }
    }
    ...
    if (!nf_instances) {
        log_error("OpenAPI_search_result_parseFromJSON() failed [nf_instances]");
        goto end;
    }
    ...
    if (!cJSON_IsString(search_id) && !cJSON_IsNull(search_id)) {
        log_error("OpenAPI_search_result_parseFromJSON() failed [search_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(num_nf_inst_complete)) {
        log_error("OpenAPI_search_result_parseFromJSON() failed [num_nf_inst_complete]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(nrf_supported_features) && !cJSON_IsNull(nrf_supported_features)) {
        log_error("OpenAPI_search_result_parseFromJSON() failed [nrf_supported_features]");
        goto end;
    }
    }
    ...
    if (nf_instance_list) {
        ...
        if (!cJSON_IsObject(nf_instance_list) && !cJSON_IsNull(nf_instance_list)) {
            log_error("OpenAPI_search_result_parseFromJSON() failed [nf_instance_list]");
            goto end;
        }
        if (cJSON_IsObject(nf_instance_list)) {
            ...
            cJSON_ArrayForEach(nf_instance_list_local_map, nf_instance_list) {
                ...
                if (cJSON_IsObject(localMapObject)) {
                    ...
                    log_error("OpenAPI_search_result_parseFromJSON() failed [inner]");
                    goto end;
                }
    ...
    if (!cJSON_IsBool(altered_priority_ind)) {
        log_error("OpenAPI_search_result_parseFromJSON() failed [altered_priority_ind]");
        goto end;
    }
    }
    ...
    if (!no_profile_match_info_local_nonprim) {
        log_error("OpenAPI_no_profile_match_info_parseFromJSON failed [no_profile_match_info]");
        goto end;
    }
    }
...
OpenAPI_search_result_t *OpenAPI_search_result_copy(OpenAPI_search_result_t *dst, OpenAPI_search_result_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_search_result_convertToJSON() failed");
        return NULL;
    }
