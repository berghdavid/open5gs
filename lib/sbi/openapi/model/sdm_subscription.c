...
)
{
    ...
    log_assert(sdm_subscription_local_var);
...
cJSON *OpenAPI_sdm_subscription_convertToJSON(OpenAPI_sdm_subscription_t *sdm_subscription)
{
    ...
    if (sdm_subscription == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [SdmSubscription]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!sdm_subscription->nf_instance_id) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [nf_instance_id]");
        return NULL;
    }
    ...
    if (cJSON_AddBoolToObject(item, "implicitUnsubscribe", sdm_subscription->implicit_unsubscribe) == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [implicit_unsubscribe]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "expires", sdm_subscription->expires) == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [expires]");
        goto end;
    }
    }

    if (!sdm_subscription->callback_reference) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [callback_reference]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "amfServiceName", sdm_subscription->amf_service_name) == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [amf_service_name]");
        goto end;
    }
    }

    if (!sdm_subscription->monitored_resource_uris) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [monitored_resource_uris]");
        return NULL;
    }
    ...
    if (single_nssai_local_JSON == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [single_nssai]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "dnn", sdm_subscription->dnn) == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [dnn]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "subscriptionId", sdm_subscription->subscription_id) == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [subscription_id]");
        goto end;
    }
    }
    ...
    if (plmn_id_local_JSON == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [plmn_id]");
        goto end;
    }
    ...
    if (cJSON_AddBoolToObject(item, "immediateReport", sdm_subscription->immediate_report) == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [immediate_report]");
        goto end;
    }
    }
    ...
    if (report_local_JSON == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [report]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", sdm_subscription->supported_features) == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (context_info_local_JSON == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [context_info]");
        goto end;
    }
    ...
    if (cJSON_AddBoolToObject(item, "nfChangeFilter", sdm_subscription->nf_change_filter) == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [nf_change_filter]");
        goto end;
    }
    }
    ...
    if (cJSON_AddBoolToObject(item, "uniqueSubscription", sdm_subscription->unique_subscription) == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [unique_subscription]");
        goto end;
    }
    }
    ...
    if (reset_idsList == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [reset_ids]");
        goto end;
    }
    OpenAPI_list_for_each(sdm_subscription->reset_ids, node) {
    ...
    if (ue_con_smf_data_sub_filter_local_JSON == NULL) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed [ue_con_smf_data_sub_filter]");
        goto end;
    }
...
OpenAPI_sdm_subscription_t *OpenAPI_sdm_subscription_parseFromJSON(cJSON *sdm_subscriptionJSON)
{
    ...
    if (!nf_instance_id) {
        log_error("OpenAPI_sdm_subscription_parseFromJSON() failed [nf_instance_id]");
        goto end;
    }
    ...
    if (!cJSON_IsBool(implicit_unsubscribe)) {
        log_error("OpenAPI_sdm_subscription_parseFromJSON() failed [implicit_unsubscribe]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(expires) && !cJSON_IsNull(expires)) {
        log_error("OpenAPI_sdm_subscription_parseFromJSON() failed [expires]");
        goto end;
    }
    }
    ...
    if (!callback_reference) {
        log_error("OpenAPI_sdm_subscription_parseFromJSON() failed [callback_reference]");
        goto end;
    }
    ...
    if (!cJSON_IsString(amf_service_name) && !cJSON_IsNull(amf_service_name)) {
        log_error("OpenAPI_sdm_subscription_parseFromJSON() failed [amf_service_name]");
        goto end;
    }
    }
    ...
    if (!monitored_resource_uris) {
        log_error("OpenAPI_sdm_subscription_parseFromJSON() failed [monitored_resource_uris]");
        goto end;
    }
    ...
    if (!cJSON_IsString(dnn) && !cJSON_IsNull(dnn)) {
        log_error("OpenAPI_sdm_subscription_parseFromJSON() failed [dnn]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(subscription_id) && !cJSON_IsNull(subscription_id)) {
        log_error("OpenAPI_sdm_subscription_parseFromJSON() failed [subscription_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(immediate_report)) {
        log_error("OpenAPI_sdm_subscription_parseFromJSON() failed [immediate_report]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(supported_features) && !cJSON_IsNull(supported_features)) {
        log_error("OpenAPI_sdm_subscription_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(nf_change_filter)) {
        log_error("OpenAPI_sdm_subscription_parseFromJSON() failed [nf_change_filter]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(unique_subscription)) {
        log_error("OpenAPI_sdm_subscription_parseFromJSON() failed [unique_subscription]");
        goto end;
    }
    }
    ...
    if (reset_ids) {
        ...
        if (!cJSON_IsArray(reset_ids)) {
            log_error("OpenAPI_sdm_subscription_parseFromJSON() failed [reset_ids]");
            goto end;
        }
...
OpenAPI_sdm_subscription_t *OpenAPI_sdm_subscription_copy(OpenAPI_sdm_subscription_t *dst, OpenAPI_sdm_subscription_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_sdm_subscription_convertToJSON() failed");
        return NULL;
    }
