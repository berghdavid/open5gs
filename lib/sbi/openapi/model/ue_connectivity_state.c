...
)
{
    ...
    log_assert(ue_connectivity_state_local_var);
...
cJSON *OpenAPI_ue_connectivity_state_convertToJSON(OpenAPI_ue_connectivity_state_t *ue_connectivity_state)
{
    ...
    if (ue_connectivity_state == NULL) {
        log_error("OpenAPI_ue_connectivity_state_convertToJSON() failed [UeConnectivityState]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (ue_connectivity_state->access_type == OpenAPI_access_type_NULL) {
        log_error("OpenAPI_ue_connectivity_state_convertToJSON() failed [access_type]");
        return NULL;
    }
    ...
    if (connectivitystate_local_JSON == NULL) {
        log_error("OpenAPI_ue_connectivity_state_convertToJSON() failed [connectivitystate]");
        goto end;
    }
...
OpenAPI_ue_connectivity_state_t *OpenAPI_ue_connectivity_state_parseFromJSON(cJSON *ue_connectivity_stateJSON)
{
    ...
    if (!access_type) {
        log_error("OpenAPI_ue_connectivity_state_parseFromJSON() failed [access_type]");
        goto end;
    }
    ...
    if (!connectivitystate_local_nonprim) {
        log_error("OpenAPI_cm_state_parseFromJSON failed [connectivitystate]");
        goto end;
    }
    }
...
OpenAPI_ue_connectivity_state_t *OpenAPI_ue_connectivity_state_copy(OpenAPI_ue_connectivity_state_t *dst, OpenAPI_ue_connectivity_state_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_ue_connectivity_state_convertToJSON() failed");
        return NULL;
    }
