...
)
{
    ...
    log_assert(abnormal_behaviour_local_var);
...
cJSON *OpenAPI_abnormal_behaviour_convertToJSON(OpenAPI_abnormal_behaviour_t *abnormal_behaviour)
{
    ...
    if (abnormal_behaviour == NULL) {
        log_error("OpenAPI_abnormal_behaviour_convertToJSON() failed [AbnormalBehaviour]");
        return NULL;
    }
    ...
    if (supisList == NULL) {
        log_error("OpenAPI_abnormal_behaviour_convertToJSON() failed [supis]");
        goto end;
    }
    OpenAPI_list_for_each(abnormal_behaviour->supis, node) {
    ...
    if (!abnormal_behaviour->excep) {
        log_error("OpenAPI_abnormal_behaviour_convertToJSON() failed [excep]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "dnn", abnormal_behaviour->dnn) == NULL) {
        log_error("OpenAPI_abnormal_behaviour_convertToJSON() failed [dnn]");
        goto end;
    }
    }
    ...
    if (snssai_local_JSON == NULL) {
        log_error("OpenAPI_abnormal_behaviour_convertToJSON() failed [snssai]");
        goto end;
    }
    ...
    if (cJSON_AddNumberToObject(item, "ratio", abnormal_behaviour->ratio) == NULL) {
        log_error("OpenAPI_abnormal_behaviour_convertToJSON() failed [ratio]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "confidence", abnormal_behaviour->confidence) == NULL) {
        log_error("OpenAPI_abnormal_behaviour_convertToJSON() failed [confidence]");
        goto end;
    }
    }
    ...
    if (addt_meas_info_local_JSON == NULL) {
        log_error("OpenAPI_abnormal_behaviour_convertToJSON() failed [addt_meas_info]");
        goto end;
    }
...
OpenAPI_abnormal_behaviour_t *OpenAPI_abnormal_behaviour_parseFromJSON(cJSON *abnormal_behaviourJSON)
{
    ...
    if (supis) {
        ...
        if (!cJSON_IsArray(supis)) {
            log_error("OpenAPI_abnormal_behaviour_parseFromJSON() failed [supis]");
            goto end;
        }
    ...
    if (!excep) {
        log_error("OpenAPI_abnormal_behaviour_parseFromJSON() failed [excep]");
        goto end;
    }
    excep_local_nonprim = OpenAPI_exception_parseFromJSON(excep);
    if (!excep_local_nonprim) {
        log_error("OpenAPI_exception_parseFromJSON failed [excep]");
        goto end;
    }
    ...
    if (!cJSON_IsString(dnn) && !cJSON_IsNull(dnn)) {
        log_error("OpenAPI_abnormal_behaviour_parseFromJSON() failed [dnn]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(ratio)) {
        log_error("OpenAPI_abnormal_behaviour_parseFromJSON() failed [ratio]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(confidence)) {
        log_error("OpenAPI_abnormal_behaviour_parseFromJSON() failed [confidence]");
        goto end;
    }
    }
    ...
    if (!addt_meas_info_local_nonprim) {
        log_error("OpenAPI_additional_measurement_parseFromJSON failed [addt_meas_info]");
        goto end;
    }
    }
...
OpenAPI_abnormal_behaviour_t *OpenAPI_abnormal_behaviour_copy(OpenAPI_abnormal_behaviour_t *dst, OpenAPI_abnormal_behaviour_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_abnormal_behaviour_convertToJSON() failed");
        return NULL;
    }
