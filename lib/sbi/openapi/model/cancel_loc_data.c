...
)
{
    ...
    log_assert(cancel_loc_data_local_var);
...
cJSON *OpenAPI_cancel_loc_data_convertToJSON(OpenAPI_cancel_loc_data_t *cancel_loc_data)
{
    ...
    if (cancel_loc_data == NULL) {
        log_error("OpenAPI_cancel_loc_data_convertToJSON() failed [CancelLocData]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!cancel_loc_data->hgmlc_call_back_uri) {
        log_error("OpenAPI_cancel_loc_data_convertToJSON() failed [hgmlc_call_back_uri]");
        return NULL;
    }
    ...
    if (!cancel_loc_data->ldr_reference) {
        log_error("OpenAPI_cancel_loc_data_convertToJSON() failed [ldr_reference]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", cancel_loc_data->supported_features) == NULL) {
        log_error("OpenAPI_cancel_loc_data_convertToJSON() failed [supported_features]");
        goto end;
    }
    }
...
OpenAPI_cancel_loc_data_t *OpenAPI_cancel_loc_data_parseFromJSON(cJSON *cancel_loc_dataJSON)
{
    ...
    if (!hgmlc_call_back_uri) {
        log_error("OpenAPI_cancel_loc_data_parseFromJSON() failed [hgmlc_call_back_uri]");
        goto end;
    }
    ...
    if (!ldr_reference) {
        log_error("OpenAPI_cancel_loc_data_parseFromJSON() failed [ldr_reference]");
        goto end;
    }
    ...
    if (!cJSON_IsString(supported_features) && !cJSON_IsNull(supported_features)) {
        log_error("OpenAPI_cancel_loc_data_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
...
OpenAPI_cancel_loc_data_t *OpenAPI_cancel_loc_data_copy(OpenAPI_cancel_loc_data_t *dst, OpenAPI_cancel_loc_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_cancel_loc_data_convertToJSON() failed");
        return NULL;
    }
