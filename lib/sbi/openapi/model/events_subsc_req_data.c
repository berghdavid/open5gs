...
)
{
    ...
    log_assert(events_subsc_req_data_local_var);
...
cJSON *OpenAPI_events_subsc_req_data_convertToJSON(OpenAPI_events_subsc_req_data_t *events_subsc_req_data)
{
    ...
    if (events_subsc_req_data == NULL) {
        log_error("OpenAPI_events_subsc_req_data_convertToJSON() failed [EventsSubscReqData]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!events_subsc_req_data->events) {
        log_error("OpenAPI_events_subsc_req_data_convertToJSON() failed [events]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "notifUri", events_subsc_req_data->notif_uri) == NULL) {
        log_error("OpenAPI_events_subsc_req_data_convertToJSON() failed [notif_uri]");
        goto end;
    }
    }
    ...
    if (req_qos_mon_paramsList == NULL) {
        log_error("OpenAPI_events_subsc_req_data_convertToJSON() failed [req_qos_mon_params]");
        goto end;
    }
    ...
    if (qos_mon_local_JSON == NULL) {
        log_error("OpenAPI_events_subsc_req_data_convertToJSON() failed [qos_mon]");
        goto end;
    }
    ...
    if (req_anisList == NULL) {
        log_error("OpenAPI_events_subsc_req_data_convertToJSON() failed [req_anis]");
        goto end;
    }
    ...
    if (usg_thres_local_JSON == NULL) {
        log_error("OpenAPI_events_subsc_req_data_convertToJSON() failed [usg_thres]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "notifCorreId", events_subsc_req_data->notif_corre_id) == NULL) {
        log_error("OpenAPI_events_subsc_req_data_convertToJSON() failed [notif_corre_id]");
        goto end;
    }
    }
    ...
    if (af_app_idsList == NULL) {
        log_error("OpenAPI_events_subsc_req_data_convertToJSON() failed [af_app_ids]");
        goto end;
    }
    OpenAPI_list_for_each(events_subsc_req_data->af_app_ids, node) {
    ...
    if (cJSON_AddBoolToObject(item, "directNotifInd", events_subsc_req_data->direct_notif_ind) == NULL) {
        log_error("OpenAPI_events_subsc_req_data_convertToJSON() failed [direct_notif_ind]");
        goto end;
    }
    }
...
OpenAPI_events_subsc_req_data_t *OpenAPI_events_subsc_req_data_parseFromJSON(cJSON *events_subsc_req_dataJSON)
{
    ...
    if (!events) {
        log_error("OpenAPI_events_subsc_req_data_parseFromJSON() failed [events]");
        goto end;
    }
    ...
    if (!cJSON_IsString(notif_uri) && !cJSON_IsNull(notif_uri)) {
        log_error("OpenAPI_events_subsc_req_data_parseFromJSON() failed [notif_uri]");
        goto end;
    }
    }
    ...
    if (req_qos_mon_params) {
        ...
        if (!cJSON_IsArray(req_qos_mon_params)) {
            log_error("OpenAPI_events_subsc_req_data_parseFromJSON() failed [req_qos_mon_params]");
            goto end;
        }
        ...
        if (req_qos_mon_paramsList->count == 0) {
            log_error("OpenAPI_events_subsc_req_data_parseFromJSON() failed: Expected req_qos_mon_paramsList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
    ...
    if (req_anis) {
        ...
        if (!cJSON_IsArray(req_anis)) {
            log_error("OpenAPI_events_subsc_req_data_parseFromJSON() failed [req_anis]");
            goto end;
        }
        ...
        if (req_anisList->count == 0) {
            log_error("OpenAPI_events_subsc_req_data_parseFromJSON() failed: Expected req_anisList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
    ...
    if (!cJSON_IsString(notif_corre_id) && !cJSON_IsNull(notif_corre_id)) {
        log_error("OpenAPI_events_subsc_req_data_parseFromJSON() failed [notif_corre_id]");
        goto end;
    }
    }
    ...
    if (af_app_ids) {
        ...
        if (!cJSON_IsArray(af_app_ids)) {
            log_error("OpenAPI_events_subsc_req_data_parseFromJSON() failed [af_app_ids]");
            goto end;
        }
    ...
    if (!cJSON_IsBool(direct_notif_ind)) {
        log_error("OpenAPI_events_subsc_req_data_parseFromJSON() failed [direct_notif_ind]");
        goto end;
    }
    }
...
OpenAPI_events_subsc_req_data_t *OpenAPI_events_subsc_req_data_copy(OpenAPI_events_subsc_req_data_t *dst, OpenAPI_events_subsc_req_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_events_subsc_req_data_convertToJSON() failed");
        return NULL;
    }
