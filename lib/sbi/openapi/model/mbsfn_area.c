...
)
{
    ...
    log_assert(mbsfn_area_local_var);
...
cJSON *OpenAPI_mbsfn_area_convertToJSON(OpenAPI_mbsfn_area_t *mbsfn_area)
{
    ...
    if (mbsfn_area == NULL) {
        log_error("OpenAPI_mbsfn_area_convertToJSON() failed [MbsfnArea]");
        return NULL;
    }
    ...
    if (cJSON_AddNumberToObject(item, "mbsfnAreaId", mbsfn_area->mbsfn_area_id) == NULL) {
        log_error("OpenAPI_mbsfn_area_convertToJSON() failed [mbsfn_area_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "carrierFrequency", mbsfn_area->carrier_frequency) == NULL) {
        log_error("OpenAPI_mbsfn_area_convertToJSON() failed [carrier_frequency]");
        goto end;
    }
    }
...
OpenAPI_mbsfn_area_t *OpenAPI_mbsfn_area_parseFromJSON(cJSON *mbsfn_areaJSON)
{
    ...
    if (!cJSON_IsNumber(mbsfn_area_id)) {
        log_error("OpenAPI_mbsfn_area_parseFromJSON() failed [mbsfn_area_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(carrier_frequency)) {
        log_error("OpenAPI_mbsfn_area_parseFromJSON() failed [carrier_frequency]");
        goto end;
    }
    }
...
OpenAPI_mbsfn_area_t *OpenAPI_mbsfn_area_copy(OpenAPI_mbsfn_area_t *dst, OpenAPI_mbsfn_area_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_mbsfn_area_convertToJSON() failed");
        return NULL;
    }
