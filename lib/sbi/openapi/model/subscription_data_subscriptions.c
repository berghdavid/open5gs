...
)
{
    ...
    log_assert(subscription_data_subscriptions_local_var);
...
cJSON *OpenAPI_subscription_data_subscriptions_convertToJSON(OpenAPI_subscription_data_subscriptions_t *subscription_data_subscriptions)
{
    ...
    if (subscription_data_subscriptions == NULL) {
        log_error("OpenAPI_subscription_data_subscriptions_convertToJSON() failed [SubscriptionDataSubscriptions]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "ueId", subscription_data_subscriptions->ue_id) == NULL) {
        log_error("OpenAPI_subscription_data_subscriptions_convertToJSON() failed [ue_id]");
        goto end;
    }
    }

    if (!subscription_data_subscriptions->callback_reference) {
        log_error("OpenAPI_subscription_data_subscriptions_convertToJSON() failed [callback_reference]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "originalCallbackReference", subscription_data_subscriptions->original_callback_reference) == NULL) {
        log_error("OpenAPI_subscription_data_subscriptions_convertToJSON() failed [original_callback_reference]");
        goto end;
    }
    }

    if (!subscription_data_subscriptions->monitored_resource_uris) {
        log_error("OpenAPI_subscription_data_subscriptions_convertToJSON() failed [monitored_resource_uris]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "expiry", subscription_data_subscriptions->expiry) == NULL) {
        log_error("OpenAPI_subscription_data_subscriptions_convertToJSON() failed [expiry]");
        goto end;
    }
    }
    ...
    if (sdm_subscription_local_JSON == NULL) {
        log_error("OpenAPI_subscription_data_subscriptions_convertToJSON() failed [sdm_subscription]");
        goto end;
    }
    ...
    if (hss_subscription_info_local_JSON == NULL) {
        log_error("OpenAPI_subscription_data_subscriptions_convertToJSON() failed [hss_subscription_info]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "subscriptionId", subscription_data_subscriptions->subscription_id) == NULL) {
        log_error("OpenAPI_subscription_data_subscriptions_convertToJSON() failed [subscription_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddBoolToObject(item, "uniqueSubscription", subscription_data_subscriptions->unique_subscription) == NULL) {
        log_error("OpenAPI_subscription_data_subscriptions_convertToJSON() failed [unique_subscription]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", subscription_data_subscriptions->supported_features) == NULL) {
        log_error("OpenAPI_subscription_data_subscriptions_convertToJSON() failed [supported_features]");
        goto end;
    }
    }
...
OpenAPI_subscription_data_subscriptions_t *OpenAPI_subscription_data_subscriptions_parseFromJSON(cJSON *subscription_data_subscriptionsJSON)
{
    ...
    if (!cJSON_IsString(ue_id) && !cJSON_IsNull(ue_id)) {
        log_error("OpenAPI_subscription_data_subscriptions_parseFromJSON() failed [ue_id]");
        goto end;
    }
    }
    ...
    if (!callback_reference) {
        log_error("OpenAPI_subscription_data_subscriptions_parseFromJSON() failed [callback_reference]");
        goto end;
    }
    ...
    if (!cJSON_IsString(original_callback_reference) && !cJSON_IsNull(original_callback_reference)) {
        log_error("OpenAPI_subscription_data_subscriptions_parseFromJSON() failed [original_callback_reference]");
        goto end;
    }
    }
    ...
    if (!monitored_resource_uris) {
        log_error("OpenAPI_subscription_data_subscriptions_parseFromJSON() failed [monitored_resource_uris]");
        goto end;
    }
    ...
    if (!cJSON_IsString(expiry) && !cJSON_IsNull(expiry)) {
        log_error("OpenAPI_subscription_data_subscriptions_parseFromJSON() failed [expiry]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(subscription_id) && !cJSON_IsNull(subscription_id)) {
        log_error("OpenAPI_subscription_data_subscriptions_parseFromJSON() failed [subscription_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(unique_subscription)) {
        log_error("OpenAPI_subscription_data_subscriptions_parseFromJSON() failed [unique_subscription]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(supported_features) && !cJSON_IsNull(supported_features)) {
        log_error("OpenAPI_subscription_data_subscriptions_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
...
OpenAPI_subscription_data_subscriptions_t *OpenAPI_subscription_data_subscriptions_copy(OpenAPI_subscription_data_subscriptions_t *dst, OpenAPI_subscription_data_subscriptions_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_subscription_data_subscriptions_convertToJSON() failed");
        return NULL;
    }
