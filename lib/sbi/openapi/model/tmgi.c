...
cJSON *OpenAPI_tmgi_convertToJSON(OpenAPI_tmgi_t *tmgi)
{
    ...
    if (tmgi == NULL) {
        log_error("OpenAPI_tmgi_convertToJSON() failed [Tmgi]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!tmgi->mbs_service_id) {
        log_error("OpenAPI_tmgi_convertToJSON() failed [mbs_service_id]");
        return NULL;
    }
    ...
    if (!tmgi->plmn_id) {
        log_error("OpenAPI_tmgi_convertToJSON() failed [plmn_id]");
        return NULL;
    }
...
OpenAPI_tmgi_t *OpenAPI_tmgi_parseFromJSON(cJSON *tmgiJSON)
{
    ...
    if (!mbs_service_id) {
        log_error("OpenAPI_tmgi_parseFromJSON() failed [mbs_service_id]");
        goto end;
    }
    ...
    if (!plmn_id) {
        log_error("OpenAPI_tmgi_parseFromJSON() failed [plmn_id]");
        goto end;
    }
...
OpenAPI_tmgi_t *OpenAPI_tmgi_copy(OpenAPI_tmgi_t *dst, OpenAPI_tmgi_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_tmgi_convertToJSON() failed");
        return NULL;
    }
