...
)
{
    ...
    log_assert(spatial_validity_cond_local_var);
...
cJSON *OpenAPI_spatial_validity_cond_convertToJSON(OpenAPI_spatial_validity_cond_t *spatial_validity_cond)
{
    ...
    if (spatial_validity_cond == NULL) {
        log_error("OpenAPI_spatial_validity_cond_convertToJSON() failed [SpatialValidityCond]");
        return NULL;
    }
    ...
    if (tracking_area_listList == NULL) {
        log_error("OpenAPI_spatial_validity_cond_convertToJSON() failed [tracking_area_list]");
        goto end;
    }
    OpenAPI_list_for_each(spatial_validity_cond->tracking_area_list, node) {
    ...
    if (countriesList == NULL) {
        log_error("OpenAPI_spatial_validity_cond_convertToJSON() failed [countries]");
        goto end;
    }
    OpenAPI_list_for_each(spatial_validity_cond->countries, node) {
    ...
    if (geographical_service_area_local_JSON == NULL) {
        log_error("OpenAPI_spatial_validity_cond_convertToJSON() failed [geographical_service_area]");
        goto end;
    }
...
OpenAPI_spatial_validity_cond_t *OpenAPI_spatial_validity_cond_parseFromJSON(cJSON *spatial_validity_condJSON)
{
    ...
    if (tracking_area_list) {
        ...
        if (!cJSON_IsArray(tracking_area_list)) {
            log_error("OpenAPI_spatial_validity_cond_parseFromJSON() failed [tracking_area_list]");
            goto end;
        }
    ...
    if (countries) {
        ...
        if (!cJSON_IsArray(countries)) {
            log_error("OpenAPI_spatial_validity_cond_parseFromJSON() failed [countries]");
            goto end;
        }
    ...
    if (!geographical_service_area_local_nonprim) {
        log_error("OpenAPI_geo_service_area_parseFromJSON failed [geographical_service_area]");
        goto end;
    }
    }
...
OpenAPI_spatial_validity_cond_t *OpenAPI_spatial_validity_cond_copy(OpenAPI_spatial_validity_cond_t *dst, OpenAPI_spatial_validity_cond_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_spatial_validity_cond_convertToJSON() failed");
        return NULL;
    }
