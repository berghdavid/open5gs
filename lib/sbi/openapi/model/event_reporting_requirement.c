...
)
{
    ...
    log_assert(event_reporting_requirement_local_var);
...
cJSON *OpenAPI_event_reporting_requirement_convertToJSON(OpenAPI_event_reporting_requirement_t *event_reporting_requirement)
{
    ...
    if (event_reporting_requirement == NULL) {
        log_error("OpenAPI_event_reporting_requirement_convertToJSON() failed [EventReportingRequirement]");
        return NULL;
    }
    ...
    if (accuracy_local_JSON == NULL) {
        log_error("OpenAPI_event_reporting_requirement_convertToJSON() failed [accuracy]");
        goto end;
    }
    ...
    if (acc_per_subsetList == NULL) {
        log_error("OpenAPI_event_reporting_requirement_convertToJSON() failed [acc_per_subset]");
        goto end;
    }
    OpenAPI_list_for_each(event_reporting_requirement->acc_per_subset, node) {
    ...
    if (cJSON_AddStringToObject(item, "startTs", event_reporting_requirement->start_ts) == NULL) {
        log_error("OpenAPI_event_reporting_requirement_convertToJSON() failed [start_ts]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "endTs", event_reporting_requirement->end_ts) == NULL) {
        log_error("OpenAPI_event_reporting_requirement_convertToJSON() failed [end_ts]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "offsetPeriod", event_reporting_requirement->offset_period) == NULL) {
        log_error("OpenAPI_event_reporting_requirement_convertToJSON() failed [offset_period]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "sampRatio", event_reporting_requirement->samp_ratio) == NULL) {
        log_error("OpenAPI_event_reporting_requirement_convertToJSON() failed [samp_ratio]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "maxObjectNbr", event_reporting_requirement->max_object_nbr) == NULL) {
        log_error("OpenAPI_event_reporting_requirement_convertToJSON() failed [max_object_nbr]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "maxSupiNbr", event_reporting_requirement->max_supi_nbr) == NULL) {
        log_error("OpenAPI_event_reporting_requirement_convertToJSON() failed [max_supi_nbr]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "timeAnaNeeded", event_reporting_requirement->time_ana_needed) == NULL) {
        log_error("OpenAPI_event_reporting_requirement_convertToJSON() failed [time_ana_needed]");
        goto end;
    }
    }
    ...
    if (ana_metaList == NULL) {
        log_error("OpenAPI_event_reporting_requirement_convertToJSON() failed [ana_meta]");
        goto end;
    }
    OpenAPI_list_for_each(event_reporting_requirement->ana_meta, node) {
    ...
    if (ana_meta_ind_local_JSON == NULL) {
        log_error("OpenAPI_event_reporting_requirement_convertToJSON() failed [ana_meta_ind]");
        goto end;
    }
...
OpenAPI_event_reporting_requirement_t *OpenAPI_event_reporting_requirement_parseFromJSON(cJSON *event_reporting_requirementJSON)
{
    ...
    if (!accuracy_local_nonprim) {
        log_error("OpenAPI_accuracy_parseFromJSON failed [accuracy]");
        goto end;
    }
    }
    ...
    if (acc_per_subset) {
        ...
        if (!cJSON_IsArray(acc_per_subset)) {
            log_error("OpenAPI_event_reporting_requirement_parseFromJSON() failed [acc_per_subset]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(acc_per_subset_local, acc_per_subset) {
            ...
            if (!acc_per_subsetItem) {
                log_error("No acc_per_subsetItem");
                goto end;
            }
    ...
    if (!cJSON_IsString(start_ts) && !cJSON_IsNull(start_ts)) {
        log_error("OpenAPI_event_reporting_requirement_parseFromJSON() failed [start_ts]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(end_ts) && !cJSON_IsNull(end_ts)) {
        log_error("OpenAPI_event_reporting_requirement_parseFromJSON() failed [end_ts]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(offset_period)) {
        log_error("OpenAPI_event_reporting_requirement_parseFromJSON() failed [offset_period]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(samp_ratio)) {
        log_error("OpenAPI_event_reporting_requirement_parseFromJSON() failed [samp_ratio]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(max_object_nbr)) {
        log_error("OpenAPI_event_reporting_requirement_parseFromJSON() failed [max_object_nbr]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(max_supi_nbr)) {
        log_error("OpenAPI_event_reporting_requirement_parseFromJSON() failed [max_supi_nbr]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(time_ana_needed) && !cJSON_IsNull(time_ana_needed)) {
        log_error("OpenAPI_event_reporting_requirement_parseFromJSON() failed [time_ana_needed]");
        goto end;
    }
    }
    ...
    if (ana_meta) {
        ...
        if (!cJSON_IsArray(ana_meta)) {
            log_error("OpenAPI_event_reporting_requirement_parseFromJSON() failed [ana_meta]");
            goto end;
        }
    ...
    if (!ana_meta_ind_local_nonprim) {
        log_error("OpenAPI_analytics_metadata_indication_parseFromJSON failed [ana_meta_ind]");
        goto end;
    }
    }
...
OpenAPI_event_reporting_requirement_t *OpenAPI_event_reporting_requirement_copy(OpenAPI_event_reporting_requirement_t *dst, OpenAPI_event_reporting_requirement_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_event_reporting_requirement_convertToJSON() failed");
        return NULL;
    }
