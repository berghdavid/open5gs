...
)
{
    ...
    log_assert(id_translation_result_local_var);
...
cJSON *OpenAPI_id_translation_result_convertToJSON(OpenAPI_id_translation_result_t *id_translation_result)
{
    ...
    if (id_translation_result == NULL) {
        log_error("OpenAPI_id_translation_result_convertToJSON() failed [IdTranslationResult]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", id_translation_result->supported_features) == NULL) {
        log_error("OpenAPI_id_translation_result_convertToJSON() failed [supported_features]");
        goto end;
    }
    }

    if (!id_translation_result->supi) {
        log_error("OpenAPI_id_translation_result_convertToJSON() failed [supi]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "gpsi", id_translation_result->gpsi) == NULL) {
        log_error("OpenAPI_id_translation_result_convertToJSON() failed [gpsi]");
        goto end;
    }
    }
    ...
    if (additional_supisList == NULL) {
        log_error("OpenAPI_id_translation_result_convertToJSON() failed [additional_supis]");
        goto end;
    }
    OpenAPI_list_for_each(id_translation_result->additional_supis, node) {
    ...
    if (additional_gpsisList == NULL) {
        log_error("OpenAPI_id_translation_result_convertToJSON() failed [additional_gpsis]");
        goto end;
    }
    OpenAPI_list_for_each(id_translation_result->additional_gpsis, node) {
...
OpenAPI_id_translation_result_t *OpenAPI_id_translation_result_parseFromJSON(cJSON *id_translation_resultJSON)
{
    ...
    if (!cJSON_IsString(supported_features) && !cJSON_IsNull(supported_features)) {
        log_error("OpenAPI_id_translation_result_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (!supi) {
        log_error("OpenAPI_id_translation_result_parseFromJSON() failed [supi]");
        goto end;
    }
    ...
    if (!cJSON_IsString(gpsi) && !cJSON_IsNull(gpsi)) {
        log_error("OpenAPI_id_translation_result_parseFromJSON() failed [gpsi]");
        goto end;
    }
    }
    ...
    if (additional_supis) {
        ...
        if (!cJSON_IsArray(additional_supis)) {
            log_error("OpenAPI_id_translation_result_parseFromJSON() failed [additional_supis]");
            goto end;
        }
    ...
    if (additional_gpsis) {
        ...
        if (!cJSON_IsArray(additional_gpsis)) {
            log_error("OpenAPI_id_translation_result_parseFromJSON() failed [additional_gpsis]");
            goto end;
        }
...
OpenAPI_id_translation_result_t *OpenAPI_id_translation_result_copy(OpenAPI_id_translation_result_t *dst, OpenAPI_id_translation_result_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_id_translation_result_convertToJSON() failed");
        return NULL;
    }
