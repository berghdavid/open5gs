...
)
{
    ...
    log_assert(network_area_info_local_var);
...
cJSON *OpenAPI_network_area_info_convertToJSON(OpenAPI_network_area_info_t *network_area_info)
{
    ...
    if (network_area_info == NULL) {
        log_error("OpenAPI_network_area_info_convertToJSON() failed [NetworkAreaInfo]");
        return NULL;
    }
    ...
    if (ecgisList == NULL) {
        log_error("OpenAPI_network_area_info_convertToJSON() failed [ecgis]");
        goto end;
    }
    OpenAPI_list_for_each(network_area_info->ecgis, node) {
    ...
    if (ncgisList == NULL) {
        log_error("OpenAPI_network_area_info_convertToJSON() failed [ncgis]");
        goto end;
    }
    OpenAPI_list_for_each(network_area_info->ncgis, node) {
    ...
    if (g_ran_node_idsList == NULL) {
        log_error("OpenAPI_network_area_info_convertToJSON() failed [g_ran_node_ids]");
        goto end;
    }
    OpenAPI_list_for_each(network_area_info->g_ran_node_ids, node) {
    ...
    if (taisList == NULL) {
        log_error("OpenAPI_network_area_info_convertToJSON() failed [tais]");
        goto end;
    }
    OpenAPI_list_for_each(network_area_info->tais, node) {
...
OpenAPI_network_area_info_t *OpenAPI_network_area_info_parseFromJSON(cJSON *network_area_infoJSON)
{
    ...
    if (ecgis) {
        ...
        if (!cJSON_IsArray(ecgis)) {
            log_error("OpenAPI_network_area_info_parseFromJSON() failed [ecgis]");
            goto end;
        }
    ...
    if (ncgis) {
        ...
        if (!cJSON_IsArray(ncgis)) {
            log_error("OpenAPI_network_area_info_parseFromJSON() failed [ncgis]");
            goto end;
        }
    ...
    if (g_ran_node_ids) {
        ...
        if (!cJSON_IsArray(g_ran_node_ids)) {
            log_error("OpenAPI_network_area_info_parseFromJSON() failed [g_ran_node_ids]");
            goto end;
        }
    ...
    if (tais) {
        ...
        if (!cJSON_IsArray(tais)) {
            log_error("OpenAPI_network_area_info_parseFromJSON() failed [tais]");
            goto end;
        }
...
OpenAPI_network_area_info_t *OpenAPI_network_area_info_copy(OpenAPI_network_area_info_t *dst, OpenAPI_network_area_info_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_network_area_info_convertToJSON() failed");
        return NULL;
    }
