...
)
{
    ...
    log_assert(pdu_session_types_1_local_var);
...
cJSON *OpenAPI_pdu_session_types_1_convertToJSON(OpenAPI_pdu_session_types_1_t *pdu_session_types_1)
{
    ...
    if (pdu_session_types_1 == NULL) {
        log_error("OpenAPI_pdu_session_types_1_convertToJSON() failed [PduSessionTypes_1]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "defaultSessionType", OpenAPI_pdu_session_type_ToString(pdu_session_types_1->default_session_type)) == NULL) {
        log_error("OpenAPI_pdu_session_types_1_convertToJSON() failed [default_session_type]");
        goto end;
    }
    }
    ...
    if (allowed_session_typesList == NULL) {
        log_error("OpenAPI_pdu_session_types_1_convertToJSON() failed [allowed_session_types]");
        goto end;
    }
...
OpenAPI_pdu_session_types_1_t *OpenAPI_pdu_session_types_1_parseFromJSON(cJSON *pdu_session_types_1JSON)
{
    ...
    if (!cJSON_IsString(default_session_type)) {
        log_error("OpenAPI_pdu_session_types_1_parseFromJSON() failed [default_session_type]");
        goto end;
    }
    ...
    if (allowed_session_types) {
        ...
        if (!cJSON_IsArray(allowed_session_types)) {
            log_error("OpenAPI_pdu_session_types_1_parseFromJSON() failed [allowed_session_types]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(allowed_session_types_local, allowed_session_types) {
            ...
            if (!localEnum) {
                log_info("Enum value \"%s\" for field \"allowed_session_types\" is not supported. Ignoring it ...",
        ...
        if (allowed_session_typesList->count == 0) {
            log_error("OpenAPI_pdu_session_types_1_parseFromJSON() failed: Expected allowed_session_typesList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
...
OpenAPI_pdu_session_types_1_t *OpenAPI_pdu_session_types_1_copy(OpenAPI_pdu_session_types_1_t *dst, OpenAPI_pdu_session_types_1_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_pdu_session_types_1_convertToJSON() failed");
        return NULL;
    }
