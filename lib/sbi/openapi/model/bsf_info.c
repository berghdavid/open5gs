...
cJSON *OpenAPI_bsf_info_convertToJSON(OpenAPI_bsf_info_t *bsf_info)
{
    ...
    if (bsf_info == NULL) {
        log_error("OpenAPI_bsf_info_convertToJSON() failed [BsfInfo]");
        return NULL;
    }
    ...
    if (dnn_listList == NULL) {
        log_error("OpenAPI_bsf_info_convertToJSON() failed [dnn_list]");
        goto end;
    }
    OpenAPI_list_for_each(bsf_info->dnn_list, node) {
    ...
    if (ip_domain_listList == NULL) {
        log_error("OpenAPI_bsf_info_convertToJSON() failed [ip_domain_list]");
        goto end;
    }
    OpenAPI_list_for_each(bsf_info->ip_domain_list, node) {
    ...
    if (ipv4_address_rangesList == NULL) {
        log_error("OpenAPI_bsf_info_convertToJSON() failed [ipv4_address_ranges]");
        goto end;
    }
    OpenAPI_list_for_each(bsf_info->ipv4_address_ranges, node) {
    ...
    if (ipv6_prefix_rangesList == NULL) {
        log_error("OpenAPI_bsf_info_convertToJSON() failed [ipv6_prefix_ranges]");
        goto end;
    }
    OpenAPI_list_for_each(bsf_info->ipv6_prefix_ranges, node) {
    ...
    if (cJSON_AddStringToObject(item, "rxDiamHost", bsf_info->rx_diam_host) == NULL) {
        log_error("OpenAPI_bsf_info_convertToJSON() failed [rx_diam_host]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "rxDiamRealm", bsf_info->rx_diam_realm) == NULL) {
        log_error("OpenAPI_bsf_info_convertToJSON() failed [rx_diam_realm]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "groupId", bsf_info->group_id) == NULL) {
        log_error("OpenAPI_bsf_info_convertToJSON() failed [group_id]");
        goto end;
    }
    }
    ...
    if (supi_rangesList == NULL) {
        log_error("OpenAPI_bsf_info_convertToJSON() failed [supi_ranges]");
        goto end;
    }
    OpenAPI_list_for_each(bsf_info->supi_ranges, node) {
    ...
    if (gpsi_rangesList == NULL) {
        log_error("OpenAPI_bsf_info_convertToJSON() failed [gpsi_ranges]");
        goto end;
    }
    OpenAPI_list_for_each(bsf_info->gpsi_ranges, node) {
...
OpenAPI_bsf_info_t *OpenAPI_bsf_info_parseFromJSON(cJSON *bsf_infoJSON)
{
    ...
    if (dnn_list) {
        ...
        if (!cJSON_IsArray(dnn_list)) {
            log_error("OpenAPI_bsf_info_parseFromJSON() failed [dnn_list]");
            goto end;
        }
    ...
    if (ip_domain_list) {
        ...
        if (!cJSON_IsArray(ip_domain_list)) {
            log_error("OpenAPI_bsf_info_parseFromJSON() failed [ip_domain_list]");
            goto end;
        }
    ...
    if (ipv4_address_ranges) {
        ...
        if (!cJSON_IsArray(ipv4_address_ranges)) {
            log_error("OpenAPI_bsf_info_parseFromJSON() failed [ipv4_address_ranges]");
            goto end;
        }
    ...
    if (ipv6_prefix_ranges) {
        ...
        if (!cJSON_IsArray(ipv6_prefix_ranges)) {
            log_error("OpenAPI_bsf_info_parseFromJSON() failed [ipv6_prefix_ranges]");
            goto end;
        }
    ...
    if (!cJSON_IsString(rx_diam_host) && !cJSON_IsNull(rx_diam_host)) {
        log_error("OpenAPI_bsf_info_parseFromJSON() failed [rx_diam_host]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(rx_diam_realm) && !cJSON_IsNull(rx_diam_realm)) {
        log_error("OpenAPI_bsf_info_parseFromJSON() failed [rx_diam_realm]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(group_id) && !cJSON_IsNull(group_id)) {
        log_error("OpenAPI_bsf_info_parseFromJSON() failed [group_id]");
        goto end;
    }
    }
    ...
    if (supi_ranges) {
        ...
        if (!cJSON_IsArray(supi_ranges)) {
            log_error("OpenAPI_bsf_info_parseFromJSON() failed [supi_ranges]");
            goto end;
        }
    ...
    if (gpsi_ranges) {
        ...
        if (!cJSON_IsArray(gpsi_ranges)) {
            log_error("OpenAPI_bsf_info_parseFromJSON() failed [gpsi_ranges]");
            goto end;
        }
...
OpenAPI_bsf_info_t *OpenAPI_bsf_info_copy(OpenAPI_bsf_info_t *dst, OpenAPI_bsf_info_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_bsf_info_convertToJSON() failed");
        return NULL;
    }
