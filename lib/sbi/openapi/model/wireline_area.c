...
)
{
    ...
    log_assert(wireline_area_local_var);
...
cJSON *OpenAPI_wireline_area_convertToJSON(OpenAPI_wireline_area_t *wireline_area)
{
    ...
    if (wireline_area == NULL) {
        log_error("OpenAPI_wireline_area_convertToJSON() failed [WirelineArea]");
        return NULL;
    }
    ...
    if (global_line_idsList == NULL) {
        log_error("OpenAPI_wireline_area_convertToJSON() failed [global_line_ids]");
        goto end;
    }
    OpenAPI_list_for_each(wireline_area->global_line_ids, node) {
    ...
    if (hfc_n_idsList == NULL) {
        log_error("OpenAPI_wireline_area_convertToJSON() failed [hfc_n_ids]");
        goto end;
    }
    OpenAPI_list_for_each(wireline_area->hfc_n_ids, node) {
    ...
    if (cJSON_AddStringToObject(item, "areaCodeB", wireline_area->area_code_b) == NULL) {
        log_error("OpenAPI_wireline_area_convertToJSON() failed [area_code_b]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "areaCodeC", wireline_area->area_code_c) == NULL) {
        log_error("OpenAPI_wireline_area_convertToJSON() failed [area_code_c]");
        goto end;
    }
    }
...
OpenAPI_wireline_area_t *OpenAPI_wireline_area_parseFromJSON(cJSON *wireline_areaJSON)
{
    ...
    if (global_line_ids) {
        ...
        if (!cJSON_IsArray(global_line_ids)) {
            log_error("OpenAPI_wireline_area_parseFromJSON() failed [global_line_ids]");
            goto end;
        }
    ...
    if (hfc_n_ids) {
        ...
        if (!cJSON_IsArray(hfc_n_ids)) {
            log_error("OpenAPI_wireline_area_parseFromJSON() failed [hfc_n_ids]");
            goto end;
        }
    ...
    if (!cJSON_IsString(area_code_b) && !cJSON_IsNull(area_code_b)) {
        log_error("OpenAPI_wireline_area_parseFromJSON() failed [area_code_b]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(area_code_c) && !cJSON_IsNull(area_code_c)) {
        log_error("OpenAPI_wireline_area_parseFromJSON() failed [area_code_c]");
        goto end;
    }
    }
...
OpenAPI_wireline_area_t *OpenAPI_wireline_area_copy(OpenAPI_wireline_area_t *dst, OpenAPI_wireline_area_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_wireline_area_convertToJSON() failed");
        return NULL;
    }
