...
)
{
    ...
    log_assert(scheduled_communication_time_1_local_var);
...
cJSON *OpenAPI_scheduled_communication_time_1_convertToJSON(OpenAPI_scheduled_communication_time_1_t *scheduled_communication_time_1)
{
    ...
    if (scheduled_communication_time_1 == NULL) {
        log_error("OpenAPI_scheduled_communication_time_1_convertToJSON() failed [ScheduledCommunicationTime_1]");
        return NULL;
    }
    ...
    if (days_of_weekList == NULL) {
        log_error("OpenAPI_scheduled_communication_time_1_convertToJSON() failed [days_of_week]");
        goto end;
    }
    OpenAPI_list_for_each(scheduled_communication_time_1->days_of_week, node) {
        if (node->data == NULL) {
    ...
    if (cJSON_AddStringToObject(item, "timeOfDayStart", scheduled_communication_time_1->time_of_day_start) == NULL) {
        log_error("OpenAPI_scheduled_communication_time_1_convertToJSON() failed [time_of_day_start]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "timeOfDayEnd", scheduled_communication_time_1->time_of_day_end) == NULL) {
        log_error("OpenAPI_scheduled_communication_time_1_convertToJSON() failed [time_of_day_end]");
        goto end;
    }
    }
...
OpenAPI_scheduled_communication_time_1_t *OpenAPI_scheduled_communication_time_1_parseFromJSON(cJSON *scheduled_communication_time_1JSON)
{
    ...
    if (days_of_week) {
        ...
        if (!cJSON_IsArray(days_of_week)) {
            log_error("OpenAPI_scheduled_communication_time_1_parseFromJSON() failed [days_of_week]");
            goto end;
        }
    ...
    if (!cJSON_IsString(time_of_day_start) && !cJSON_IsNull(time_of_day_start)) {
        log_error("OpenAPI_scheduled_communication_time_1_parseFromJSON() failed [time_of_day_start]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(time_of_day_end) && !cJSON_IsNull(time_of_day_end)) {
        log_error("OpenAPI_scheduled_communication_time_1_parseFromJSON() failed [time_of_day_end]");
        goto end;
    }
    }
...
OpenAPI_scheduled_communication_time_1_t *OpenAPI_scheduled_communication_time_1_copy(OpenAPI_scheduled_communication_time_1_t *dst, OpenAPI_scheduled_communication_time_1_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_scheduled_communication_time_1_convertToJSON() failed");
        return NULL;
    }
