...
)
{
    ...
    log_assert(bsf_notification_local_var);
...
cJSON *OpenAPI_bsf_notification_convertToJSON(OpenAPI_bsf_notification_t *bsf_notification)
{
    ...
    if (bsf_notification == NULL) {
        log_error("OpenAPI_bsf_notification_convertToJSON() failed [BsfNotification]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!bsf_notification->notif_corre_id) {
        log_error("OpenAPI_bsf_notification_convertToJSON() failed [notif_corre_id]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "pcfId", bsf_notification->pcf_id) == NULL) {
        log_error("OpenAPI_bsf_notification_convertToJSON() failed [pcf_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "pcfSetId", bsf_notification->pcf_set_id) == NULL) {
        log_error("OpenAPI_bsf_notification_convertToJSON() failed [pcf_set_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "bindLevel", OpenAPI_binding_level_ToString(bsf_notification->bind_level)) == NULL) {
        log_error("OpenAPI_bsf_notification_convertToJSON() failed [bind_level]");
        goto end;
    }
    }

    if (!bsf_notification->event_notifs) {
        log_error("OpenAPI_bsf_notification_convertToJSON() failed [event_notifs]");
        return NULL;
    }
...
OpenAPI_bsf_notification_t *OpenAPI_bsf_notification_parseFromJSON(cJSON *bsf_notificationJSON)
{
    ...
    if (!notif_corre_id) {
        log_error("OpenAPI_bsf_notification_parseFromJSON() failed [notif_corre_id]");
        goto end;
    }
    ...
    if (!cJSON_IsString(pcf_id) && !cJSON_IsNull(pcf_id)) {
        log_error("OpenAPI_bsf_notification_parseFromJSON() failed [pcf_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(pcf_set_id) && !cJSON_IsNull(pcf_set_id)) {
        log_error("OpenAPI_bsf_notification_parseFromJSON() failed [pcf_set_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(bind_level)) {
        log_error("OpenAPI_bsf_notification_parseFromJSON() failed [bind_level]");
        goto end;
    }
    ...
    if (!event_notifs) {
        log_error("OpenAPI_bsf_notification_parseFromJSON() failed [event_notifs]");
        goto end;
    }
...
OpenAPI_bsf_notification_t *OpenAPI_bsf_notification_copy(OpenAPI_bsf_notification_t *dst, OpenAPI_bsf_notification_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_bsf_notification_convertToJSON() failed");
        return NULL;
    }
