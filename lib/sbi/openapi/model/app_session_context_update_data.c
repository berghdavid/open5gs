...
)
{
    ...
    log_assert(app_session_context_update_data_local_var);
...
cJSON *OpenAPI_app_session_context_update_data_convertToJSON(OpenAPI_app_session_context_update_data_t *app_session_context_update_data)
{
    ...
    if (app_session_context_update_data == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [AppSessionContextUpdateData]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "afAppId", app_session_context_update_data->af_app_id) == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [af_app_id]");
        goto end;
    }
    }
    ...
    if (af_rout_req_local_JSON == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [af_rout_req]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "aspId", app_session_context_update_data->asp_id) == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [asp_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "bdtRefId", app_session_context_update_data->bdt_ref_id) == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [bdt_ref_id]");
        goto end;
    }
    }
    ...
    if (ev_subsc_local_JSON == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [ev_subsc]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "mcpttId", app_session_context_update_data->mcptt_id) == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [mcptt_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "mcVideoId", app_session_context_update_data->mc_video_id) == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [mc_video_id]");
        goto end;
    }
    }
    ...
    if (med_components == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [med_components]");
        goto end;
    }
    ...
    if (app_session_context_update_data->med_components) {
        OpenAPI_list_for_each(app_session_context_update_data->med_components, node) {
            ...
            cJSON *itemLocal = localKeyValue->value ?
                ...
                cJSON_CreateNull();
            if (itemLocal == NULL) {
                log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [inner]");
                goto end;
            }
    ...
    if (cJSON_AddStringToObject(item, "mpsAction", OpenAPI_mps_action_ToString(app_session_context_update_data->mps_action)) == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [mps_action]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "mpsId", app_session_context_update_data->mps_id) == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [mps_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "mcsId", app_session_context_update_data->mcs_id) == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [mcs_id]");
        goto end;
    }
    }
    ...
    if (preempt_control_info_local_JSON == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [preempt_control_info]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "resPrio", OpenAPI_reserv_priority_ToString(app_session_context_update_data->res_prio)) == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [res_prio]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "servInfStatus", OpenAPI_service_info_status_ToString(app_session_context_update_data->serv_inf_status)) == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [serv_inf_status]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "sipForkInd", OpenAPI_sip_forking_indication_ToString(app_session_context_update_data->sip_fork_ind)) == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [sip_fork_ind]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "sponId", app_session_context_update_data->spon_id) == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [spon_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "sponStatus", OpenAPI_sponsoring_status_ToString(app_session_context_update_data->spon_status)) == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [spon_status]");
        goto end;
    }
    }
    ...
    if (tsn_bridge_man_cont_local_JSON == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [tsn_bridge_man_cont]");
        goto end;
    }
    ...
    if (tsn_port_man_cont_dstt_local_JSON == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [tsn_port_man_cont_dstt]");
        goto end;
    }
    ...
    if (tsn_port_man_cont_nwttsList == NULL) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed [tsn_port_man_cont_nwtts]");
        goto end;
    }
    OpenAPI_list_for_each(app_session_context_update_data->tsn_port_man_cont_nwtts, node) {
...
OpenAPI_app_session_context_update_data_t *OpenAPI_app_session_context_update_data_parseFromJSON(cJSON *app_session_context_update_dataJSON)
{
    ...
    if (!cJSON_IsString(af_app_id) && !cJSON_IsNull(af_app_id)) {
        log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [af_app_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(asp_id) && !cJSON_IsNull(asp_id)) {
        log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [asp_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(bdt_ref_id) && !cJSON_IsNull(bdt_ref_id)) {
        log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [bdt_ref_id]");
        goto end;
    }
    }
    ...
    if (!ev_subsc_local_nonprim) {
        log_error("OpenAPI_events_subsc_req_data_rm_parseFromJSON failed [ev_subsc]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsString(mcptt_id) && !cJSON_IsNull(mcptt_id)) {
        log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [mcptt_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(mc_video_id) && !cJSON_IsNull(mc_video_id)) {
        log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [mc_video_id]");
        goto end;
    }
    }
    ...
    if (med_components) {
        ...
        if (!cJSON_IsObject(med_components) && !cJSON_IsNull(med_components)) {
            log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [med_components]");
            goto end;
        }
        if (cJSON_IsObject(med_components)) {
            ...
            cJSON_ArrayForEach(med_components_local_map, med_components) {
                ...
                if (cJSON_IsObject(localMapObject)) {
                    ...
                    log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [inner]");
                    goto end;
                }
    ...
    if (!cJSON_IsString(mps_action)) {
        log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [mps_action]");
        goto end;
    }
    ...
    if (!cJSON_IsString(mps_id) && !cJSON_IsNull(mps_id)) {
        log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [mps_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(mcs_id) && !cJSON_IsNull(mcs_id)) {
        log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [mcs_id]");
        goto end;
    }
    }
    ...
    if (!preempt_control_info_local_nonprim) {
        log_error("OpenAPI_preemption_control_information_rm_parseFromJSON failed [preempt_control_info]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(res_prio)) {
        log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [res_prio]");
        goto end;
    }
    ...
    if (!cJSON_IsString(serv_inf_status)) {
        log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [serv_inf_status]");
        goto end;
    }
    ...
    if (!cJSON_IsString(sip_fork_ind)) {
        log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [sip_fork_ind]");
        goto end;
    }
    ...
    if (!cJSON_IsString(spon_id) && !cJSON_IsNull(spon_id)) {
        log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [spon_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(spon_status)) {
        log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [spon_status]");
        goto end;
    }
    ...
    if (tsn_port_man_cont_nwtts) {
        ...
        if (!cJSON_IsArray(tsn_port_man_cont_nwtts)) {
            log_error("OpenAPI_app_session_context_update_data_parseFromJSON() failed [tsn_port_man_cont_nwtts]");
            goto end;
        }
...
OpenAPI_app_session_context_update_data_t *OpenAPI_app_session_context_update_data_copy(OpenAPI_app_session_context_update_data_t *dst, OpenAPI_app_session_context_update_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_app_session_context_update_data_convertToJSON() failed");
        return NULL;
    }
