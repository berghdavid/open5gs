...
)
{
    ...
    log_assert(nssai_ack_data_local_var);
...
cJSON *OpenAPI_nssai_ack_data_convertToJSON(OpenAPI_nssai_ack_data_t *nssai_ack_data)
{
    ...
    if (nssai_ack_data == NULL) {
        log_error("OpenAPI_nssai_ack_data_convertToJSON() failed [NssaiAckData]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!nssai_ack_data->provisioning_time) {
        log_error("OpenAPI_nssai_ack_data_convertToJSON() failed [provisioning_time]");
        return NULL;
    }
    ...
    if (nssai_ack_data->ue_update_status == OpenAPI_ue_update_status_NULL) {
        log_error("OpenAPI_nssai_ack_data_convertToJSON() failed [ue_update_status]");
        return NULL;
    }
...
OpenAPI_nssai_ack_data_t *OpenAPI_nssai_ack_data_parseFromJSON(cJSON *nssai_ack_dataJSON)
{
    ...
    if (!provisioning_time) {
        log_error("OpenAPI_nssai_ack_data_parseFromJSON() failed [provisioning_time]");
        goto end;
    }
    ...
    if (!ue_update_status) {
        log_error("OpenAPI_nssai_ack_data_parseFromJSON() failed [ue_update_status]");
        goto end;
    }
...
OpenAPI_nssai_ack_data_t *OpenAPI_nssai_ack_data_copy(OpenAPI_nssai_ack_data_t *dst, OpenAPI_nssai_ack_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_nssai_ack_data_convertToJSON() failed");
        return NULL;
    }
