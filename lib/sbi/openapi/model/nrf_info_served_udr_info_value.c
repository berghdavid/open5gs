...
)
{
    ...
    log_assert(nrf_info_served_udr_info_value_local_var);
...
cJSON *OpenAPI_nrf_info_served_udr_info_value_convertToJSON(OpenAPI_nrf_info_served_udr_info_value_t *nrf_info_served_udr_info_value)
{
    ...
    if (nrf_info_served_udr_info_value == NULL) {
        log_error("OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [NrfInfo_servedUdrInfo_value]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "groupId", nrf_info_served_udr_info_value->group_id) == NULL) {
        log_error("OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [group_id]");
        goto end;
    }
    }
    ...
    if (supi_rangesList == NULL) {
        log_error("OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [supi_ranges]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_udr_info_value->supi_ranges, node) {
    ...
    if (gpsi_rangesList == NULL) {
        log_error("OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [gpsi_ranges]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_udr_info_value->gpsi_ranges, node) {
    ...
    if (external_group_identifiers_rangesList == NULL) {
        log_error("OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [external_group_identifiers_ranges]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_udr_info_value->external_group_identifiers_ranges, node) {
        cJSON *itemLocal = OpenAPI_identity_range_convertToJSON(node->data);
    ...
    if (supported_data_setsList == NULL) {
        log_error("OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [supported_data_sets]");
        goto end;
    }
    ...
    if (shared_data_id_rangesList == NULL) {
        log_error("OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [shared_data_id_ranges]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_udr_info_value->shared_data_id_ranges, node) {
...
OpenAPI_nrf_info_served_udr_info_value_t *OpenAPI_nrf_info_served_udr_info_value_parseFromJSON(cJSON *nrf_info_served_udr_info_valueJSON)
{
    ...
    if (!cJSON_IsString(group_id) && !cJSON_IsNull(group_id)) {
        log_error("OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed [group_id]");
        goto end;
    }
    }
    ...
    if (supi_ranges) {
        ...
        if (!cJSON_IsArray(supi_ranges)) {
            log_error("OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed [supi_ranges]");
            goto end;
        }
    ...
    if (gpsi_ranges) {
        ...
        if (!cJSON_IsArray(gpsi_ranges)) {
            log_error("OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed [gpsi_ranges]");
            goto end;
        }
    ...
    if (external_group_identifiers_ranges) {
        ...
        if (!cJSON_IsArray(external_group_identifiers_ranges)) {
            log_error("OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed [external_group_identifiers_ranges]");
            goto end;
        }
    ...
    if (supported_data_sets) {
        ...
        if (!cJSON_IsArray(supported_data_sets)) {
            log_error("OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed [supported_data_sets]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(supported_data_sets_local, supported_data_sets) {
            ...
            if (!localEnum) {
                log_info("Enum value \"%s\" for field \"supported_data_sets\" is not supported. Ignoring it ...",
        ...
        if (supported_data_setsList->count == 0) {
            log_error("OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed: Expected supported_data_setsList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
    ...
    if (shared_data_id_ranges) {
        ...
        if (!cJSON_IsArray(shared_data_id_ranges)) {
            log_error("OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed [shared_data_id_ranges]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(shared_data_id_ranges_local, shared_data_id_ranges) {
            ...
            if (!shared_data_id_rangesItem) {
                log_error("No shared_data_id_rangesItem");
                goto end;
            }
...
OpenAPI_nrf_info_served_udr_info_value_t *OpenAPI_nrf_info_served_udr_info_value_copy(OpenAPI_nrf_info_served_udr_info_value_t *dst, OpenAPI_nrf_info_served_udr_info_value_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed");
        return NULL;
    }
