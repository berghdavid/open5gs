...
)
{
    ...
    log_assert(ee_subscription_local_var);
...
cJSON *OpenAPI_ee_subscription_convertToJSON(OpenAPI_ee_subscription_t *ee_subscription)
{
    ...
    if (ee_subscription == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [EeSubscription]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!ee_subscription->callback_reference) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [callback_reference]");
        return NULL;
    }
    ...
    if (!ee_subscription->monitoring_configurations) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [monitoring_configurations]");
        return NULL;
    }
    ...
    if (ee_subscription->monitoring_configurations) {
        OpenAPI_list_for_each(ee_subscription->monitoring_configurations, node) {
            ...
            cJSON *itemLocal = localKeyValue->value ?
                ...
                cJSON_CreateNull();
            if (itemLocal == NULL) {
                log_error("OpenAPI_ee_subscription_convertToJSON() failed [inner]");
                goto end;
            }
    ...
    if (reporting_options_local_JSON == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [reporting_options]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", ee_subscription->supported_features) == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "subscriptionId", ee_subscription->subscription_id) == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [subscription_id]");
        goto end;
    }
    }
    ...
    if (context_info_local_JSON == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [context_info]");
        goto end;
    }
    ...
    if (cJSON_AddBoolToObject(item, "epcAppliedInd", ee_subscription->epc_applied_ind) == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [epc_applied_ind]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "scefDiamHost", ee_subscription->scef_diam_host) == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [scef_diam_host]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "scefDiamRealm", ee_subscription->scef_diam_realm) == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [scef_diam_realm]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "notifyCorrelationId", ee_subscription->notify_correlation_id) == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [notify_correlation_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "secondCallbackRef", ee_subscription->second_callback_ref) == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [second_callback_ref]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "gpsi", ee_subscription->gpsi) == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [gpsi]");
        goto end;
    }
    }
    ...
    if (exclude_gpsi_listList == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [exclude_gpsi_list]");
        goto end;
    }
    OpenAPI_list_for_each(ee_subscription->exclude_gpsi_list, node) {
    ...
    if (include_gpsi_listList == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [include_gpsi_list]");
        goto end;
    }
    OpenAPI_list_for_each(ee_subscription->include_gpsi_list, node) {
    ...
    if (cJSON_AddStringToObject(item, "dataRestorationCallbackUri", ee_subscription->data_restoration_callback_uri) == NULL) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed [data_restoration_callback_uri]");
        goto end;
    }
    }
...
OpenAPI_ee_subscription_t *OpenAPI_ee_subscription_parseFromJSON(cJSON *ee_subscriptionJSON)
{
    ...
    if (!callback_reference) {
        log_error("OpenAPI_ee_subscription_parseFromJSON() failed [callback_reference]");
        goto end;
    }
    ...
    if (!monitoring_configurations) {
        log_error("OpenAPI_ee_subscription_parseFromJSON() failed [monitoring_configurations]");
        goto end;
    }
        ...
        if (cJSON_IsObject(monitoring_configurations)) {
            ...
            cJSON_ArrayForEach(monitoring_configurations_local_map, monitoring_configurations) {
                ...
                if (cJSON_IsObject(localMapObject)) {
                    ...
                    log_error("OpenAPI_ee_subscription_parseFromJSON() failed [inner]");
                    goto end;
                }
    ...
    if (!reporting_options_local_nonprim) {
        log_error("OpenAPI_reporting_options_parseFromJSON failed [reporting_options]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(supported_features) && !cJSON_IsNull(supported_features)) {
        log_error("OpenAPI_ee_subscription_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(subscription_id) && !cJSON_IsNull(subscription_id)) {
        log_error("OpenAPI_ee_subscription_parseFromJSON() failed [subscription_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(epc_applied_ind)) {
        log_error("OpenAPI_ee_subscription_parseFromJSON() failed [epc_applied_ind]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(scef_diam_host) && !cJSON_IsNull(scef_diam_host)) {
        log_error("OpenAPI_ee_subscription_parseFromJSON() failed [scef_diam_host]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(scef_diam_realm) && !cJSON_IsNull(scef_diam_realm)) {
        log_error("OpenAPI_ee_subscription_parseFromJSON() failed [scef_diam_realm]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(notify_correlation_id) && !cJSON_IsNull(notify_correlation_id)) {
        log_error("OpenAPI_ee_subscription_parseFromJSON() failed [notify_correlation_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(second_callback_ref) && !cJSON_IsNull(second_callback_ref)) {
        log_error("OpenAPI_ee_subscription_parseFromJSON() failed [second_callback_ref]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(gpsi) && !cJSON_IsNull(gpsi)) {
        log_error("OpenAPI_ee_subscription_parseFromJSON() failed [gpsi]");
        goto end;
    }
    }
    ...
    if (exclude_gpsi_list) {
        ...
        if (!cJSON_IsArray(exclude_gpsi_list)) {
            log_error("OpenAPI_ee_subscription_parseFromJSON() failed [exclude_gpsi_list]");
            goto end;
        }
    ...
    if (include_gpsi_list) {
        ...
        if (!cJSON_IsArray(include_gpsi_list)) {
            log_error("OpenAPI_ee_subscription_parseFromJSON() failed [include_gpsi_list]");
            goto end;
        }
    ...
    if (!cJSON_IsString(data_restoration_callback_uri) && !cJSON_IsNull(data_restoration_callback_uri)) {
        log_error("OpenAPI_ee_subscription_parseFromJSON() failed [data_restoration_callback_uri]");
        goto end;
    }
    }
...
OpenAPI_ee_subscription_t *OpenAPI_ee_subscription_copy(OpenAPI_ee_subscription_t *dst, OpenAPI_ee_subscription_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_ee_subscription_convertToJSON() failed");
        return NULL;
    }
