...
)
{
    ...
    log_assert(n2_information_transfer_req_data_local_var);
...
cJSON *OpenAPI_n2_information_transfer_req_data_convertToJSON(OpenAPI_n2_information_transfer_req_data_t *n2_information_transfer_req_data)
{
    ...
    if (n2_information_transfer_req_data == NULL) {
        log_error("OpenAPI_n2_information_transfer_req_data_convertToJSON() failed [N2InformationTransferReqData]");
        return NULL;
    }
    ...
    if (tai_listList == NULL) {
        log_error("OpenAPI_n2_information_transfer_req_data_convertToJSON() failed [tai_list]");
        goto end;
    }
    OpenAPI_list_for_each(n2_information_transfer_req_data->tai_list, node) {
    ...
    if (cJSON_AddStringToObject(item, "ratSelector", OpenAPI_rat_selector_ToString(n2_information_transfer_req_data->rat_selector)) == NULL) {
        log_error("OpenAPI_n2_information_transfer_req_data_convertToJSON() failed [rat_selector]");
        goto end;
    }
    }
    ...
    if (global_ran_node_listList == NULL) {
        log_error("OpenAPI_n2_information_transfer_req_data_convertToJSON() failed [global_ran_node_list]");
        goto end;
    }
    OpenAPI_list_for_each(n2_information_transfer_req_data->global_ran_node_list, node) {
    ...
    if (!n2_information_transfer_req_data->n2_information) {
        log_error("OpenAPI_n2_information_transfer_req_data_convertToJSON() failed [n2_information]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", n2_information_transfer_req_data->supported_features) == NULL) {
        log_error("OpenAPI_n2_information_transfer_req_data_convertToJSON() failed [supported_features]");
        goto end;
    }
    }
...
OpenAPI_n2_information_transfer_req_data_t *OpenAPI_n2_information_transfer_req_data_parseFromJSON(cJSON *n2_information_transfer_req_dataJSON)
{
    ...
    if (tai_list) {
        ...
        if (!cJSON_IsArray(tai_list)) {
            log_error("OpenAPI_n2_information_transfer_req_data_parseFromJSON() failed [tai_list]");
            goto end;
        }
    ...
    if (!cJSON_IsString(rat_selector)) {
        log_error("OpenAPI_n2_information_transfer_req_data_parseFromJSON() failed [rat_selector]");
        goto end;
    }
    ...
    if (global_ran_node_list) {
        ...
        if (!cJSON_IsArray(global_ran_node_list)) {
            log_error("OpenAPI_n2_information_transfer_req_data_parseFromJSON() failed [global_ran_node_list]");
            goto end;
        }
    ...
    if (!n2_information) {
        log_error("OpenAPI_n2_information_transfer_req_data_parseFromJSON() failed [n2_information]");
        goto end;
    }
    ...
    if (!cJSON_IsString(supported_features) && !cJSON_IsNull(supported_features)) {
        log_error("OpenAPI_n2_information_transfer_req_data_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
...
OpenAPI_n2_information_transfer_req_data_t *OpenAPI_n2_information_transfer_req_data_copy(OpenAPI_n2_information_transfer_req_data_t *dst, OpenAPI_n2_information_transfer_req_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_n2_information_transfer_req_data_convertToJSON() failed");
        return NULL;
    }
