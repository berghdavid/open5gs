...
)
{
    ...
    log_assert(dccf_info_local_var);
...
cJSON *OpenAPI_dccf_info_convertToJSON(OpenAPI_dccf_info_t *dccf_info)
{
    ...
    if (dccf_info == NULL) {
        log_error("OpenAPI_dccf_info_convertToJSON() failed [DccfInfo]");
        return NULL;
    }
    ...
    if (serving_nf_type_listList == NULL) {
        log_error("OpenAPI_dccf_info_convertToJSON() failed [serving_nf_type_list]");
        goto end;
    }
    ...
    if (serving_nf_set_id_listList == NULL) {
        log_error("OpenAPI_dccf_info_convertToJSON() failed [serving_nf_set_id_list]");
        goto end;
    }
    OpenAPI_list_for_each(dccf_info->serving_nf_set_id_list, node) {
    ...
    if (tai_listList == NULL) {
        log_error("OpenAPI_dccf_info_convertToJSON() failed [tai_list]");
        goto end;
    }
    OpenAPI_list_for_each(dccf_info->tai_list, node) {
    ...
    if (tai_range_listList == NULL) {
        log_error("OpenAPI_dccf_info_convertToJSON() failed [tai_range_list]");
        goto end;
    }
    OpenAPI_list_for_each(dccf_info->tai_range_list, node) {
...
OpenAPI_dccf_info_t *OpenAPI_dccf_info_parseFromJSON(cJSON *dccf_infoJSON)
{
    ...
    if (serving_nf_type_list) {
        ...
        if (!cJSON_IsArray(serving_nf_type_list)) {
            log_error("OpenAPI_dccf_info_parseFromJSON() failed [serving_nf_type_list]");
            goto end;
        }
        ...
        if (serving_nf_type_listList->count == 0) {
            log_error("OpenAPI_dccf_info_parseFromJSON() failed: Expected serving_nf_type_listList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
    ...
    if (serving_nf_set_id_list) {
        ...
        if (!cJSON_IsArray(serving_nf_set_id_list)) {
            log_error("OpenAPI_dccf_info_parseFromJSON() failed [serving_nf_set_id_list]");
            goto end;
        }
    ...
    if (tai_list) {
        ...
        if (!cJSON_IsArray(tai_list)) {
            log_error("OpenAPI_dccf_info_parseFromJSON() failed [tai_list]");
            goto end;
        }
    ...
    if (tai_range_list) {
        ...
        if (!cJSON_IsArray(tai_range_list)) {
            log_error("OpenAPI_dccf_info_parseFromJSON() failed [tai_range_list]");
            goto end;
        }
...
OpenAPI_dccf_info_t *OpenAPI_dccf_info_copy(OpenAPI_dccf_info_t *dst, OpenAPI_dccf_info_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_dccf_info_convertToJSON() failed");
        return NULL;
    }
