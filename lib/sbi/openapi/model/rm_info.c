...
cJSON *OpenAPI_rm_info_convertToJSON(OpenAPI_rm_info_t *rm_info)
{
    ...
    if (rm_info == NULL) {
        log_error("OpenAPI_rm_info_convertToJSON() failed [RmInfo]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!rm_info->rm_state) {
        log_error("OpenAPI_rm_info_convertToJSON() failed [rm_state]");
        return NULL;
    }
    ...
    if (rm_info->access_type == OpenAPI_access_type_NULL) {
        log_error("OpenAPI_rm_info_convertToJSON() failed [access_type]");
        return NULL;
    }
...
OpenAPI_rm_info_t *OpenAPI_rm_info_parseFromJSON(cJSON *rm_infoJSON)
{
    ...
    if (!rm_state) {
        log_error("OpenAPI_rm_info_parseFromJSON() failed [rm_state]");
        goto end;
    }
    rm_state_local_nonprim = OpenAPI_rm_state_parseFromJSON(rm_state);
    if (!rm_state_local_nonprim) {
        log_error("OpenAPI_rm_state_parseFromJSON failed [rm_state]");
        goto end;
    }

    access_type = cJSON_GetObjectItemCaseSensitive(rm_infoJSON, "accessType");
    if (!access_type) {
        log_error("OpenAPI_rm_info_parseFromJSON() failed [access_type]");
        goto end;
    }
...
OpenAPI_rm_info_t *OpenAPI_rm_info_copy(OpenAPI_rm_info_t *dst, OpenAPI_rm_info_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_rm_info_convertToJSON() failed");
        return NULL;
    }
