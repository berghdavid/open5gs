...
)
{
    ...
    log_assert(traffic_control_data_local_var);
...
cJSON *OpenAPI_traffic_control_data_convertToJSON(OpenAPI_traffic_control_data_t *traffic_control_data)
{
    ...
    if (traffic_control_data == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [TrafficControlData]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!traffic_control_data->tc_id) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [tc_id]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "flowStatus", OpenAPI_flow_status_ToString(traffic_control_data->flow_status)) == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [flow_status]");
        goto end;
    }
    }
    ...
    if (redirect_info_local_JSON == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [redirect_info]");
        goto end;
    }
    ...
    if (add_redirect_infoList == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [add_redirect_info]");
        goto end;
    }
    OpenAPI_list_for_each(traffic_control_data->add_redirect_info, node) {
    ...
    if (cJSON_AddBoolToObject(item, "muteNotif", traffic_control_data->mute_notif) == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [mute_notif]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "trafficSteeringPolIdDl", traffic_control_data->traffic_steering_pol_id_dl) == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [traffic_steering_pol_id_dl]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "trafficSteeringPolIdUl", traffic_control_data->traffic_steering_pol_id_ul) == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [traffic_steering_pol_id_ul]");
        goto end;
    }
    ...
    if (route_to_locsList == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [route_to_locs]");
        goto end;
    }
    OpenAPI_list_for_each(traffic_control_data->route_to_locs, node) {
    ...
    if (cJSON_AddNumberToObject(item, "maxAllowedUpLat", traffic_control_data->max_allowed_up_lat) == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [max_allowed_up_lat]");
        goto end;
    }
    ...
    if (eas_ip_replace_infosList == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [eas_ip_replace_infos]");
        goto end;
    }
    OpenAPI_list_for_each(traffic_control_data->eas_ip_replace_infos, node) {
    ...
    if (cJSON_AddBoolToObject(item, "traffCorreInd", traffic_control_data->traff_corre_ind) == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [traff_corre_ind]");
        goto end;
    }
    }
    ...
    if (cJSON_AddBoolToObject(item, "simConnInd", traffic_control_data->sim_conn_ind) == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [sim_conn_ind]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "simConnTerm", traffic_control_data->sim_conn_term) == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [sim_conn_term]");
        goto end;
    }
    }
    ...
    if (up_path_chg_event_local_JSON == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [up_path_chg_event]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "steerFun", OpenAPI_steering_functionality_ToString(traffic_control_data->steer_fun)) == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [steer_fun]");
        goto end;
    }
    }
    ...
    if (steer_mode_dl_local_JSON == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [steer_mode_dl]");
        goto end;
    }
    ...
    if (steer_mode_ul_local_JSON == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [steer_mode_ul]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "mulAccCtrl", OpenAPI_npcf_multicast_access_control_ToString(traffic_control_data->mul_acc_ctrl)) == NULL) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed [mul_acc_ctrl]");
        goto end;
    }
    }
...
OpenAPI_traffic_control_data_t *OpenAPI_traffic_control_data_parseFromJSON(cJSON *traffic_control_dataJSON)
{
    ...
    if (!tc_id) {
        log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [tc_id]");
        goto end;
    }
    ...
    if (!cJSON_IsString(flow_status)) {
        log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [flow_status]");
        goto end;
    }
    ...
    if (!redirect_info_local_nonprim) {
        log_error("OpenAPI_redirect_information_parseFromJSON failed [redirect_info]");
        goto end;
    }
    }
    ...
    if (add_redirect_info) {
        ...
        if (!cJSON_IsArray(add_redirect_info)) {
            log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [add_redirect_info]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(add_redirect_info_local, add_redirect_info) {
            ...
            if (!add_redirect_infoItem) {
                log_error("No add_redirect_infoItem");
                goto end;
            }
    ...
    if (!cJSON_IsBool(mute_notif)) {
        log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [mute_notif]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(traffic_steering_pol_id_dl) && !cJSON_IsNull(traffic_steering_pol_id_dl)) {
        log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [traffic_steering_pol_id_dl]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsString(traffic_steering_pol_id_ul) && !cJSON_IsNull(traffic_steering_pol_id_ul)) {
        log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [traffic_steering_pol_id_ul]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsNull(route_to_locs)) {
        ...
        if (!cJSON_IsArray(route_to_locs)) {
            log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [route_to_locs]");
            goto end;
        }
    ...
    if (!cJSON_IsNumber(max_allowed_up_lat)) {
        log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [max_allowed_up_lat]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsNull(eas_ip_replace_infos)) {
        ...
        if (!cJSON_IsArray(eas_ip_replace_infos)) {
            log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [eas_ip_replace_infos]");
            goto end;
        }
    ...
    if (!cJSON_IsBool(traff_corre_ind)) {
        log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [traff_corre_ind]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(sim_conn_ind)) {
        log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [sim_conn_ind]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(sim_conn_term)) {
        log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [sim_conn_term]");
        goto end;
    }
    }
    ...
    if (!up_path_chg_event_local_nonprim) {
        log_error("OpenAPI_up_path_chg_event_parseFromJSON failed [up_path_chg_event]");
        goto end;
    }
    }
    }
    ...
    if (!cJSON_IsString(steer_fun)) {
        log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [steer_fun]");
        goto end;
    }
    ...
    if (!steer_mode_dl_local_nonprim) {
        log_error("OpenAPI_steering_mode_parseFromJSON failed [steer_mode_dl]");
        goto end;
    }
    }
    ...
    if (!steer_mode_ul_local_nonprim) {
        log_error("OpenAPI_steering_mode_parseFromJSON failed [steer_mode_ul]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(mul_acc_ctrl)) {
        log_error("OpenAPI_traffic_control_data_parseFromJSON() failed [mul_acc_ctrl]");
        goto end;
    }
...
OpenAPI_traffic_control_data_t *OpenAPI_traffic_control_data_copy(OpenAPI_traffic_control_data_t *dst, OpenAPI_traffic_control_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_traffic_control_data_convertToJSON() failed");
        return NULL;
    }
