...
)
{
    ...
    log_assert(notify_item_local_var);
...
cJSON *OpenAPI_notify_item_convertToJSON(OpenAPI_notify_item_t *notify_item)
{
    ...
    if (notify_item == NULL) {
        log_error("OpenAPI_notify_item_convertToJSON() failed [NotifyItem]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!notify_item->resource_id) {
        log_error("OpenAPI_notify_item_convertToJSON() failed [resource_id]");
        return NULL;
    }
    ...
    if (!notify_item->changes) {
        log_error("OpenAPI_notify_item_convertToJSON() failed [changes]");
        return NULL;
    }
...
OpenAPI_notify_item_t *OpenAPI_notify_item_parseFromJSON(cJSON *notify_itemJSON)
{
    ...
    if (!resource_id) {
        log_error("OpenAPI_notify_item_parseFromJSON() failed [resource_id]");
        goto end;
    }
    ...
    if (!changes) {
        log_error("OpenAPI_notify_item_parseFromJSON() failed [changes]");
        goto end;
    }
...
OpenAPI_notify_item_t *OpenAPI_notify_item_copy(OpenAPI_notify_item_t *dst, OpenAPI_notify_item_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_notify_item_convertToJSON() failed");
        return NULL;
    }
