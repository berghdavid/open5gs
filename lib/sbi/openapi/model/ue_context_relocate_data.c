...
)
{
    ...
    log_assert(ue_context_relocate_data_local_var);
...
cJSON *OpenAPI_ue_context_relocate_data_convertToJSON(OpenAPI_ue_context_relocate_data_t *ue_context_relocate_data)
{
    ...
    if (ue_context_relocate_data == NULL) {
        log_error("OpenAPI_ue_context_relocate_data_convertToJSON() failed [UeContextRelocateData]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!ue_context_relocate_data->ue_context) {
        log_error("OpenAPI_ue_context_relocate_data_convertToJSON() failed [ue_context]");
        return NULL;
    }
    ...
    if (!ue_context_relocate_data->target_id) {
        log_error("OpenAPI_ue_context_relocate_data_convertToJSON() failed [target_id]");
        return NULL;
    }
    ...
    if (!ue_context_relocate_data->source_to_target_data) {
        log_error("OpenAPI_ue_context_relocate_data_convertToJSON() failed [source_to_target_data]");
        return NULL;
    }
    ...
    if (!ue_context_relocate_data->forward_relocation_request) {
        log_error("OpenAPI_ue_context_relocate_data_convertToJSON() failed [forward_relocation_request]");
        return NULL;
    }
    ...
    if (pdu_session_listList == NULL) {
        log_error("OpenAPI_ue_context_relocate_data_convertToJSON() failed [pdu_session_list]");
        goto end;
    }
    OpenAPI_list_for_each(ue_context_relocate_data->pdu_session_list, node) {
    ...
    if (ue_radio_capability_local_JSON == NULL) {
        log_error("OpenAPI_ue_context_relocate_data_convertToJSON() failed [ue_radio_capability]");
        goto end;
    }
    ...
    if (ngap_cause_local_JSON == NULL) {
        log_error("OpenAPI_ue_context_relocate_data_convertToJSON() failed [ngap_cause]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", ue_context_relocate_data->supported_features) == NULL) {
        log_error("OpenAPI_ue_context_relocate_data_convertToJSON() failed [supported_features]");
        goto end;
    }
    }
...
OpenAPI_ue_context_relocate_data_t *OpenAPI_ue_context_relocate_data_parseFromJSON(cJSON *ue_context_relocate_dataJSON)
{
    ...
    if (!ue_context) {
        log_error("OpenAPI_ue_context_relocate_data_parseFromJSON() failed [ue_context]");
        goto end;
    }
    ...
    if (!target_id) {
        log_error("OpenAPI_ue_context_relocate_data_parseFromJSON() failed [target_id]");
        goto end;
    }
    ...
    if (!source_to_target_data) {
        log_error("OpenAPI_ue_context_relocate_data_parseFromJSON() failed [source_to_target_data]");
        goto end;
    }
    ...
    if (!forward_relocation_request) {
        log_error("OpenAPI_ue_context_relocate_data_parseFromJSON() failed [forward_relocation_request]");
        goto end;
    }
    ...
    if (!forward_relocation_request_local_nonprim) {
        log_error("OpenAPI_ref_to_binary_data_parseFromJSON failed [forward_relocation_request]");
        goto end;
    }
    ...
    if (pdu_session_list) {
        ...
        if (!cJSON_IsArray(pdu_session_list)) {
            log_error("OpenAPI_ue_context_relocate_data_parseFromJSON() failed [pdu_session_list]");
            goto end;
        }
    ...
    if (!cJSON_IsString(supported_features) && !cJSON_IsNull(supported_features)) {
        log_error("OpenAPI_ue_context_relocate_data_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
...
OpenAPI_ue_context_relocate_data_t *OpenAPI_ue_context_relocate_data_copy(OpenAPI_ue_context_relocate_data_t *dst, OpenAPI_ue_context_relocate_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_ue_context_relocate_data_convertToJSON() failed");
        return NULL;
    }
