...
)
{
    ...
    log_assert(bsf_subscription_resp_local_var);
...
cJSON *OpenAPI_bsf_subscription_resp_convertToJSON(OpenAPI_bsf_subscription_resp_t *bsf_subscription_resp)
{
    ...
    if (bsf_subscription_resp == NULL) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed [BsfSubscriptionResp]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (bsf_subscription_resp->events == OpenAPI_bsf_event_NULL) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed [events]");
        return NULL;
    }
    ...
    if (!bsf_subscription_resp->notif_uri) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed [notif_uri]");
        return NULL;
    }
    ...
    if (!bsf_subscription_resp->notif_corre_id) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed [notif_corre_id]");
        return NULL;
    }
    ...
    if (!bsf_subscription_resp->supi) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed [supi]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "gpsi", bsf_subscription_resp->gpsi) == NULL) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed [gpsi]");
        goto end;
    }
    }
    ...
    if (snssai_dnn_pairs_local_JSON == NULL) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed [snssai_dnn_pairs]");
        goto end;
    }
    ...
    if (add_snssai_dnn_pairsList == NULL) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed [add_snssai_dnn_pairs]");
        goto end;
    }
    OpenAPI_list_for_each(bsf_subscription_resp->add_snssai_dnn_pairs, node) {
    ...
    if (cJSON_AddStringToObject(item, "suppFeat", bsf_subscription_resp->supp_feat) == NULL) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed [supp_feat]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "pcfId", bsf_subscription_resp->pcf_id) == NULL) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed [pcf_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "pcfSetId", bsf_subscription_resp->pcf_set_id) == NULL) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed [pcf_set_id]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "bindLevel", OpenAPI_binding_level_ToString(bsf_subscription_resp->bind_level)) == NULL) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed [bind_level]");
        goto end;
    }
    }

    if (!bsf_subscription_resp->event_notifs) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed [event_notifs]");
        return NULL;
    }
...
OpenAPI_bsf_subscription_resp_t *OpenAPI_bsf_subscription_resp_parseFromJSON(cJSON *bsf_subscription_respJSON)
{
    ...
    if (!events) {
        log_error("OpenAPI_bsf_subscription_resp_parseFromJSON() failed [events]");
        goto end;
    }
        ...
        cJSON_ArrayForEach(events_local, events) {
            ...
            if (!localEnum) {
                log_info("Enum value \"%s\" for field \"events\" is not supported. Ignoring it ...",
        ...
        if (eventsList->count == 0) {
            log_error("OpenAPI_bsf_subscription_resp_parseFromJSON() failed: Expected eventsList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }

    notif_uri = cJSON_GetObjectItemCaseSensitive(bsf_subscription_respJSON, "notifUri");
    if (!notif_uri) {
        log_error("OpenAPI_bsf_subscription_resp_parseFromJSON() failed [notif_uri]");
        goto end;
    }
    ...
    if (!notif_corre_id) {
        log_error("OpenAPI_bsf_subscription_resp_parseFromJSON() failed [notif_corre_id]");
        goto end;
    }
    ...
    if (!supi) {
        log_error("OpenAPI_bsf_subscription_resp_parseFromJSON() failed [supi]");
        goto end;
    }
    ...
    if (!cJSON_IsString(gpsi) && !cJSON_IsNull(gpsi)) {
        log_error("OpenAPI_bsf_subscription_resp_parseFromJSON() failed [gpsi]");
        goto end;
    }
    }
    ...
    if (!snssai_dnn_pairs_local_nonprim) {
        log_error("OpenAPI_snssai_dnn_pair_parseFromJSON failed [snssai_dnn_pairs]");
        goto end;
    }
    }
    ...
    if (add_snssai_dnn_pairs) {
        ...
        if (!cJSON_IsArray(add_snssai_dnn_pairs)) {
            log_error("OpenAPI_bsf_subscription_resp_parseFromJSON() failed [add_snssai_dnn_pairs]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(add_snssai_dnn_pairs_local, add_snssai_dnn_pairs) {
            ...
            if (!add_snssai_dnn_pairsItem) {
                log_error("No add_snssai_dnn_pairsItem");
                goto end;
            }
    ...
    if (!cJSON_IsString(supp_feat) && !cJSON_IsNull(supp_feat)) {
        log_error("OpenAPI_bsf_subscription_resp_parseFromJSON() failed [supp_feat]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(pcf_id) && !cJSON_IsNull(pcf_id)) {
        log_error("OpenAPI_bsf_subscription_resp_parseFromJSON() failed [pcf_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(pcf_set_id) && !cJSON_IsNull(pcf_set_id)) {
        log_error("OpenAPI_bsf_subscription_resp_parseFromJSON() failed [pcf_set_id]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(bind_level)) {
        log_error("OpenAPI_bsf_subscription_resp_parseFromJSON() failed [bind_level]");
        goto end;
    }
    ...
    if (!event_notifs) {
        log_error("OpenAPI_bsf_subscription_resp_parseFromJSON() failed [event_notifs]");
        goto end;
    }
...
OpenAPI_bsf_subscription_resp_t *OpenAPI_bsf_subscription_resp_copy(OpenAPI_bsf_subscription_resp_t *dst, OpenAPI_bsf_subscription_resp_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_bsf_subscription_resp_convertToJSON() failed");
        return NULL;
    }
