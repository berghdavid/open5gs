...
)
{
    ...
    log_assert(tmgi_range_local_var);
...
cJSON *OpenAPI_tmgi_range_convertToJSON(OpenAPI_tmgi_range_t *tmgi_range)
{
    ...
    if (tmgi_range == NULL) {
        log_error("OpenAPI_tmgi_range_convertToJSON() failed [TmgiRange]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!tmgi_range->mbs_service_id_start) {
        log_error("OpenAPI_tmgi_range_convertToJSON() failed [mbs_service_id_start]");
        return NULL;
    }
    ...
    if (!tmgi_range->mbs_service_id_end) {
        log_error("OpenAPI_tmgi_range_convertToJSON() failed [mbs_service_id_end]");
        return NULL;
    }
    ...
    if (!tmgi_range->plmn_id) {
        log_error("OpenAPI_tmgi_range_convertToJSON() failed [plmn_id]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "nid", tmgi_range->nid) == NULL) {
        log_error("OpenAPI_tmgi_range_convertToJSON() failed [nid]");
        goto end;
    }
    }
...
OpenAPI_tmgi_range_t *OpenAPI_tmgi_range_parseFromJSON(cJSON *tmgi_rangeJSON)
{
    ...
    if (!mbs_service_id_start) {
        log_error("OpenAPI_tmgi_range_parseFromJSON() failed [mbs_service_id_start]");
        goto end;
    }
    ...
    if (!mbs_service_id_end) {
        log_error("OpenAPI_tmgi_range_parseFromJSON() failed [mbs_service_id_end]");
        goto end;
    }
    ...
    if (!plmn_id) {
        log_error("OpenAPI_tmgi_range_parseFromJSON() failed [plmn_id]");
        goto end;
    }
    ...
    if (!cJSON_IsString(nid) && !cJSON_IsNull(nid)) {
        log_error("OpenAPI_tmgi_range_parseFromJSON() failed [nid]");
        goto end;
    }
    }
...
OpenAPI_tmgi_range_t *OpenAPI_tmgi_range_copy(OpenAPI_tmgi_range_t *dst, OpenAPI_tmgi_range_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_tmgi_range_convertToJSON() failed");
        return NULL;
    }
