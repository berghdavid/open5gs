...
)
{
    ...
    log_assert(dispersion_info_local_var);
...
cJSON *OpenAPI_dispersion_info_convertToJSON(OpenAPI_dispersion_info_t *dispersion_info)
{
    ...
    if (dispersion_info == NULL) {
        log_error("OpenAPI_dispersion_info_convertToJSON() failed [DispersionInfo]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!dispersion_info->ts_start) {
        log_error("OpenAPI_dispersion_info_convertToJSON() failed [ts_start]");
        return NULL;
    }
    ...
    if (cJSON_AddNumberToObject(item, "tsDuration", dispersion_info->ts_duration) == NULL) {
        log_error("OpenAPI_dispersion_info_convertToJSON() failed [ts_duration]");
        goto end;
    }

    if (!dispersion_info->disper_collects) {
        log_error("OpenAPI_dispersion_info_convertToJSON() failed [disper_collects]");
        return NULL;
    }
    ...
    if (!dispersion_info->disper_type) {
        log_error("OpenAPI_dispersion_info_convertToJSON() failed [disper_type]");
        return NULL;
    }
...
OpenAPI_dispersion_info_t *OpenAPI_dispersion_info_parseFromJSON(cJSON *dispersion_infoJSON)
{
    ...
    if (!ts_start) {
        log_error("OpenAPI_dispersion_info_parseFromJSON() failed [ts_start]");
        goto end;
    }
    ...
    if (!ts_duration) {
        log_error("OpenAPI_dispersion_info_parseFromJSON() failed [ts_duration]");
        goto end;
    }
    ...
    if (!disper_collects) {
        log_error("OpenAPI_dispersion_info_parseFromJSON() failed [disper_collects]");
        goto end;
    }
        ...
        cJSON_ArrayForEach(disper_collects_local, disper_collects) {
            ...
            if (!disper_collectsItem) {
                log_error("No disper_collectsItem");
                goto end;
            }
    ...
    if (!disper_type) {
        log_error("OpenAPI_dispersion_info_parseFromJSON() failed [disper_type]");
        goto end;
    }
...
OpenAPI_dispersion_info_t *OpenAPI_dispersion_info_copy(OpenAPI_dispersion_info_t *dst, OpenAPI_dispersion_info_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_dispersion_info_convertToJSON() failed");
        return NULL;
    }
