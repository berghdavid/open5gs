...
)
{
    ...
    log_assert(trace_data_local_var);
...
cJSON *OpenAPI_trace_data_convertToJSON(OpenAPI_trace_data_t *trace_data)
{
    ...
    if (trace_data == NULL) {
        log_error("OpenAPI_trace_data_convertToJSON() failed [TraceData]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!trace_data->trace_ref) {
        log_error("OpenAPI_trace_data_convertToJSON() failed [trace_ref]");
        return NULL;
    }
    ...
    if (trace_data->trace_depth == OpenAPI_trace_depth_NULL) {
        log_error("OpenAPI_trace_data_convertToJSON() failed [trace_depth]");
        return NULL;
    }
    ...
    if (!trace_data->ne_type_list) {
        log_error("OpenAPI_trace_data_convertToJSON() failed [ne_type_list]");
        return NULL;
    }
    ...
    if (!trace_data->event_list) {
        log_error("OpenAPI_trace_data_convertToJSON() failed [event_list]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "collectionEntityIpv4Addr", trace_data->collection_entity_ipv4_addr) == NULL) {
        log_error("OpenAPI_trace_data_convertToJSON() failed [collection_entity_ipv4_addr]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "collectionEntityIpv6Addr", trace_data->collection_entity_ipv6_addr) == NULL) {
        log_error("OpenAPI_trace_data_convertToJSON() failed [collection_entity_ipv6_addr]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "interfaceList", trace_data->interface_list) == NULL) {
        log_error("OpenAPI_trace_data_convertToJSON() failed [interface_list]");
        goto end;
    }
    }
...
OpenAPI_trace_data_t *OpenAPI_trace_data_parseFromJSON(cJSON *trace_dataJSON)
{
    ...
    if (!trace_ref) {
        log_error("OpenAPI_trace_data_parseFromJSON() failed [trace_ref]");
        goto end;
    }
    ...
    if (!trace_depth) {
        log_error("OpenAPI_trace_data_parseFromJSON() failed [trace_depth]");
        goto end;
    }
    ...
    if (!ne_type_list) {
        log_error("OpenAPI_trace_data_parseFromJSON() failed [ne_type_list]");
        goto end;
    }
    ...
    if (!event_list) {
        log_error("OpenAPI_trace_data_parseFromJSON() failed [event_list]");
        goto end;
    }
    ...
    if (!cJSON_IsString(collection_entity_ipv4_addr) && !cJSON_IsNull(collection_entity_ipv4_addr)) {
        log_error("OpenAPI_trace_data_parseFromJSON() failed [collection_entity_ipv4_addr]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(collection_entity_ipv6_addr) && !cJSON_IsNull(collection_entity_ipv6_addr)) {
        log_error("OpenAPI_trace_data_parseFromJSON() failed [collection_entity_ipv6_addr]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(interface_list) && !cJSON_IsNull(interface_list)) {
        log_error("OpenAPI_trace_data_parseFromJSON() failed [interface_list]");
        goto end;
    }
    }
...
OpenAPI_trace_data_t *OpenAPI_trace_data_copy(OpenAPI_trace_data_t *dst, OpenAPI_trace_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_trace_data_convertToJSON() failed");
        return NULL;
    }
