...
cJSON *OpenAPI_lpi_convertToJSON(OpenAPI_lpi_t *lpi)
{
    ...
    if (lpi == NULL) {
        log_error("OpenAPI_lpi_convertToJSON() failed [Lpi]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (lpi->location_privacy_ind == OpenAPI_location_privacy_ind_NULL) {
        log_error("OpenAPI_lpi_convertToJSON() failed [location_privacy_ind]");
        return NULL;
    }
    ...
    if (valid_time_period_local_JSON == NULL) {
        log_error("OpenAPI_lpi_convertToJSON() failed [valid_time_period]");
        goto end;
    }
...
OpenAPI_lpi_t *OpenAPI_lpi_parseFromJSON(cJSON *lpiJSON)
{
    ...
    if (!location_privacy_ind) {
        log_error("OpenAPI_lpi_parseFromJSON() failed [location_privacy_ind]");
        goto end;
    }
...
OpenAPI_lpi_t *OpenAPI_lpi_copy(OpenAPI_lpi_t *dst, OpenAPI_lpi_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_lpi_convertToJSON() failed");
        return NULL;
    }
