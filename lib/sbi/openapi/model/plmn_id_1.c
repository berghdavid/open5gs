...
)
{
    ...
    log_assert(plmn_id_1_local_var);
...
cJSON *OpenAPI_plmn_id_1_convertToJSON(OpenAPI_plmn_id_1_t *plmn_id_1)
{
    ...
    if (plmn_id_1 == NULL) {
        log_error("OpenAPI_plmn_id_1_convertToJSON() failed [PlmnId_1]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!plmn_id_1->mcc) {
        log_error("OpenAPI_plmn_id_1_convertToJSON() failed [mcc]");
        return NULL;
    }
    ...
    if (!plmn_id_1->mnc) {
        log_error("OpenAPI_plmn_id_1_convertToJSON() failed [mnc]");
        return NULL;
    }
...
OpenAPI_plmn_id_1_t *OpenAPI_plmn_id_1_parseFromJSON(cJSON *plmn_id_1JSON)
{
    ...
    if (!mcc) {
        log_error("OpenAPI_plmn_id_1_parseFromJSON() failed [mcc]");
        goto end;
    }
    ...
    if (!mnc) {
        log_error("OpenAPI_plmn_id_1_parseFromJSON() failed [mnc]");
        goto end;
    }
...
OpenAPI_plmn_id_1_t *OpenAPI_plmn_id_1_copy(OpenAPI_plmn_id_1_t *dst, OpenAPI_plmn_id_1_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_plmn_id_1_convertToJSON() failed");
        return NULL;
    }
