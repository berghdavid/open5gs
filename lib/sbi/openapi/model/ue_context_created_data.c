...
)
{
    ...
    log_assert(ue_context_created_data_local_var);
...
cJSON *OpenAPI_ue_context_created_data_convertToJSON(OpenAPI_ue_context_created_data_t *ue_context_created_data)
{
    ...
    if (ue_context_created_data == NULL) {
        log_error("OpenAPI_ue_context_created_data_convertToJSON() failed [UeContextCreatedData]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!ue_context_created_data->ue_context) {
        log_error("OpenAPI_ue_context_created_data_convertToJSON() failed [ue_context]");
        return NULL;
    }
    ...
    if (!ue_context_created_data->target_to_source_data) {
        log_error("OpenAPI_ue_context_created_data_convertToJSON() failed [target_to_source_data]");
        return NULL;
    }
    ...
    if (!ue_context_created_data->pdu_session_list) {
        log_error("OpenAPI_ue_context_created_data_convertToJSON() failed [pdu_session_list]");
        return NULL;
    }
    ...
    if (failed_session_listList == NULL) {
        log_error("OpenAPI_ue_context_created_data_convertToJSON() failed [failed_session_list]");
        goto end;
    }
    OpenAPI_list_for_each(ue_context_created_data->failed_session_list, node) {
        cJSON *itemLocal = OpenAPI_n2_sm_information_convertToJSON(node->data);
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", ue_context_created_data->supported_features) == NULL) {
        log_error("OpenAPI_ue_context_created_data_convertToJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (cJSON_AddBoolToObject(item, "pcfReselectedInd", ue_context_created_data->pcf_reselected_ind) == NULL) {
        log_error("OpenAPI_ue_context_created_data_convertToJSON() failed [pcf_reselected_ind]");
        goto end;
    }
    }
    ...
    if (analytics_not_used_listList == NULL) {
        log_error("OpenAPI_ue_context_created_data_convertToJSON() failed [analytics_not_used_list]");
        goto end;
    }
    OpenAPI_list_for_each(ue_context_created_data->analytics_not_used_list, node) {
...
OpenAPI_ue_context_created_data_t *OpenAPI_ue_context_created_data_parseFromJSON(cJSON *ue_context_created_dataJSON)
{
    ...
    if (!ue_context) {
        log_error("OpenAPI_ue_context_created_data_parseFromJSON() failed [ue_context]");
        goto end;
    }
    ...
    if (!target_to_source_data) {
        log_error("OpenAPI_ue_context_created_data_parseFromJSON() failed [target_to_source_data]");
        goto end;
    }
    ...
    if (!target_to_source_data_local_nonprim) {
        log_error("OpenAPI_n2_info_content_parseFromJSON failed [target_to_source_data]");
        goto end;
    }

    pdu_session_list = cJSON_GetObjectItemCaseSensitive(ue_context_created_dataJSON, "pduSessionList");
    if (!pdu_session_list) {
        log_error("OpenAPI_ue_context_created_data_parseFromJSON() failed [pdu_session_list]");
        goto end;
    }
    ...
    if (failed_session_list) {
        ...
        if (!cJSON_IsArray(failed_session_list)) {
            log_error("OpenAPI_ue_context_created_data_parseFromJSON() failed [failed_session_list]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(failed_session_list_local, failed_session_list) {
            ...
            if (!failed_session_listItem) {
                log_error("No failed_session_listItem");
                goto end;
            }
    ...
    if (!cJSON_IsString(supported_features) && !cJSON_IsNull(supported_features)) {
        log_error("OpenAPI_ue_context_created_data_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(pcf_reselected_ind)) {
        log_error("OpenAPI_ue_context_created_data_parseFromJSON() failed [pcf_reselected_ind]");
        goto end;
    }
    }
    ...
    if (analytics_not_used_list) {
        ...
        if (!cJSON_IsArray(analytics_not_used_list)) {
            log_error("OpenAPI_ue_context_created_data_parseFromJSON() failed [analytics_not_used_list]");
            goto end;
        }
...
OpenAPI_ue_context_created_data_t *OpenAPI_ue_context_created_data_copy(OpenAPI_ue_context_created_data_t *dst, OpenAPI_ue_context_created_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_ue_context_created_data_convertToJSON() failed");
        return NULL;
    }
