...
)
{
    ...
    log_assert(av_eap_aka_prime_local_var);
...
cJSON *OpenAPI_av_eap_aka_prime_convertToJSON(OpenAPI_av_eap_aka_prime_t *av_eap_aka_prime)
{
    ...
    if (av_eap_aka_prime == NULL) {
        log_error("OpenAPI_av_eap_aka_prime_convertToJSON() failed [AvEapAkaPrime]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (av_eap_aka_prime->av_type == OpenAPI_av_type_NULL) {
        log_error("OpenAPI_av_eap_aka_prime_convertToJSON() failed [av_type]");
        return NULL;
    }
    ...
    if (!av_eap_aka_prime->rand) {
        log_error("OpenAPI_av_eap_aka_prime_convertToJSON() failed [rand]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "xres", av_eap_aka_prime->xres) == NULL) {
        log_error("OpenAPI_av_eap_aka_prime_convertToJSON() failed [xres]");
        goto end;
    }
    }

    if (!av_eap_aka_prime->autn) {
        log_error("OpenAPI_av_eap_aka_prime_convertToJSON() failed [autn]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "ckPrime", av_eap_aka_prime->ck_prime) == NULL) {
        log_error("OpenAPI_av_eap_aka_prime_convertToJSON() failed [ck_prime]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "ikPrime", av_eap_aka_prime->ik_prime) == NULL) {
        log_error("OpenAPI_av_eap_aka_prime_convertToJSON() failed [ik_prime]");
        goto end;
    }
    }
...
OpenAPI_av_eap_aka_prime_t *OpenAPI_av_eap_aka_prime_parseFromJSON(cJSON *av_eap_aka_primeJSON)
{
    ...
    if (!av_type) {
        log_error("OpenAPI_av_eap_aka_prime_parseFromJSON() failed [av_type]");
        goto end;
    }
    ...
    if (!rand) {
        log_error("OpenAPI_av_eap_aka_prime_parseFromJSON() failed [rand]");
        goto end;
    }
    ...
    if (!cJSON_IsString(xres) && !cJSON_IsNull(xres)) {
        log_error("OpenAPI_av_eap_aka_prime_parseFromJSON() failed [xres]");
        goto end;
    }
    }
    ...
    if (!autn) {
        log_error("OpenAPI_av_eap_aka_prime_parseFromJSON() failed [autn]");
        goto end;
    }
    ...
    if (!cJSON_IsString(ck_prime) && !cJSON_IsNull(ck_prime)) {
        log_error("OpenAPI_av_eap_aka_prime_parseFromJSON() failed [ck_prime]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(ik_prime) && !cJSON_IsNull(ik_prime)) {
        log_error("OpenAPI_av_eap_aka_prime_parseFromJSON() failed [ik_prime]");
        goto end;
    }
    }
...
OpenAPI_av_eap_aka_prime_t *OpenAPI_av_eap_aka_prime_copy(OpenAPI_av_eap_aka_prime_t *dst, OpenAPI_av_eap_aka_prime_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_av_eap_aka_prime_convertToJSON() failed");
        return NULL;
    }
