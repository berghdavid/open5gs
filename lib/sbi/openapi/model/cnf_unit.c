...
cJSON *OpenAPI_cnf_unit_convertToJSON(OpenAPI_cnf_unit_t *cnf_unit)
{
    ...
    if (cnf_unit == NULL) {
        log_error("OpenAPI_cnf_unit_convertToJSON() failed [CnfUnit]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!cnf_unit->cnf_unit) {
        log_error("OpenAPI_cnf_unit_convertToJSON() failed [cnf_unit]");
        return NULL;
    }
...
OpenAPI_cnf_unit_t *OpenAPI_cnf_unit_parseFromJSON(cJSON *cnf_unitJSON)
{
    ...
    if (!cnf_unit) {
        log_error("OpenAPI_cnf_unit_parseFromJSON() failed [cnf_unit]");
        goto end;
    }
...
OpenAPI_cnf_unit_t *OpenAPI_cnf_unit_copy(OpenAPI_cnf_unit_t *dst, OpenAPI_cnf_unit_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_cnf_unit_convertToJSON() failed");
        return NULL;
    }
