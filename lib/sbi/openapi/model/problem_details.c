...
)
{
    ...
    log_assert(problem_details_local_var);
...
cJSON *OpenAPI_problem_details_convertToJSON(OpenAPI_problem_details_t *problem_details)
{
    ...
    if (problem_details == NULL) {
        log_error("OpenAPI_problem_details_convertToJSON() failed [ProblemDetails]");
        return NULL;
    }
    ...
    if (cJSON_AddStringToObject(item, "type", problem_details->type) == NULL) {
        log_error("OpenAPI_problem_details_convertToJSON() failed [type]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "title", problem_details->title) == NULL) {
        log_error("OpenAPI_problem_details_convertToJSON() failed [title]");
        goto end;
    }
    }
    ...
    if (cJSON_AddNumberToObject(item, "status", problem_details->status) == NULL) {
        log_error("OpenAPI_problem_details_convertToJSON() failed [status]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "detail", problem_details->detail) == NULL) {
        log_error("OpenAPI_problem_details_convertToJSON() failed [detail]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "instance", problem_details->instance) == NULL) {
        log_error("OpenAPI_problem_details_convertToJSON() failed [instance]");
        goto end;
    }
    }
    ...
    if (cJSON_AddStringToObject(item, "cause", problem_details->cause) == NULL) {
        log_error("OpenAPI_problem_details_convertToJSON() failed [cause]");
        goto end;
    }
    }
    ...
    if (invalid_paramsList == NULL) {
        log_error("OpenAPI_problem_details_convertToJSON() failed [invalid_params]");
        goto end;
    }
    OpenAPI_list_for_each(problem_details->invalid_params, node) {
    ...
    if (cJSON_AddStringToObject(item, "supportedFeatures", problem_details->supported_features) == NULL) {
        log_error("OpenAPI_problem_details_convertToJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (access_token_error_local_JSON == NULL) {
        log_error("OpenAPI_problem_details_convertToJSON() failed [access_token_error]");
        goto end;
    }
    ...
    if (access_token_request_local_JSON == NULL) {
        log_error("OpenAPI_problem_details_convertToJSON() failed [access_token_request]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "nrfId", problem_details->nrf_id) == NULL) {
        log_error("OpenAPI_problem_details_convertToJSON() failed [nrf_id]");
        goto end;
    }
    }
...
OpenAPI_problem_details_t *OpenAPI_problem_details_parseFromJSON(cJSON *problem_detailsJSON)
{
    ...
    if (!cJSON_IsString(type) && !cJSON_IsNull(type)) {
        log_error("OpenAPI_problem_details_parseFromJSON() failed [type]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(title) && !cJSON_IsNull(title)) {
        log_error("OpenAPI_problem_details_parseFromJSON() failed [title]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsNumber(status)) {
        log_error("OpenAPI_problem_details_parseFromJSON() failed [status]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(detail) && !cJSON_IsNull(detail)) {
        log_error("OpenAPI_problem_details_parseFromJSON() failed [detail]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(instance) && !cJSON_IsNull(instance)) {
        log_error("OpenAPI_problem_details_parseFromJSON() failed [instance]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(cause) && !cJSON_IsNull(cause)) {
        log_error("OpenAPI_problem_details_parseFromJSON() failed [cause]");
        goto end;
    }
    }
    ...
    if (invalid_params) {
        ...
        if (!cJSON_IsArray(invalid_params)) {
            log_error("OpenAPI_problem_details_parseFromJSON() failed [invalid_params]");
            goto end;
        }
    ...
    if (!cJSON_IsString(supported_features) && !cJSON_IsNull(supported_features)) {
        log_error("OpenAPI_problem_details_parseFromJSON() failed [supported_features]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsString(nrf_id) && !cJSON_IsNull(nrf_id)) {
        log_error("OpenAPI_problem_details_parseFromJSON() failed [nrf_id]");
        goto end;
    }
    }
...
OpenAPI_problem_details_t *OpenAPI_problem_details_copy(OpenAPI_problem_details_t *dst, OpenAPI_problem_details_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_problem_details_convertToJSON() failed");
        return NULL;
    }
