...
cJSON *OpenAPI_cag_info_convertToJSON(OpenAPI_cag_info_t *cag_info)
{
    ...
    if (cag_info == NULL) {
        log_error("OpenAPI_cag_info_convertToJSON() failed [CagInfo]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!cag_info->allowed_cag_list) {
        log_error("OpenAPI_cag_info_convertToJSON() failed [allowed_cag_list]");
        return NULL;
    }
    ...
    if (cJSON_AddBoolToObject(item, "cagOnlyIndicator", cag_info->cag_only_indicator) == NULL) {
        log_error("OpenAPI_cag_info_convertToJSON() failed [cag_only_indicator]");
        goto end;
    }
    }
...
OpenAPI_cag_info_t *OpenAPI_cag_info_parseFromJSON(cJSON *cag_infoJSON)
{
    ...
    if (!allowed_cag_list) {
        log_error("OpenAPI_cag_info_parseFromJSON() failed [allowed_cag_list]");
        goto end;
    }
    ...
    if (!cJSON_IsBool(cag_only_indicator)) {
        log_error("OpenAPI_cag_info_parseFromJSON() failed [cag_only_indicator]");
        goto end;
    }
    }
...
OpenAPI_cag_info_t *OpenAPI_cag_info_copy(OpenAPI_cag_info_t *dst, OpenAPI_cag_info_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_cag_info_convertToJSON() failed");
        return NULL;
    }
