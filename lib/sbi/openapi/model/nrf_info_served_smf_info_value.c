...
)
{
    ...
    log_assert(nrf_info_served_smf_info_value_local_var);
...
cJSON *OpenAPI_nrf_info_served_smf_info_value_convertToJSON(OpenAPI_nrf_info_served_smf_info_value_t *nrf_info_served_smf_info_value)
{
    ...
    if (nrf_info_served_smf_info_value == NULL) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed [NrfInfo_servedSmfInfo_value]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!nrf_info_served_smf_info_value->s_nssai_smf_info_list) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed [s_nssai_smf_info_list]");
        return NULL;
    }
    ...
    if (tai_listList == NULL) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed [tai_list]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_smf_info_value->tai_list, node) {
    ...
    if (tai_range_listList == NULL) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed [tai_range_list]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_smf_info_value->tai_range_list, node) {
    ...
    if (cJSON_AddStringToObject(item, "pgwFqdn", nrf_info_served_smf_info_value->pgw_fqdn) == NULL) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed [pgw_fqdn]");
        goto end;
    }
    }
    ...
    if (pgw_ip_addr_listList == NULL) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed [pgw_ip_addr_list]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_smf_info_value->pgw_ip_addr_list, node) {
    ...
    if (access_typeList == NULL) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed [access_type]");
        goto end;
    }
    ...
    if (cJSON_AddNumberToObject(item, "priority", nrf_info_served_smf_info_value->priority) == NULL) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed [priority]");
        goto end;
    }
    }
    ...
    if (cJSON_AddBoolToObject(item, "vsmfSupportInd", nrf_info_served_smf_info_value->vsmf_support_ind) == NULL) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed [vsmf_support_ind]");
        goto end;
    }
    }
    ...
    if (pgw_fqdn_listList == NULL) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed [pgw_fqdn_list]");
        goto end;
    }
    OpenAPI_list_for_each(nrf_info_served_smf_info_value->pgw_fqdn_list, node) {
    ...
    if (cJSON_AddBoolToObject(item, "smfOnboardingCapability", nrf_info_served_smf_info_value->smf_onboarding_capability) == NULL) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed [smf_onboarding_capability]");
        goto end;
    }
    }
    ...
    if (cJSON_AddBoolToObject(item, "ismfSupportInd", nrf_info_served_smf_info_value->ismf_support_ind) == NULL) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed [ismf_support_ind]");
        goto end;
    }
    }
    ...
    if (cJSON_AddBoolToObject(item, "smfUPRPCapability", nrf_info_served_smf_info_value->smf_uprp_capability) == NULL) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed [smf_uprp_capability]");
        goto end;
    }
    }
...
OpenAPI_nrf_info_served_smf_info_value_t *OpenAPI_nrf_info_served_smf_info_value_parseFromJSON(cJSON *nrf_info_served_smf_info_valueJSON)
{
    ...
    if (!s_nssai_smf_info_list) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_parseFromJSON() failed [s_nssai_smf_info_list]");
        goto end;
    }
        ...
        cJSON_ArrayForEach(s_nssai_smf_info_list_local, s_nssai_smf_info_list) {
            ...
            if (!s_nssai_smf_info_listItem) {
                log_error("No s_nssai_smf_info_listItem");
                goto end;
            }
    ...
    if (tai_list) {
        ...
        if (!cJSON_IsArray(tai_list)) {
            log_error("OpenAPI_nrf_info_served_smf_info_value_parseFromJSON() failed [tai_list]");
            goto end;
        }
    ...
    if (tai_range_list) {
        ...
        if (!cJSON_IsArray(tai_range_list)) {
            log_error("OpenAPI_nrf_info_served_smf_info_value_parseFromJSON() failed [tai_range_list]");
            goto end;
        }
    ...
    if (!cJSON_IsString(pgw_fqdn) && !cJSON_IsNull(pgw_fqdn)) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_parseFromJSON() failed [pgw_fqdn]");
        goto end;
    }
    }
    ...
    if (pgw_ip_addr_list) {
        ...
        if (!cJSON_IsArray(pgw_ip_addr_list)) {
            log_error("OpenAPI_nrf_info_served_smf_info_value_parseFromJSON() failed [pgw_ip_addr_list]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(pgw_ip_addr_list_local, pgw_ip_addr_list) {
            ...
            if (!pgw_ip_addr_listItem) {
                log_error("No pgw_ip_addr_listItem");
                goto end;
            }
    ...
    if (access_type) {
        ...
        if (!cJSON_IsArray(access_type)) {
            log_error("OpenAPI_nrf_info_served_smf_info_value_parseFromJSON() failed [access_type]");
            goto end;
        }
        ...
        if (access_typeList->count == 0) {
            log_error("OpenAPI_nrf_info_served_smf_info_value_parseFromJSON() failed: Expected access_typeList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
    ...
    if (!cJSON_IsNumber(priority)) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_parseFromJSON() failed [priority]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(vsmf_support_ind)) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_parseFromJSON() failed [vsmf_support_ind]");
        goto end;
    }
    }
    ...
    if (pgw_fqdn_list) {
        ...
        if (!cJSON_IsArray(pgw_fqdn_list)) {
            log_error("OpenAPI_nrf_info_served_smf_info_value_parseFromJSON() failed [pgw_fqdn_list]");
            goto end;
        }
    ...
    if (!cJSON_IsBool(smf_onboarding_capability)) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_parseFromJSON() failed [smf_onboarding_capability]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(ismf_support_ind)) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_parseFromJSON() failed [ismf_support_ind]");
        goto end;
    }
    }
    ...
    if (!cJSON_IsBool(smf_uprp_capability)) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_parseFromJSON() failed [smf_uprp_capability]");
        goto end;
    }
    }
...
OpenAPI_nrf_info_served_smf_info_value_t *OpenAPI_nrf_info_served_smf_info_value_copy(OpenAPI_nrf_info_served_smf_info_value_t *dst, OpenAPI_nrf_info_served_smf_info_value_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_nrf_info_served_smf_info_value_convertToJSON() failed");
        return NULL;
    }
