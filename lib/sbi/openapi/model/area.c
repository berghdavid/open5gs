...
cJSON *OpenAPI_area_convertToJSON(OpenAPI_area_t *area)
{
    ...
    if (area == NULL) {
        log_error("OpenAPI_area_convertToJSON() failed [Area]");
        return NULL;
    }
    ...
    if (tacsList == NULL) {
        log_error("OpenAPI_area_convertToJSON() failed [tacs]");
        goto end;
    }
    OpenAPI_list_for_each(area->tacs, node) {
    ...
    if (cJSON_AddStringToObject(item, "areaCode", area->area_code) == NULL) {
        log_error("OpenAPI_area_convertToJSON() failed [area_code]");
        goto end;
    }
    }
...
OpenAPI_area_t *OpenAPI_area_parseFromJSON(cJSON *areaJSON)
{
    ...
    if (tacs) {
        ...
        if (!cJSON_IsArray(tacs)) {
            log_error("OpenAPI_area_parseFromJSON() failed [tacs]");
            goto end;
        }
    ...
    if (!cJSON_IsString(area_code) && !cJSON_IsNull(area_code)) {
        log_error("OpenAPI_area_parseFromJSON() failed [area_code]");
        goto end;
    }
    }
...
OpenAPI_area_t *OpenAPI_area_copy(OpenAPI_area_t *dst, OpenAPI_area_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_area_convertToJSON() failed");
        return NULL;
    }
