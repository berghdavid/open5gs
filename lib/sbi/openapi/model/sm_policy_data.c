...
)
{
    ...
    log_assert(sm_policy_data_local_var);
...
cJSON *OpenAPI_sm_policy_data_convertToJSON(OpenAPI_sm_policy_data_t *sm_policy_data)
{
    ...
    if (sm_policy_data == NULL) {
        log_error("OpenAPI_sm_policy_data_convertToJSON() failed [SmPolicyData]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (!sm_policy_data->sm_policy_snssai_data) {
        log_error("OpenAPI_sm_policy_data_convertToJSON() failed [sm_policy_snssai_data]");
        return NULL;
    }
    ...
    if (sm_policy_data->sm_policy_snssai_data) {
        OpenAPI_list_for_each(sm_policy_data->sm_policy_snssai_data, node) {
            ...
            cJSON *itemLocal = localKeyValue->value ?
                ...
                cJSON_CreateNull();
            if (itemLocal == NULL) {
                log_error("OpenAPI_sm_policy_data_convertToJSON() failed [inner]");
                goto end;
            }
    ...
    if (um_data_limits == NULL) {
        log_error("OpenAPI_sm_policy_data_convertToJSON() failed [um_data_limits]");
        goto end;
    }
    ...
    if (um_data == NULL) {
        log_error("OpenAPI_sm_policy_data_convertToJSON() failed [um_data]");
        goto end;
    }
    ...
    if (cJSON_AddStringToObject(item, "suppFeat", sm_policy_data->supp_feat) == NULL) {
        log_error("OpenAPI_sm_policy_data_convertToJSON() failed [supp_feat]");
        goto end;
    }
    }
    ...
    if (reset_idsList == NULL) {
        log_error("OpenAPI_sm_policy_data_convertToJSON() failed [reset_ids]");
        goto end;
    }
    OpenAPI_list_for_each(sm_policy_data->reset_ids, node) {
...
OpenAPI_sm_policy_data_t *OpenAPI_sm_policy_data_parseFromJSON(cJSON *sm_policy_dataJSON)
{
    ...
    if (!sm_policy_snssai_data) {
        log_error("OpenAPI_sm_policy_data_parseFromJSON() failed [sm_policy_snssai_data]");
        goto end;
    }
        ...
        if (cJSON_IsObject(sm_policy_snssai_data)) {
            ...
            cJSON_ArrayForEach(sm_policy_snssai_data_local_map, sm_policy_snssai_data) {
                ...
                if (cJSON_IsObject(localMapObject)) {
                    ...
                    log_error("OpenAPI_sm_policy_data_parseFromJSON() failed [inner]");
                    goto end;
                }
    ...
    if (um_data_limits) {
        ...
        if (!cJSON_IsObject(um_data_limits) && !cJSON_IsNull(um_data_limits)) {
            log_error("OpenAPI_sm_policy_data_parseFromJSON() failed [um_data_limits]");
            goto end;
        }
    ...
    if (um_data) {
        ...
        if (!cJSON_IsObject(um_data) && !cJSON_IsNull(um_data)) {
            log_error("OpenAPI_sm_policy_data_parseFromJSON() failed [um_data]");
            goto end;
        }
    ...
    if (!cJSON_IsString(supp_feat) && !cJSON_IsNull(supp_feat)) {
        log_error("OpenAPI_sm_policy_data_parseFromJSON() failed [supp_feat]");
        goto end;
    }
    }
    ...
    if (reset_ids) {
        ...
        if (!cJSON_IsArray(reset_ids)) {
            log_error("OpenAPI_sm_policy_data_parseFromJSON() failed [reset_ids]");
            goto end;
        }
...
OpenAPI_sm_policy_data_t *OpenAPI_sm_policy_data_copy(OpenAPI_sm_policy_data_t *dst, OpenAPI_sm_policy_data_t *src)
{
    ...
    if (!item) {
        log_error("OpenAPI_sm_policy_data_convertToJSON() failed");
        return NULL;
    }
